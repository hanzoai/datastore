 ...
 *
 *   # Simple query
 *   echo 'SELECT 1;' | ./datastore-examples parser_memory_profiler
 *   # Output: 9    2437760    2441632    3872
 *   # Format: query_length \t before \t after \t diff (bytes)
 *
 *   # Complex query
 *   echo 'SELECT a, b, c FROM t1 JOIN t2 ON t1.id = t2.id WHERE x > 10;' | ./datastore-examples parser_memory_profiler
 *
 *
 ...
 *       ./datastore-examples parser_memory_profiler --profile /tmp/query_ <<< 'SELECT 1;'
 *
 *   # This generates:
 ...
 *       echo "$query" | ./datastore-examples parser_memory_profiler
 *   done < queries.txt
 *
