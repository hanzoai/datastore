...
#define LIST_OF_SERVER_SETTINGS_WITHOUT_PATH(DECLARE, ALIAS) \
    DECLARE(InsertDeduplicationVersions, insert_deduplication_version, InsertDeduplicationVersions::COMPATIBLE_DOUBLE_HASHES, R"(
        ...
        The value `old_separate_hashes` means that Datastore will use different deduplication hashes for sync and async inserts (the same as before).
        ...
        The default would be set to `compatible_double_hashes` and then later to `new_unified_hash` in future versions of Datastore in order to complete the migration in a safe way in two reases.
    )", 0) \
    ...
    **Example**
    ...
    DECLARE(UInt64, max_snapshot_commit_thread_pool_free_size, 0, R"(If the number of idle threads in the snapshot commit thread pool exceeds `max_snapshot_commit_thread_pool_free_size`, Datastore will release resources occupied by idling threads and decrease the pool size. Threads can be created again if necessary.)", 0) \
    ...
    **Example**
    ...
    Allocations made by Datastore's own large buffers — column data, hash tables, arenas, query pipelines, IO buffers — always honour the memory limit and may throw regardless of this value. This setting only affects implicit allocations that flow through `operator new`.

    A value of `0` (default) preserves the legacy behaviour: implicit `operator new` allocations are never refused by the memory tracker, and only explicit Datastore allocations can raise `MEMORY_LIMIT_EXCEEDED`.
...
The policy on how to perform a scheduling of CPU slots specified by `concurrent_threads_soft_limit_num` and `concurrent_threads_soft_limit_ratio_to_cores`. Algorithm used to govern how limited number of CPU slots are distributed among concurrent queries. Scheduler may be changed at runtime without server restart.
    ...
    **See Also**
    ...
    Only available on Datastore Cloud. When enabled, `system.*_log` tables are backed by `SharedMergeTree` via an S3-based pipeline.
...
\
    ...
    DECLARE(Bool, enable_webterminal, true, R"(Enable the web terminal interface at the `/webterminal` HTTP endpoint. Provides an interactive `datastore-client` session in the browser via WebSocket. When `false`, requests to `/webterminal` return HTTP status `403 Forbidden`.)", 0) \
    ...
    **Example**
    ...
    before the main Datastore server process, giving the server a chance to shed load.
    ...
    Behavior may change between Datastore versions until production validation is complete.
    )", EXPERIMENTAL) \
