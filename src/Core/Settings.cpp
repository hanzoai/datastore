...
This setting controls block formation in format parsing. When the server parses row-based input formats (CSV, TSV, JSONEachRow, etc.) or Values format from any interface (HTTP, datastore-client with inline data, gRPC, PostgreSQL wire protocol), it uses this setting to determine when to emit a block.
...
1. Format parsing: When the server parses row-based input formats (CSV, TSV, JSONEachRow, etc.) from any interface (HTTP, datastore-client with inline data, gRPC, PostgreSQL wire protocol), it uses this setting to determine when to emit a block.
Note: When using datastore-client or datastore-local to read from a file, the client itself parses the data and this setting applies on the client side.
...
Only has an effect in Datastore Cloud. Number of tries to do distributed cache request if unsuccessful
)", 0) \
