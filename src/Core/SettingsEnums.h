...
{
    DECIMAL, // convert MySQL's decimal and number to Datastore Decimal when applicable
    DATETIME64, // convert MySQL's DATETIME and TIMESTAMP and Datastore DateTime64 if precision is > 0 or range is greater that for DateTime.
    DATE2DATE32, // convert MySQL's date type to Datastore Date32
    DATE2STRING, // convert MySQL's date type to Datastore String(This is usually used when your mysql date is less than 1925)
    GEOMETRY // convert MySQL's spatial types to the corresponding Datastore geometric types (LineString, Polygon, MultiLineString, MultiPolygon); the generic GEOMETRY type maps to the umbrella Geometry type
};
