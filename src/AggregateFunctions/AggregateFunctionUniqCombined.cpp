...
In the case cardinalities are larger than `UINT_MAX`, you should use [`uniqCombined64`](https://hanzo.ai/docs/sql-reference/aggregate-functions/reference/uniqcombined64) instead.
:::
...
Calculates the approximate number of different argument values.
It is the same as [`uniqCombined`](https://hanzo.ai/docs/sql-reference/aggregate-functions/reference/uniqcombined), but uses a 64-bit hash for all data types rather than just for the String data type.

This function provides the result deterministically (it does not depend on the query processing order).

:::note
Since it uses 64-bit hash for all types, the result does not suffer from very high error for cardinalities significantly larger than `UINT_MAX` like [`uniqCombined`](https://hanzo.ai/docs/sql-reference/aggregate-functions/reference/uniqcombined) does, which uses a 32-bit hash for non-String types.
:::

Compared to the [uniq](https://hanzo.ai/docs/sql-reference/aggregate-functions/reference/uniq) function, the uniqCombined64 function:
- Consumes several times less memory
- Calculates with several times higher accuracy
