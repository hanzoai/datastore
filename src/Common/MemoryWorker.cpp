...
CgroupLevelAvailability decideCgroupLevelAvailability(std::string_view max_token, uint64_t used, uint64_t host_memory_bytes)
{
    ...
    /// real cgroup imposes on a running Datastore.
        ...
        for (auto & level : cgroup_memory_levels)
        {
            try
            {
                ...
                if (!level.current_path.empty())
                {
                    ...
                    /// Datastore throw `MEMORY_LIMIT_EXCEEDED` even though most of that memory
        ...
        try
        {
            ...
            if (ratio > 0.0)
            {
                ...
                if (ceiling >= 0)
                {
                    ...
                    /// when Datastore should be shrinking its budget the most.
                    ...
                    if (available_opt)
                    {
                        ...
                        /// adjustment: that Datastore should *shrink* its budget when free memory
