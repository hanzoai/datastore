...
#define APPLY_FOR_BUILTIN_EVENTS(M) \
    ...
    M(UniqueKeyIndexCacheLookupMicroseconds, "Wall-clock time inside `UniqueKeyIndexCache::Lookup` + `UniqueKeyIndexCache::Insert` (Datastore-side `CacheBase` adapter for the RocksDB block cache).", ValueType::Microseconds) \
    ...
    M(FileProgressCallbackInvocations, "Number of times the per-query `FileProgressCallback` was invoked (counts each per-file `FileProgress` event delivered to the client over native TCP or via `datastore-local`).", ValueType::Number) \
    \
