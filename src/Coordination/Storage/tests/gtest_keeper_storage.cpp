...
TEST(KeeperStorage, NodeSerializationRoundTrip)
{
    ...
    EXPECT_EQ(tomb_path.str(), path_tomb);
...
TEST(KeeperStorage, BackgroundFlushAndMerge)
{
    ...
    {
        DB::Arena arena;
        ChildrenSet2 children = storage.listCommittedChildren(NodePath(parent).withCalculatedHash(), arena);
        size_t count = 0;
        for (const auto & child : children.set)
            if (child.action != NodeAction::Remove)
                ++count;
        return count;
    };
    ...
    for (int i = num_removed; i < num_k; ++i)
    {
        ...
        FullNode out;
        ref.readFull(out, buf);
        EXPECT_EQ(out.data, "K");
    }
...
TEST(KeeperStorage, SnapshotWriter)
{
    ...
    while (true)
    {
        ...
        if (snapshot.at_end)
            break;
        EXPECT_TRUE(snapshot.node.basic.action == NodeAction::Create);
