...
void SortedRunNodeStream::next()
{
    ...
    for (;;)
    {
        ...
        ref.readFull(node, path_buf);
        offset += node.basic.serialized_size;
    ...
    : memtable(std::move(memtable_))
{
    ...
    for (uint32_t block_idx = 0; block_idx < memtable->blocks.size(); ++block_idx)
    {
        ...
        for (uint32_t offset = block->entries_start; offset < block->size;)
        {
            ...
            NodeBasicInfo basic;
            NodePath path;
            ref.read(basic, &path, &path_buf, nullptr, nullptr);
...
void MemtableSortedNodeStream::next()
{
    ...
    ref.readFullWithPath(node, info.path);
}
...
void MemtableReversedNodeStream::next()
{
    ...
    while (idx == 0) // current block exhausted
    {
        ...
        for (uint32_t offset = block->entries_start; offset < block->size;)
        {
            ref.offset = offset;
            NodeBasicInfo basic;
            ref.read(basic, nullptr, nullptr, nullptr, nullptr);
...
void MergingNodeStream::next()
{
    ...
    while (true) // one path per iteration
    {
        ...
        /// current path so far.
        std::optional<NodeAction> combined_action = substream.stream->node.basic.action;
        while (!pq.empty() && pq.top().stream->node.path.compare(substream.stream->node.path) == 0)
        {
            ...
            combined_action = combineActions(combined_action, next_substream.stream->node.basic.action, /*strict=*/ true);
        ...
        if (combined_action.has_value())
        {
            ...
            node.basic.action = *combined_action;
...
void SnapshotWriterNodeStream::next()
{
    ...
    while (true)
    {
        ...
        if (memtable_stream)
        {
            ...
            if (memtable_stream->node.basic.action == NodeAction::Remove)
                continue;

            node = memtable_stream->node;
            node.basic.action = NodeAction::Create;
            return;
        }
        ...
        chassert(merging_stream->node.basic.action == NodeAction::Create);
