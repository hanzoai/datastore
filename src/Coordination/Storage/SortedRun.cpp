...
void SortedRun::listChildren(NodePath range_start, NodePath range_end, ChildrenSet2 & out, DB::Arena & out_arena, BlockCache * block_cache) const
{
    ...
    for (; file_it != files.end(); ++file_it)
    {
        ...
        if (file.blocks.front().min_path.compare(range_end) >= 0)
            break;
        file.listChildren(range_start, range_end, out, out_arena, block_cache);
    }
}
...
void SortedRunWriter::appendNode(FullNode & node)
{
    ...
    file->node_count_delta += nodeCountDelta(node.basic.action);
