...
struct SortedRun
{
    ...
    /// Scan the nodes with range_start < path < range_end (both bounds exclusive) and insertCombine
    /// the last component of each path into `out` (with strict actions, like everything in committed
    /// state), copying the names into `out_arena`. Used for listing children: the caller passes the
    /// range that selects exactly the direct children of some node.
    void listChildren(NodePath range_start, NodePath range_end, ChildrenSet2 & out, DB::Arena & out_arena, BlockCache * block_cache) const;
