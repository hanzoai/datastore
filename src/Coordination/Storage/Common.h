...
struct NodeRef
{
    ...
    ///
    /// this->action is ignored, the caller doesn't need to know NodeAction in order to read the node.
    ///
    ...
    /// Like readFull, but takes the path from the caller (who typically looked the node up by
    /// path, so already knows it) instead of allocating and deserializing it.
    void readFullWithPath(FullNode & out_node, const NodePath & path, NodePathHash path_hash = 0) const;
};
