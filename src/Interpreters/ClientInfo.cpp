...
/// Detect whether the client (datastore-client or datastore-local) is being invoked under a known
...
void ClientInfo::write(WriteBuffer & out, UInt64 server_protocol_revision, bool with_trailing_fields) const
{
    ...
    /// when a datastore-local query (LOCAL interface) is forwarded to remote shards.
...
void ClientInfo::setFromHTTPRequest(const Poco::Net::HTTPRequest & request)
{
    ...
    for (const auto & header : request)
    {
        ...
        if (key_lowercase.starts_with("x-datastore") || key_lowercase == "authentication" || key_lowercase == "authorization")
            continue;
