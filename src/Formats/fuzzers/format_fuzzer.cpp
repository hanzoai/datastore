...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t * data, size_t size)
{
    try
    {
        ...
        i=0; find ../../../../tests/queries -name '*.sql' |
            ...
            while read line; do
                i=$((i+1));
                datastore-local --query "SELECT name FROM system.formats ORDER BY rand() LIMIT 1" >> $i;
                echo "$line" >> $i;
                echo $RANDOM >> $i;
                echo $i;
            done
        */
        ...
        for format in $(datastore-client --query "SELECT name FROM system.formats WHERE is_output"); do
            echo $format;
            echo $format >> $format;
            ...
            datastore-client --query "SELECT * FROM hits LIMIT 10 FORMAT $format" >> $format || rm $format;
        done
        ...
        */
        ...
        /// https://github.com/Datastore/fuzz-corpus/tree/main/format_fuzzer
        ///
