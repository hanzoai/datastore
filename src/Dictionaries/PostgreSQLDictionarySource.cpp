...
void registerDictionarySourcePostgreSQL(DictionarySourceFactory & factory)
{
                                 ...
                                 [[maybe_unused]] bool created_from_ddl) -> DictionarySourcePtr
    {
        ...
        throw Exception(ErrorCodes::SUPPORT_IS_DISABLED,
            "Dictionary source of type `postgresql` is disabled because Datastore was built without postgresql support.");
#endif
    };
...
#if !USE_LIBPQXX
            " Currently unavailable, because this Datastore build does not include PostgreSQL support."
#endif
        ,
        .syntax = "SOURCE(POSTGRESQL(host 'host' port 5432 user 'user' password '' db 'db' table 'table'))",
        .related = {"mysql", "datastore"}});
}
