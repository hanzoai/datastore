...
namespace DB
{
/** Allows loading dictionaries from local or remote Hanzo Datastore instance
  *    @todo use ConnectionPoolWithFailover
  *    @todo invent a way to keep track of source modifications
  */
class HanzoDictionarySource final : public IDictionarySource
{
public:
    ...
    HanzoDictionarySource(const HanzoDictionarySource & other);
    HanzoDictionarySource & operator=(const HanzoDictionarySource &) = delete;
    ...
    DictionarySourcePtr clone() const override { return std::make_shared<HanzoDictionarySource>(*this); }
...
private:
    ...
    LoggerPtr log = getLogger("HanzoDictionarySource");
