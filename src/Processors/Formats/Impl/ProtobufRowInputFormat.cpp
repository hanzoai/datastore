...
:::note Example files
The files used in this example are available in the [examples repository](https://github.com/Datastore/formats/ProtoBuf)
:::

In this example we will read some data from a file `protobuf_message.bin` into a Datastore table. We'll then write it
back out to a file called `protobuf_message_from_datastore.bin` using the `Protobuf` format.
...
```sql
SELECT * FROM test.protobuf_messages INTO OUTFILE 'protobuf_message_from_datastore.bin' FORMAT Protobuf SETTINGS format_schema = 'schemafile:MessageType'
```

With your Protobuf schema, you can now deserialize the data which was written out from Datastore to file `protobuf_message_from_datastore.bin`.
