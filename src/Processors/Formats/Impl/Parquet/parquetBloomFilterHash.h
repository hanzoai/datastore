...
/*
 * Try to hash a Datastore field, nullopt in case it can't be done
 * */
std::optional<uint64_t> parquetTryHashField(const Field & field, const parquet::ColumnDescriptor * parquet_column_descriptor);


/*
 * Try to hash elements in a Datastore column; Will return std::nullopt in case one of them can't be hashed
 * */
