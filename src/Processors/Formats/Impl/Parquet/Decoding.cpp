...
struct BitPackedRLEDecoder : public PageDecoder
{
    ...
    void decode(size_t num_values, IColumn & col, const UInt8 * filter, size_t /*asdqwe_delete_dis*/, size_t output_size_upper_bound) override
    {
...
void Dictionary::decode(parq::Encoding::type encoding, const PageDecoderInfo & info, size_t num_values, std::span<const char> data_, const IDataType & raw_decoded_type)
{
    ...
    if (decode_generic)
    {
        ...
        decoder->decode(num_values, *c, nullptr, 0, num_values);
        col = std::move(c);
