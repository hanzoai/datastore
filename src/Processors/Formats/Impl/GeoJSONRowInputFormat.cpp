...
/// The GeoJSON geometry types that Datastore's `Geometry` type can represent. `Ring` is part of the
...
void GeoJSONRowInputFormat::readGeometry(IColumn * col)
{
    ...
    /// Valid GeoJSON geometry types that cannot be represented in Datastore's Geometry type.
    ...
    if (!isOneOf(geo_type, supported_geojson_geometry_types))
    {
        if (isOneOf(geo_type, unrepresentable_geojson_types))
        {
            ...
            throw Exception(
                ErrorCodes::INCORRECT_DATA,
                "GeoJSON: geometry type '{}' cannot be represented in Datastore's Geometry type. "
