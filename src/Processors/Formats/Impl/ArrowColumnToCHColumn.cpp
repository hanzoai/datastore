...
{
    // Check for our Datastore/Arrow extension name
...
/// Arrow stores Parquet::DATETIME in Int64, while Datastore stores DateTime in UInt32. Therefore, it should be checked before saving
...
static ColumnPtr readOffsetsFromArrowListColumn(const std::shared_ptr<arrow::ChunkedArray> & arrow_column, const String & column_name)
{
    ...
    for (int chunk_i = 0, num_chunks = arrow_column->num_chunks(); chunk_i < num_chunks; ++chunk_i)
    {
         ...
         * https://github.com/ClickHouse/Datastore/pull/43297 and https://github.com/ClickHouse/Datastore/pull/54370
         * */
...
/*
 * Arrow Dictionary and Datastore LowCardinality types are a bit different.
 ...
 * In Datastore LowCardinality, it's different. The dictionary contains null (if dictionary type is Nullable)
 ...
 * So, we should remap indexes while converting Arrow Dictionary to Datastore LowCardinality
 * */
template <typename NumericType, typename VectorType = ColumnVector<NumericType>>
...
static std::shared_ptr<arrow::ChunkedArray> getNestedArrowColumn(const std::shared_ptr<arrow::ChunkedArray> & arrow_column, const String & column_name)
{
    ...
    for (int chunk_i = 0, num_chunks = arrow_column->num_chunks(); chunk_i < num_chunks; ++chunk_i)
    {
         ...
         * https://github.com/ClickHouse/Datastore/pull/43297
         * */
    ...
    const std::optional<std::unordered_map<String, String>> & clickhouse_columns_to_parquet)
{
    switch (arrow_column->type()->id())
    {
        ...
        // Datastore writes DateTime as arrow UINT32,
...
#    undef DISPATCH
        ...
        {
            /// Preserve interval semantics on round-trip from Datastore -> Arrow -> Datastore.
...
static std::shared_ptr<arrow::ChunkedArray> createArrowColumn(const std::shared_ptr<arrow::Field> & field, const String & format_name)
{
    ...
    // without destroying the logical metadata required by Datastore for type inference
    ...
    const std::optional<std::unordered_map<String, String>> & clickhouse_columns_to_parquet)
{
    ...
    {
        /// Create empty arrow column by it's type and convert it to Datastore column.
