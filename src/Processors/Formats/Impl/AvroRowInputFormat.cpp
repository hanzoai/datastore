...
static AvroDeserializer::DeserializeFn createDecimalDeserializeFn(const avro::NodePtr & root_node, const DataTypePtr & target_type, bool is_fixed)
{
    ...
    if (decimal_type.getScale() != static_cast<UInt32>(logical_type.scale()) || decimal_type.getPrecision() != static_cast<UInt32>(logical_type.precision()))
        throw Exception(
            ...
            "Cannot insert Avro decimal with scale {} and precision {} to Datastore type {} with scale {} and precision {}",
            logical_type.scale(),
...
DataTypePtr AvroSchemaReader::avroNodeToDataTypeImpl(const avro::NodePtr & node, std::unordered_set<std::string> & seen_names, bool allow_nullable_tuple_type)
{
    switch (node->type())
    {
        ...
        case avro::Type::AVRO_ENUM:
        {
            ...
            throw Exception(ErrorCodes::ILLEGAL_COLUMN, "Datastore supports only 8 and 16-bit Enum.");
        }
...
Data types of Datastore table columns can differ from the corresponding fields of the Avro data inserted. When inserting data, Datastore interprets data types according to the table above and then [casts](/reference/functions/regular-functions/type-conversion-functions#CAST) the data to the corresponding column type.
...
Using the Datastore [`DESCRIBE`](/reference/statements/describe-table) function, you can quickly view the inferred format of an Avro file like the following example. 
...
To monitor ingestion progress and debug errors with the Kafka consumer, you can query the [`system.kafka_consumers` system table](/reference/system-tables/kafka_consumers). If your deployment has multiple replicas (e.g., Datastore Cloud), you must use the [`clusterAllReplicas`](/reference/functions/table-functions/cluster) table function.
...
If you run into schema resolution issues, you can use [kafkacat](https://github.com/edenhill/kafkacat) with [datastore-local](/concepts/features/tools-and-utilities/datastore-local) to troubleshoot:
