...
Datastore supports read and write operations for these formats.
`Arrow` is Apache Arrow's "file mode" format, designed for in-memory random access.

## Data types matching {#data-types-matching}

The table below shows the supported data types and how they correspond to Datastore [data types](/reference/data-types/index) in `INSERT` and `SELECT` queries.
...
The data types of Datastore table columns do not have to match the corresponding Arrow data fields. When inserting data, Datastore interprets data types according to the table above and then [casts](/reference/functions/regular-functions/type-conversion-functions#CAST) the data to the data type set for the Datastore table column.
...
[`datastore-local`](/concepts/features/tools-and-utilities/datastore-local) using the
...
To load an Arrow file into a Datastore table, pipe it into `datastore-client`
with `FORMAT Arrow`:

```bash
cat forex_eurusd.arrow | datastore-client --query="INSERT INTO some_table FORMAT Arrow"
```
