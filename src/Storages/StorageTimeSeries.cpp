...
The outer `time_series` column can be listed explicitly in a `CREATE TABLE` statement to override its default `Array(Tuple(DateTime64(3), Float64))` type. Datastore extracts the timestamp and scalar types from the tuple and propagates them to the inner samples table:
...
The `id` column type must be one of `UUID`, `UInt64`, `UInt128`, or `FixedString(16)`. If no `DEFAULT` expression is given, Datastore will choose it automatically based on the `id` type. The `id` types declared in the samples and tags inner tables must match.
