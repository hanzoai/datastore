...
To create a distributed table engine in Datastore Cloud, you can use the [`remote` and `remoteSecure`](../../../sql-reference/table-functions/remote) table functions.
The `Distributed(...)` syntax cannot be used in Datastore Cloud.
:::
...
| `skip_unavailable_shards`                  | If true, Datastore silently skips unavailable shards. Shard is marked as unavailable when: 1) The shard cannot be reached due to a connection failure. 2) Shard is unresolvable through DNS. 3) Table does not exist on the shard.   | `false`       |
...
| `bind_host`   | The source address to use when connecting to the remote server from this node. IPv4 address only supported. Intended for advanced deployment use cases where setting the source IP address used by Datastore distributed queries is needed.                                                                                             | -            |
