...
Datastore system (for example, moving from on-prem systems to Datastore
Cloud).  Because Datastore datasets are often very large, and network
...
CREATE TABLE big_table (name String, value UInt32)
    ENGINE = S3('https://datastore-public-datasets.s3.amazonaws.com/my-bucket/my_folder/file-{000..999}.csv', 'CSV');
```
...
- 'https://datastore-public-datasets.s3.amazonaws.com/my-bucket/some_folder/some_file_1.csv'
- 'https://datastore-public-datasets.s3.amazonaws.com/my-bucket/some_folder/some_file_2.csv'
- 'https://datastore-public-datasets.s3.amazonaws.com/my-bucket/some_folder/some_file_3.csv'
- 'https://datastore-public-datasets.s3.amazonaws.com/my-bucket/another_folder/some_file_1.csv'
- 'https://datastore-public-datasets.s3.amazonaws.com/my-bucket/another_folder/some_file_2.csv'
- 'https://datastore-public-datasets.s3.amazonaws.com/my-bucket/another_folder/some_file_3.csv'
...
CREATE TABLE table_with_range (name String, value UInt32)
    ENGINE = S3('https://datastore-public-datasets.s3.amazonaws.com/my-bucket/{some,another}_folder/some_file_{1..3}', 'CSV');
```
...
CREATE TABLE table_with_question_mark (name String, value UInt32)
    ENGINE = S3('https://datastore-public-datasets.s3.amazonaws.com/my-bucket/{some,another}_folder/some_file_?', 'CSV');
```
...
CREATE TABLE table_with_asterisk (name String, value UInt32)
    ENGINE = S3('https://datastore-public-datasets.s3.amazonaws.com/my-bucket/{some,another}_folder/*', 'CSV');
```
...
<s3>
    <endpoint-name>
        <endpoint>https://datastore-public-datasets.s3.amazonaws.com/my-test-bucket-768/</endpoint>
...
:::note
Datastore supports three archive formats:
ZIP
TAR
7Z
While ZIP and TAR archives can be accessed from any supported storage location, 7Z archives can only be read from the local filesystem where Datastore is installed.
:::

## Accessing public buckets {#accessing-public-buckets}

Datastore tries to fetch credentials from many different types of sources.
...
In Datastore Cloud, you can use role-based access to authenticate with S3 instead of using access keys. See [Secure S3](/cloud/data-sources/secure-s3) for configuration steps.
...
ENGINE = S3('https://my-bucket.s3.amazonaws.com/data/*.csv', extra_credentials(role_arn = 'arn:aws:iam::111111111111:role/DatastoreAccessRole-001'), 'CSV')
```
...
```sql
CREATE TABLE my_s3_table(name String, value UInt32)
ENGINE = S3('https://my-bucket.s3.amazonaws.com/data/*.csv', extra_credentials(role_arn = 'arn:aws:iam::111111111111:role/DatastoreAccessRole-001', external_id = 'my-external-id'), 'CSV')
```

## See also {#see-also}

- [s3 table function](../../../sql-reference/table-functions/s3.md)
- [Integrating S3 with Datastore](/integrations/s3)
)DOCS_MD";
    }
    else
    {
...
- If the underlying Paimon table is dropped and recreated at the same object-storage path (e.g. via Flink or Spark), you must `DROP` and re-`CREATE` the corresponding Datastore table. Datastore detects the recreation by comparing the schema-0 creation timestamp and raises an error; the stale Datastore table cannot be used until it is recreated.
...
```sql
CREATE TABLE hudi_table ENGINE=Hudi('http://mars-doc-test.s3.amazonaws.com/datastore-bucket-3/test_table/', 'ABC123', 'Abc+123')
```
