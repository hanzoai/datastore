...
#define LIST_OF_DISTRIBUTED_SETTINGS(DECLARE, DECLARE_WITH_ALIAS) \
    ...
    DECLARE(Bool, skip_unavailable_shards, false, "If true, Datastore silently skips unavailable shards. Shard is marked as unavailable when: 1) The shard cannot be reached due to a connection failure. 2) Shard is unresolvable through DNS. 3) Table does not exist on the shard.", 0) \
    /** Inserts settings. */ \
