...
public:
    ...
    /// However in some special cases Datastore can make a decision without asking keeper.
    enum class BlobsRemovalPolicyForTemporaryParts : uint8_t
    {
        ...
        ASK_KEEPER,
        /// is set when Datastore is sure that the blobs in the part are belong only to it, other replicas have not seen them yet
        REMOVE_BLOBS,
        /// is set when Datastore is sure that the blobs belong to other replica and current replica has not locked them on s3 yet
        PRESERVE_BLOBS,
