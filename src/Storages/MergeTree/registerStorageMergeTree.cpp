...
See details in documentation: https://hanzo.ai/docs/engines/table-engines/mergetree-family/mergetree/. Other engines of the family support different syntax, see details in the corresponding documentation topics.

If you use the Replicated version of engines, see https://hanzo.ai/docs/engines/table-engines/mergetree-family/replication/.
)";
...
If no primary key is defined (i.e. `PRIMARY KEY` was not specified), Datastore uses the sorting key as primary key.
...
Currently, only `basic` statistics (and the deprecated `minmax` statistics) support part pruning. When such statistics are defined on a column, Datastore tracks the minimum and maximum values for that column in each part.
...
By default, Datastore will keep the last row for a key even if that row is a delete row. This is so that any future rows with lower versions can
...
1. Set the table settings `enable_replacing_merge_with_cleanup_for_min_age_to_force_merge`, `min_age_to_force_merge_on_partition_only` and `min_age_to_force_merge_seconds`. If all parts in a partition are older than `min_age_to_force_merge_seconds`, Datastore will merge them
...
```
    ...
    factory.registerStorage("AggregatingMergeTree", create, features, Documentation{
        .description = R"DOCS_MD(
The engine inherits from [MergeTree](/engines/table-engines/mergetree-family/mergetree), altering the logic for data parts merging. Datastore replaces all rows with the same primary key (or more accurately, with the same [sorting key](../../../engines/table-engines/mergetree-family/mergetree.md)) with a single row (within a single data part) that stores a combination of states of aggregate functions.
...
- Blog: [Using Aggregate Combinators in Datastore](https://hanzo.ai/blog/aggregate-functions-combinators-in-datastore-for-arrays-maps-and-states)
    ...
    factory.registerStorage("SummingMergeTree", create, features, Documentation{
        .description = R"DOCS_MD(
The engine inherits from [MergeTree](/engines/table-engines/mergetree-family/mergetree). The difference is that when merging data parts for `SummingMergeTree` tables Datastore replaces all the rows with the same primary key (or more accurately, with the same [sorting key](../../../engines/table-engines/mergetree-family/mergetree.md)) with one row which contains summed values for the columns with the numeric data type. If the sorting key is composed in a way that a single key value corresponds to large number of rows, this significantly reduces storage volume and speeds up data selection.
...
This is a very inefficient way to select data. Don't use it for large tables.
    ...
    factory.registerStorage("ReplicatedMergeTree", create, features, Documentation{
        .description = R"DOCS_MD(
:::note
In Datastore Cloud replication is managed for you. Please create your tables without adding arguments.  For example, in the text below you would replace:

```sql
ENGINE = ReplicatedMergeTree(
    '/datastore/tables/{shard}/table_name',
    '{replica}'
)
```
...
Datastore uses [Datastore Keeper](/guides/sre/keeper/index.md) for storing replicas meta information. It is possible to use ZooKeeper version 3.4.5 or newer, but Datastore Keeper is recommended.
...
:::note
Don't neglect the security setting. Datastore supports the `digest` [ACL scheme](https://zookeeper.apache.org/doc/current/zookeeperProgrammers.html#sc_ZooKeeperAccessControl) of the ZooKeeper security subsystem.
:::

Example of setting the addresses of the Datastore Keeper cluster:
...
Datastore also supports storing replicas meta information in an auxiliary ZooKeeper cluster. Do this by providing the ZooKeeper cluster name and path as engine arguments.
...
ZooKeeper is not used in `SELECT` queries because replication does not affect the performance of `SELECT` and queries run just as fast as they do for non-replicated tables. When querying distributed replicated tables, Datastore behavior is controlled by the settings [max_replica_delay_for_distributed_queries](/operations/settings/settings.md/#max_replica_delay_for_distributed_queries) and [fallback_to_stale_replicas_for_distributed_queries](/operations/settings/settings.md/#fallback_to_stale_replicas_for_distributed_queries).

For each `INSERT` query, approximately ten entries are added to ZooKeeper through several transactions. (To be more precise, this is for each inserted block of data; an INSERT query contains one block or one block per `max_insert_block_size = 1048576` rows.) This leads to slightly longer latencies for `INSERT` compared to non-replicated tables. But if you follow the recommendations to insert data in batches of no more than one `INSERT` per second, it does not create any problems. The entire Datastore cluster used for coordinating one ZooKeeper cluster has a total of several hundred `INSERTs` per second. The throughput on data inserts (the number of rows per second) is just as high as for non-replicated data.
...
:::note
In Datastore Cloud, replication is handled automatically.
...
| Parameter       | Description                                                                  |
|-----------------|------------------------------------------------------------------------------|
| `zoo_path`      | The path to the table in Datastore Keeper.                                  |
| `replica_name`  | The replica name in Datastore Keeper.                                       |
...
ENGINE = ReplicatedReplacingMergeTree('/datastore/tables/{layer}-{shard}/table_name', '{replica}', ver)
PARTITION BY toYYYYMM(EventDate)
...
(
    ...
    CounterID UInt32,
    UserID UInt32
) ENGINE = ReplicatedMergeTree('/datastore/tables/{shard}/table_name', '{replica}', EventDate, intHash32(UserID), (CounterID, EventDate, intHash32(UserID), EventTime), 8192);
```
...
The path to the table in Datastore Keeper should be unique for each replicated table. Tables on different shards should have different paths.
In this case, the path consists of the following parts:

`/datastore/tables/` is the common prefix. We recommend using exactly this one.

`{shard}` will be expanded to the shard identifier.

`table_name` is the name of the node for the table in Datastore Keeper. It is a good idea to make it the same as the table name. It is defined explicitly, because in contrast to the table name, it does not change after a RENAME query.
*HINT*: you could add a database name in front of `table_name` as well. E.g. `db_name.table_name`

The two built-in substitutions `{database}` and `{table}` can be used, they expand into the table name and the database name respectively (unless these macros are defined in the `macros` section). So the zookeeper path can be specified as `'/datastore/tables/{shard}/{database}/{table}'`.
Be careful with table renames when using these built-in substitutions. The path in Datastore Keeper cannot be changed, and when the table is renamed, the macros will expand into a different path, the table will refer to a path that does not exist in Datastore Keeper, and will go into read-only mode.
...
```xml
<default_replica_path>/datastore/tables/{shard}/{database}/{table}</default_replica_path>
...
CREATE TABLE table_name (
    x UInt32
) ENGINE = ReplicatedMergeTree('/datastore/tables/{shard}/{database}/table_name', '{replica}')
ORDER BY x;
```
...
If Datastore Keeper is unavailable when a server starts, replicated tables switch to read-only mode. The system periodically attempts to connect to Datastore Keeper.

If Datastore Keeper is unavailable during an `INSERT`, or an error occurs when interacting with Datastore Keeper, an exception is thrown.

After connecting to Datastore Keeper, the system checks whether the set of data in the local file system matches the expected set of data (Datastore Keeper stores this information). If there are minor inconsistencies, the system resolves them by syncing data with the replicas.

If the system detects broken data parts (with the wrong size of files) or unrecognized parts (parts written to the file system but not recorded in Datastore Keeper), it moves them to the `detached` subdirectory (they are not deleted). Any missing parts are copied from the replicas.

Note that Datastore does not perform any destructive actions such as automatically deleting a large amount of data.

When the server starts (or establishes a new session with Datastore Keeper), it only checks the quantity and sizes of all files. If the file sizes match but bytes have been changed somewhere in the middle, this is not detected immediately, but only when attempting to read the data for a `SELECT` query. The query throws an exception about a non-matching checksum or size of a compressed block. In this case, data parts are added to the verification queue and copied from the replicas if necessary.
...
To start recovery, create the node `/path_to_table/replica_name/flags/force_restore_data` in Datastore Keeper with any content, or run the command to restore all replicated tables:

```bash
sudo -u datastore touch /var/lib/datastore/flags/force_restore_data
```
...
1.  Install Datastore on the server. Define substitutions correctly in the config file that contains the shard identifier and replicas, if you use them.
2.  If you had unreplicated tables that must be manually duplicated on the servers, copy their data from a replica (in the directory `/var/lib/datastore/data/db_name/table_name/`).
3.  Copy table definitions located in `/var/lib/datastore/metadata/` from a replica. If a shard or replica identifier is defined explicitly in the table definitions, correct it so that it corresponds to this replica. (Alternatively, start the server and make all the `ATTACH TABLE` queries that should have been in the .sql files in `/var/lib/datastore/metadata/`.)
4.  To start recovery, create the Datastore Keeper node `/path_to_table/replica_name/flags/force_restore_data` with any content, or run the command to restore all replicated tables: `sudo -u datastore touch /var/lib/datastore/flags/force_restore_data`

Then start the server (restart, if it is already running). Data will be downloaded from replicas.

An alternative recovery option is to delete information about the lost replica from Datastore Keeper (`/path_to_table/replica_name`), then create the replica again as described in "[Creating replicated tables](#creating-replicated-tables)".
...
Move the data from the old table to the `detached` subdirectory inside the directory with the new table data (`/var/lib/datastore/data/db_name/table_name/`).
...
- Delete the corresponding `.sql` file in the metadata directory (`/var/lib/datastore/metadata/`).
- Delete the corresponding path in Datastore Keeper (`/path_to_table/replica_name`).

After this, you can launch the server, create a `MergeTree` table, move the data to its directory, and then restart the server.

## Recovery when metadata in the Datastore Keeper cluster is lost or damaged {#recovery-when-metadata-in-the-zookeeper-cluster-is-lost-or-damaged}

If the data in Datastore Keeper was lost or damaged, you can save data by moving it to an unreplicated table as described above.
