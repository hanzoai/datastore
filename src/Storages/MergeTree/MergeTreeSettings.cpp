...
when writing into replicated tables, Datastore writes deduplication hash sums into
Datastore Keeper. Hash sums are stored only for the most recent
...
legacy async-insert hashes are kept in the `async_blocks` directory in Datastore Keeper, which is
...
are kept in the `async_blocks` directory in Datastore Keeper (written by older replicas during a
...
`deduplication_hashes` directory in Datastore Keeper so inserts can detect duplicates without a
Keeper round-trip.
)", 0) \
