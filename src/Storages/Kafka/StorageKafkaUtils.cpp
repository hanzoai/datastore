...
#undef CHECK_KAFKA_STORAGE_ARGUMENT
        ...
        if (!args.getLocalContext()->getSettingsRef()[Setting::kafka_disable_num_consumers_limit] && num_consumers > max_consumers)
        {
            throw Exception(
                ...
                "See also https://hanzo.ai/docs/integrations/kafka/kafka-table-engine#tuning-performance",
                max_consumers);
        }
        if (num_consumers < 1)
        {
        ...
        if (args.columns.getAll() != supported_columns)
        {
            throw Exception(
                ...
                "See https://hanzo.ai/docs/engines/table-engines/integrations/kafka/#configuration");
        }
