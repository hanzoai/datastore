...
- Data export from Datastore to file.
- Convert data from one format to another.
- Updating data in Datastore via editing a file on a disk.

:::note
This engine is not currently available in Datastore Cloud, please [use the S3 table function instead](/sql-reference/table-functions/s3.md).
:::
...
Datastore does not allow specifying filesystem path for `File`. It will use folder defined by [path](../../../operations/server-configuration-parameters/settings.md) setting in server configuration.
...
:::note
Be careful with this functionality, because Datastore does not keep track of external changes to such files. The result of simultaneous writes via Datastore and outside of Datastore is undefined.
:::
...
By default Datastore will create folder `/var/lib/datastore/data/default/file_engine_table`.

**2.** Manually create `/var/lib/datastore/data/default/file_engine_table/data.TabSeparated` containing:
...
## Usage in Datastore-local {#usage-in-datastore-local}

In [datastore-local](../../../operations/utilities/datastore-local.md) File engine accepts file path in addition to `Format`. Default input/output streams can be specified using numeric or human-readable names like `0` or `stdin`, `1` or `stdout`. It is possible to read and write compressed files based on an additional engine parameter or file extension (`gz`, `br` or `xz`).

**Example:**

```bash
$ echo -e "1,2\n3,4" | datastore-local -q "CREATE TABLE table (a Int64, b Int64) ENGINE = File(CSV, stdin); SELECT a, b FROM table; DROP TABLE table"
```
...
- [storage_file_read_method](/operations/settings/settings#engine_file_empty_if_not_exists) - method of reading data from storage file, one of: `read`, `pread`, `mmap`. The mmap method does not apply to datastore-server (it's intended for datastore-local). Default value: `pread` for datastore-server, `mmap` for datastore-local.
)DOCS_MD",
