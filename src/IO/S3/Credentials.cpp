    ...
    , expiration_window_seconds(expiration_window_seconds_)
{
    ...
    /// Use Datastore's retry strategy so the STS web-identity client stays bounded and respects query
...
void SSOCredentialsProvider::Reload()
{
    ...
    /// Use Datastore's retry strategy so the SSO client stays bounded and respects query cancellation /
...
std::shared_ptr<Aws::Auth::AWSCredentialsProvider> AwsAuthSTSAssumeRoleCredentialsProvider::create(
    ...
    const std::string & sts_endpoint_override)
{
    /// The STS AssumeRole client uses the AWS SDK's own retry loop, so give it Datastore's bounded,
