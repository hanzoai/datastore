...
void registerDiskObjectStorage(DiskFactory & factory, bool global_skip_access_check)
{
    ...
    factory.registerDiskType("s3_plain", creator, Documentation{
        .description = "An S3-backed disk that stores objects under their plain paths without separate Datastore metadata. Mostly read-only, used for backups. Retained for compatibility.",
    ...
    factory.registerDiskType("s3_with_keeper", creator, Documentation{
        .description = "An S3-backed disk that keeps its metadata in Datastore Keeper instead of on a local metadata disk. "
            "The `keeper` metadata storage is available only in Datastore Cloud builds; in other builds, creating a disk of this type "
    ...
    factory.registerDiskType("local_blob_storage", creator, Documentation{
        ...
        .syntax = "disk(type = local_blob_storage, path = '/var/lib/datastore/disk_local_blob/')",
