...
The same as [`path`](#path), but includes the query string and fragment of the URL.
    ...
    FunctionDocumentation::Examples examples_pathFull = {
    {
        "Usage example",
        R"(
SELECT pathFull('https://datastore.com/docs/sql-reference/functions/url-functions/?query=value#section');
        )",
        R"(
┌─pathFull('https://datastore.com⋯unctions/?query=value#section')─┐
