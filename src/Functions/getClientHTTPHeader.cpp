...
Certain HTTP headers (e.g., `Authorization`, `Authentication` and `X-Datastore-*`) are restricted.
    ...
    FunctionDocumentation::Examples examples = {
        {
            "Usage example",
            R"(
-- Over a non-HTTP interface (such as `datastore-client` or `datastore-local`) there are
