...
└─────┴──────────────┴─────────────────┘
    ...
    /// In Datastore it is a full alias of `length`, so it inherits `length`'s behavior for non-array arguments too
    /// (e.g. it returns the byte length of strings); this is a Datastore extension beyond the standard.
