...
enum Problem
{
    ...
    /// https://github.com/ClickHouse/Datastore/blob/eca34edf9ceef5d36a57dbef160a3653191d6bc7/src/Functions/tests/gtest_functions_stress.cpp#L413
...
std::pair<String, String> problemInfo(Problem p)
{
    switch (p)
    {
        ...
        case P_BROKEN_NULLABLE_INPUT: return {"broken_nullable_input",
            "function failed on multiple rows, but succeeded on each of those rows separately; some arguments are nullable, so this is probably a known problem: https://github.com/ClickHouse/Datastore/issues/93660"};
