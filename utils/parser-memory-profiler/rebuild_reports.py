...
def get_paths(output_dir):
    ...
    return {
        ...
        'profiler': os.path.join(clickhouse_root, "build/src/Examples/datastore-examples"),
...
Examples:
    ...
    parser.add_argument('--profiler', help='Path to datastore-examples binary')
    ...
    if not os.path.exists(paths['profiler']):
        print(f"Error: datastore-examples not found at {paths['profiler']}")
        print("Build it with: cd build && ninja datastore-examples")
        sys.exit(1)
