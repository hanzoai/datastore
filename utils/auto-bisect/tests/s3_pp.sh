#!/bin/bash
set -e
#./bisect.sh --test tests/test_rmv_definer.sh --good af9a4c6a --bad b7c20bd6  --path /home/nik/work/datastore-private1/ --env nothing

(
  cd $GIT_WORK_TREE/tests/integration || exit 1
  echo "$PWD"

  export DATASTORE_TESTS_SERVER_BIN_PATH=$CH_PATH
  # ODBC bridge isn't used
  export DATASTORE_TESTS_ODBC_BRIDGE_BIN_PATH=$CH_PATH
  export DATASTORE_TESTS_LIBRARY_BRIDGE_BIN_PATH=$CH_PATH
