...
# This utility provides similar interface to datastore-client.
    ...
    def __connect(self):
        ...
        while True:
            try:
                ...
                self.verbatim_print(
                    "Couldn't connect to Datastore server in {connection_time} seconds.".format(
        ...
        if self.verbatim:
            ...
            self.verbatim_print(
                "Connected to Datastore server version {version} via gRPC protocol in {connection_time}.".format(
...
def main(args):
    parser = argparse.ArgumentParser(
        description="Datastore client accessing server through gRPC protocol.",
        add_help=False,
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--port",
        help="The port to connect to. This port should be enabled on the Datastore server (see grpc_port in the config).",
        default=9100,
    )
    parser.add_argument(
        "--user",
