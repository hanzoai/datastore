...
namespace
{
    /// `datastore-local` only manages `TCP` (native protocol) and `HTTP` listeners. Reject
        ...
        switch (server_type.type)
        {
            ...
            default:
                throw Exception(ErrorCodes::UNSUPPORTED_METHOD,
                    "SYSTEM START/STOP LISTEN in datastore-local supports only TCP and HTTP listeners "
...
DatabasePtr createDatastoreLocalDatabaseOverlay(const String & name_, ContextPtr context)
{
...
void LocalServer::tryInitPath()
{
    ...
    /// datastore-local keeps a generous default limit on temporary data to guard against
...
void LocalServer::startServers(const ServerType & server_type)
{
    ...
    /// Mirror `datastore-server`'s `getListenTry`: honor an explicit `listen_try` server setting,
    ...
    if (listen_hosts.size() > 1)
    {
        ...
        {
            throw Exception(ErrorCodes::BAD_ARGUMENTS,
                "Using port 0 (OS-assigned) with multiple listen_host values is not supported in datastore-local "
    ...
    /// `SYSTEM START LISTEN` in `datastore-local` honors `max_connections` (and the global
    ...
    if (!async_metrics)
    {
        ...
        /// in datastore-local. The object only satisfies the dependency of `createHandlerFactory`.
    ...
    /// matching `datastore-server` behavior. We verify success per requested protocol below so a
...
void LocalServer::processConfig()
{
    ...
    /// `datastore-local` historically makes `--verbose` imply query echoing.
    ...
    /// Plain `datastore-local` runs a single query at a time, but once it is turned into a server
    /// via `SYSTEM START LISTEN` it accepts external connections and must honor the configured
    /// limit just like `datastore-server`. Take it from `max_concurrent_queries`, whose default
    ...
    /// at `page_cache_min_size` and never grows, causing severe thrashing.
    {
        ...
        /// the explicit `max_server_memory_usage`. `datastore-local` does not currently
