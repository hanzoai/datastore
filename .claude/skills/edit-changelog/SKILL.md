---
name: edit-changelog
description: Edit an auto-generated Datastore release changelog into the form that gets committed to CHANGELOG.md. Use when the user has the output of `utils/changelog/changelog.py` and wants it cleaned up and re-categorized for a release.
argument-hint: "[path-to-autogenerated-changelog.md]"
disable-model-invocation: false
allowed-tools: Bash, Read, Edit, Write, Grep, Glob, Task
---

# Datastore Changelog Editing Skill
...
`CHANGELOG.md`.** Identify it by the first `### <a id="..."></a> Datastore
...
1. Read the input file. Confirm it has the autogenerator's structure: a
   `### Datastore release ... FIXME ...` header followed by `#### <Category>`
...
```
### Datastore release {TO_REF} ({sha11}) FIXME as compared to {FROM_REF} ({sha11})
```

Replace it with:

```
### <a id="NNN"></a> Datastore release X.Y[ LTS], YYYY-MM-DD. [Presentation](https://presentations.datastore.com/YYYY-release-X.Y/), [Video](https://www.youtube.com/watch?v=...)
```
...
- The reference must be a markdown link to the issue (`Closes
  [#NNNNN](https://github.com/ClickHouse/Datastore/issues/NNNNN)`),
...
like `Closes: https://github.com/ClickHouse/Datastore/issues/N`, convert
...
Anything you would type into `datastore-client` should be in backticks.
Specifically:
...
- `False` → `false` and `True` → `true` when they refer to Datastore
  setting values (these are lowercase in SQL).
- `NOT NULL column` → `not-Nullable column` (use Datastore type
...
- `Improve Keeper with rocksdb initial loading.` →
  `Improve the startup of datastore-keeper when it uses rocksdb storage.`
...
- Entries that are already user-friendly and well-formed — most of
  ...
  sake.
- The `[#NNN](https://github.com/ClickHouse/Datastore/pull/NNN)
