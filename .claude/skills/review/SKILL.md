...
**Documentation:**
- Structured Datastore surfaces are documented from source registrations: SQL functions and aggregate functions (`FunctionDocumentation`), settings (`DECLARE` doc strings), table functions, table engines, formats, system tables, and similar components. Do not ask for a separate `docs/` page when this source-level documentation is present and adequate.
...
- **Native protocol change:** when the diff touches the native TCP protocol — `src/Core/Protocol.h`, `src/Core/ProtocolDefines.h`, `src/Core/Protocol.cpp`, packet handling in `src/Server/TCPHandler.*` or `src/Client/Connection.*`, the `DBMS_TCP_PROTOCOL_VERSION` / `DBMS_MIN_REVISION_*` constants, packet types, handshake/version negotiation, or the wire layout of any non-`Block` message — verify that the native protocol specification at `docs/en/interfaces/specs/NativeProtocol.md` is updated in the same PR to reflect the new feature, bug fix, or behavior change. A protocol change that lands without a corresponding spec update is a finding: the spec is the canonical reference that third-party native clients (e.g. `ch-go`, `datastore-go`) are built against, and letting it drift forces re-deriving the protocol from C++ source. Flag the missing or stale spec section and name the packet/version/field that the diff changed.
...
DATASTORE-SPECIFIC RULES (SUPPORTING CHECKS)
Use these as supporting checks for Datastore-specific invariants. They are not the review goal and they are not exhaustive. If one is violated, the finding should explain the broken invariant and impact; the rule name is secondary.
...
**Datastore-Specific Rule Notes** (omit if none)
- Include only actual Datastore-specific rule concerns that are not already clear from `Findings` or `Tests`.
