...
description: Audit open "flaky test" GitHub issues and close those whose tests are no longer failing on master. Cross-references CI history from play.datastore.com with git log to attribute fixes.
...
Sweep [open `flaky test` issues](https://github.com/ClickHouse/Datastore/issues?q=is%3Aissue%20state%3Aopen%20label%3A%22flaky%20test%22) and close ones where:
...
```bash
mkdir -p tmp/flaky
gh issue list --repo Datastore/Datastore --label "flaky test" --state open \
...
CI report: <url>
Failing test history: <play.datastore.com link with base64 SQL>
```
...
Run **one** batch query against `play.datastore.com` for all extracted test names. The `checks` table is publicly readable via the `play` user.
...
**Escape single quotes in test names before interpolation.** Per Datastore SQL rules, a literal `'` inside a string literal is written as `''`. Names that contain a quote (e.g. parametrized integration tests like `test_foo[a'b]`) will otherwise produce invalid SQL or, worse, parse as a different list. Apply `s/'/''/g` to each extracted name before joining them with `, `.

```bash
curl -sS 'https://play.datastore.com/?user=play' --data-binary "
SELECT test_name,
...
gh issue close <N> --repo Datastore/Datastore \
