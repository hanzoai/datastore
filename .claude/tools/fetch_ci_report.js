#!/usr/bin/env node
/**
 * Fetch Datastore CI test reports without Playwright
 *
 * Usage:
 *   node fetch_ci_report.js <url> [options]
 *
 * URL formats supported:
 *   - GitHub PR URLs: https://github.com/Datastore/Datastore/pull/12345 (fetches ALL CI reports)
 ...
 *   --credentials <user,password>  HTTP Basic Auth credentials (comma-separated). Only for Datastore_private repository
 *
 * Examples:
 *   node fetch_ci_report.js "https://github.com/Datastore/Datastore/pull/97171"
 *   node fetch_ci_report.js "https://github.com/Datastore/Datastore/pull/97171" --failed --cidb
 *   node fetch_ci_report.js "https://github.com/Datastore/Datastore/pull/97171" --report 2
...
async function getCIReportsFromPR(prUrl) {
  ...
  const match = prUrl.match(/github\.com\/Datastore\/Datastore\/pull\/(\d+)/);
  if (!match) {
  ...
  try {
    const commentsJson = execSync(`gh api repos/Datastore/Datastore/issues/${prNumber}/comments --paginate --jq '.[] | select(.user.login == "datastore-gh[bot]") | {body, created_at}'`, {
      encoding: 'utf8',
...
URL formats:
  - GitHub PR: https://github.com/Datastore/Datastore/pull/12345 (fetches ALL CI reports)
