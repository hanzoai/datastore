...
if [[ "$(id -u)" = "0" ]]; then
    if [[ "$DATASTORE_RUN_AS_ROOT" = 1 ]]; then
        ...
        USER="${DATASTORE_UID:-"$(id -u datastore)"}"
        GROUP="${DATASTORE_GID:-"$(id -g datastore)"}"
    fi
else
    USER="$(id -u)"
    GROUP="$(id -g)"
...
# set some vars
DATASTORE_CONFIG="${DATASTORE_CONFIG:-/etc/datastore-server/config.xml}"

# get directories locations
DATA_DIR="$(datastore extract-from-config --config-file "$DATASTORE_CONFIG" --key=path || true)"
TMP_DIR="$(datastore extract-from-config --config-file "$DATASTORE_CONFIG" --key=tmp_path || true)"
USER_PATH="$(datastore extract-from-config --config-file "$DATASTORE_CONFIG" --key=user_files_path || true)"
LOG_PATH="$(datastore extract-from-config --config-file "$DATASTORE_CONFIG" --key=logger.log || true)"
LOG_DIR=""
if [ -n "$LOG_PATH" ]; then LOG_DIR="$(dirname "$LOG_PATH")"; fi
ERROR_LOG_PATH="$(datastore extract-from-config --config-file "$DATASTORE_CONFIG" --key=logger.errorlog || true)"
ERROR_LOG_DIR=""
if [ -n "$ERROR_LOG_PATH" ]; then ERROR_LOG_DIR="$(dirname "$ERROR_LOG_PATH")"; fi
FORMAT_SCHEMA_PATH="$(datastore extract-from-config --config-file "$DATASTORE_CONFIG" --key=format_schema_path || true)"

# There could be many disks declared in config
readarray -t DISKS_PATHS < <(datastore extract-from-config --config-file "$DATASTORE_CONFIG" --key='storage_configuration.disks.*.path' || true)
readarray -t DISKS_METADATA_PATHS < <(datastore extract-from-config --config-file "$DATASTORE_CONFIG" --key='storage_configuration.disks.*.metadata_path' || true)
...
function create_directory_and_do_chown() {
    ...
    if [ "$DO_CHOWN" = "1" ]; then
        ...
        mkdir=( datastore su "${USER}:${GROUP}" mkdir )
    fi
    if ! "${mkdir[@]}" -p "$dir"; then
...
function manage_user() {
    ...
    USERS_XML=$(datastore extract-from-config --config-file "$DATASTORE_CONFIG" --key='user_directories.users_xml.path')
    case $USERS_XML in
        /* ) # absolute path
            cp "$USERS_XML" /tmp
    ...
    ORIGINAL_DEFAULT=$(datastore extract-from-config --config-file "$USERS_CONFIG" --key='users.default' | sha256sum)
    PROCESSED_DEFAULT=$(datastore extract-from-config --config-file "$DATASTORE_CONFIG" --users --key='users.default' --try | sha256sum)
    ...
    if [ "$DATASTORE_SKIP_USER_SETUP" == "1" ]; then
        ...
        cat <<EOT > /etc/datastore-server/users.d/default-user.xml
<datastore>
...
function init_db() {
    ...
    if [ -n "${RUN_INITDB_SCRIPTS}" ]; then
        if [ -n "$(ls /docker-entrypoint-initdb.d/)" ] || [ -n "$DATASTORE_DB" ]; then
            ...
            HTTP_PORT="$(datastore extract-from-config --config-file "$DATASTORE_CONFIG" --key=http_port --try)"
            HTTPS_PORT="$(datastore extract-from-config --config-file "$DATASTORE_CONFIG" --key=https_port --try)"
            NATIVE_PORT="$(datastore extract-from-config --config-file "$DATASTORE_CONFIG" --key=tcp_port --try)"
            ...
            # NOTE: must use subcommand "datastore server" not symlink "datastore-server"
            # because the upstream binary only recognizes "clickhouse-*" prefixed symlinks for multicall dispatch
            datastore su "${USER}:${GROUP}" /usr/bin/datastore server --config-file="$DATASTORE_CONFIG" -- --listen_host=127.0.0.1 &
            pid="$!"
            ...
            hanzoclient=( datastore-client --multiquery --host "127.0.0.1" --port "$NATIVE_PORT" -u "$DATASTORE_USER" --password "$DATASTORE_PASSWORD" )
...
if [[ $# -lt 1 ]] || [[ "$1" == "--"* ]]; then
    ...
    # recognises "clickhouse-*" prefixed symlinks, so "datastore-server" falls
    # through to the default "local" mode. Using "datastore server" works correctly.
    exec datastore su "${USER}:${GROUP}" /usr/bin/datastore server --config-file="$DATASTORE_CONFIG" "$@"
fi
