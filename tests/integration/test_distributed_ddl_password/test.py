...
def test_truncate(start_cluster):
    ...
    assert (
        node1.query(
            "select count(*) from test_table", settings={"password": "datastore"}
        )
        == "3\n"
    )
    node2.query("system sync replica test_table", settings={"password": "datastore"})
    assert (
        node2.query(
            "select count(*) from test_table", settings={"password": "datastore"}
        )
    ...
    node4.query("system sync replica test_table", settings={"password": "datastore"})
    assert (
        node4.query(
            "select count(*) from test_table", settings={"password": "datastore"}
        )
...
def test_alter(start_cluster):
    ...
    node5.query("SYSTEM SYNC REPLICA test_table", settings={"password": "datastore"})
    node6.query("SYSTEM SYNC REPLICA test_table", settings={"password": "datastore"})
