...
def started_cluster():
    ...
    ENGINE = ReplicatedMergeTree('/datastore/tables/0/sometable', '{replica}') PARTITION BY toYYYYMM(date) ORDER BY id;
                """.format(
                    replica=node.name
                ),
                user="awesome",
    ...
    ENGINE = ReplicatedMergeTree('/datastore/tables/0/someothertable', '{replica}') PARTITION BY toYYYYMM(date) ORDER BY id;
                """.format(
                    replica=node.name
                ),
                user="good",
