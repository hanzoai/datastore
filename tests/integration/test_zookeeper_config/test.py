...
def test_chroot_with_same_root(started_cluster):
    for i, node in enumerate([node1, node2]):
        ...
        ENGINE = ReplicatedMergeTree('/datastore/tables/0/simple', '{replica}') PARTITION BY toYYYYMM(date) ORDER BY id;
        """.format(
                replica=node.name
            )
        )
...
def test_chroot_with_different_root(started_cluster):
    for i, node in [(1, node1), (3, node3)]:
        ...
        ENGINE = ReplicatedMergeTree('/datastore/tables/0/simple_different', '{replica}') PARTITION BY toYYYYMM(date) ORDER BY id;
        """.format(
                replica=node.name
            )
        )
