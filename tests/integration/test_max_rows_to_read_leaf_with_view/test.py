...
def test_max_rows_to_read_leaf_via_view(started_cluster):
    for node in (node1, node2):
        node.query(
            f"""
            CREATE TABLE local_table(id UInt32, d DateTime) ENGINE = ReplicatedMergeTree('/datastore/tables/0/max_rows_read_leaf', '{node.name}') PARTITION BY toYYYYMM(d) ORDER BY d;
