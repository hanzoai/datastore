...
def test_single_iceberg_file(started_cluster, format_version, storage_type, with_cache):
    ...
    assert "Path suffixes" in instance.query_and_get_error(
        f"CREATE TABLE {table_name_1} ENGINE=Iceberg('/var/lib/datastore/user_files/default', 'Parquet') SETTINGS disk = '{disk_name}'"
    )
...
def test_cluster_table_function(started_cluster, storage_type, with_cache):
    ...
    table_function_expr = (
        ...
        if storage_type == "s3"
        else f"iceberg('var/lib/datastore/user_files/iceberg_data/default/{TABLE_NAME}', SETTINGS disk = '{disk_name}')"
    )
    select_regular = (
    ...
    table_function_expr_cluster = (
        ...
        if storage_type == "s3"
        else f"icebergCluster('cluster_simple', 'var/lib/datastore/user_files/iceberg_data/default/{TABLE_NAME}', SETTINGS disk = '{disk_name}')"
    )
    select_cluster = (
