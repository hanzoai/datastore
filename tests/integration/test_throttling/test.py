...
def revert_config():
    ...
    node.exec_in_container(
        [
            "bash",
            "-c",
            f"echo '<datastore></datastore>' > /etc/clickhouse-server/config.d/dynamic_overrides.xml",
        ]
    )
    node.exec_in_container(
        [
            "bash",
            "-c",
            f"echo '<datastore></datastore>' > /etc/clickhouse-server/users.d/users_overrides.xml",
        ]
    )
