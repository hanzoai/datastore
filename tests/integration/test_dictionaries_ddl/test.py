...
def started_cluster():
    try:
        cluster.start()
        for datastore in [node1, node2, node3, node4, node5]:
            datastore.query("CREATE DATABASE test", user="admin")
            datastore.query(
...
def test_create_and_select_mysql(started_cluster, datastore, name, layout):
    ...
    datastore.query("SYSTEM RELOAD DICTIONARY 'default.{}'".format(name))
    ...
    datastore.query(f"drop dictionary default.{name}")
...
def test_restricted_database(started_cluster):
    ...
    with pytest.raises(QueryRuntimeException):
        ...
        PRIMARY KEY id
        LAYOUT(FLAT())
        SOURCE(DATASTORE(HOST 'localhost' PORT 9000 USER 'default' TABLE 'table_in_restricted_db' DB 'restricted_db'))
...
def test_conflicting_name(started_cluster):
    ...
    with pytest.raises(QueryRuntimeException):
        ...
        PRIMARY KEY id
        LAYOUT(FLAT())
        SOURCE(DATASTORE(HOST 'localhost' PORT 9000 USER 'default' TABLE 'xml_dictionary_table' DB 'test'))
        LIFETIME(MIN 1 MAX 10)
        """
        )
...
def test_with_insert_query(started_cluster):
    try:
        ...
        PRIMARY KEY id
        LAYOUT(FLAT())
        SOURCE(DATASTORE(HOST 'localhost' PORT 9000 USER 'default' QUERY 'insert into test.xml_dictionary_table values (234432, 29, "htms")'))
        ...
        assert (
            "Syntax error (Query for Datastore dictionary test.ddl_dictionary_with_insert_query)"
            in str(ex)
        )
...
def test_clickhouse_remote(started_cluster):
    with pytest.raises(QueryRuntimeException):
        ...
        PRIMARY KEY id
        LAYOUT(FLAT())
        SOURCE(DATASTORE(HOST 'node4' PORT 9000 USER 'default' TABLE 'xml_dictionary_table' DB 'test'))
        LIFETIME(MIN 1 MAX 10)
        """
        )
        for i in range(5):
    ...
    with pytest.raises(QueryRuntimeException):
        node3.query(
            ...
            PRIMARY KEY id
            LAYOUT(FLAT())
            SOURCE(DATASTORE(HOST 'node4' PORT 9000 USER 'default' PASSWORD 'default' TABLE 'xml_dictionary_table' DB 'test'))
            LIFETIME(MIN 1 MAX 10)
            """
        )
...
# https://github.com/ClickHouse/Datastore/issues/38450
