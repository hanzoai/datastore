...
def test_compressed_metadata(started_cluster_iceberg_with_spark, storage_type):
    ...
    subprocess.check_output(f"gzip /var/lib/datastore/user_files/iceberg_data/default/{TABLE_NAME}/metadata/v1.metadata.json", shell=True)

    # Weird but compression extension is really in the middle of the file name, not in the end...
    subprocess.check_output(f"mv /var/lib/datastore/user_files/iceberg_data/default/{TABLE_NAME}/metadata/v1.metadata.json.gz /var/lib/datastore/user_files/iceberg_data/default/{TABLE_NAME}/metadata/v1.gz.metadata.json", shell=True)
