...
def test_metadata_file_format_with_uuid(started_cluster_iceberg_with_spark, format_version, storage_type):
    ...
    for i in range(50):
        os.rename(
            f"/var/lib/datastore/user_files/iceberg_data/default/{TABLE_NAME}/metadata/v{i + 1}.metadata.json",
            f"/var/lib/datastore/user_files/iceberg_data/default/{TABLE_NAME}/metadata/{str(i).zfill(5)}-{get_uuid_str()}.metadata.json",
        )
