    ...
    for node in nodes:
        ...
        node.query(
            """
                ...
                ENGINE = ReplicatedMergeTree('/datastore/tables/test1/{shard}/replicated/test_table', '{replica}') ORDER BY id PARTITION BY toYYYYMM(date) 
                SETTINGS min_replicated_logs_to_keep=3, max_replicated_logs_to_keep=5, cleanup_delay_period=0, cleanup_delay_period_random_add=0, cleanup_thread_preferred_points_per_iteration=0;
            """.format(
                shard=shard, replica=node.name
            )
        ...
        node.query(
            """
                ...
                ENGINE = ReplicatedMergeTree('/datastore/tables/test2/{shard}/replicated/test_table', '{replica}') ORDER BY id PARTITION BY toYYYYMM(date) 
                SETTINGS min_replicated_logs_to_keep=3, max_replicated_logs_to_keep=5, cleanup_delay_period=0, cleanup_delay_period_random_add=0, cleanup_thread_preferred_points_per_iteration=0;
            """.format(
                shard=shard, replica=node.name
            )
        ...
        node.query(
            ...
            CREATE DATABASE test3;
                ...
                ENGINE = ReplicatedMergeTree('/datastore/tables/test3/{shard}/replicated/test_table', '{replica}') ORDER BY id PARTITION BY toYYYYMM(date) 
                SETTINGS min_replicated_logs_to_keep=3, max_replicated_logs_to_keep=5, cleanup_delay_period=0, cleanup_delay_period_random_add=0, cleanup_thread_preferred_points_per_iteration=0;
            """.format(
                shard=shard, replica=node.name
            )
        ...
        node.query(
            """
                ...
                ENGINE = ReplicatedMergeTree('/datastore/tables/test4/{shard}/replicated/test_table', '{replica}') ORDER BY id PARTITION BY toYYYYMM(date) 
                SETTINGS min_replicated_logs_to_keep=3, max_replicated_logs_to_keep=5, cleanup_delay_period=0, cleanup_delay_period_random_add=0, cleanup_thread_preferred_points_per_iteration=0;
            """.format(
                shard=shard, replica=node.name
            )
...
def test_drop_replica(start_cluster):
    ...
    assert "it's active" in node_1_3.query_and_get_error(
        "SYSTEM DROP REPLICA 'node_1_1' FROM ZKPATH '/datastore/tables/test/{shard}/replicated/test_table'".format(
            shard=1
        )
    )
    ...
    assert "does not look like a table path" in node_1_3.query_and_get_error(
        "SYSTEM DROP REPLICA 'node_1_1' FROM ZKPATH '/datastore/tables/test'"
    )
    ...
    exists_replica_1_1 = check_exists(
        zk,
        "/datastore/tables/test3/{shard}/replicated/test_table/replicas/{replica}".format(
    ...
    node_1_3.query(
        "SYSTEM DROP REPLICA 'node_1_1' FROM ZKPATH '/datastore/tables/test2/{shard}/replicated/test_table'".format(
            shard=1
        )
    )
    exists_replica_1_1 = check_exists(
        zk,
        "/datastore/tables/test2/{shard}/replicated/test_table/replicas/{replica}".format(
            shard=1, replica="node_1_1"
        ),
    )
    assert exists_replica_1_1 == None
    ...
    exists_replica_1_1 = check_exists(
        zk,
        "/datastore/tables/test/{shard}/replicated/test_table/replicas/{replica}".format(
            shard=1, replica="node_1_1"
        ),
    )
    assert exists_replica_1_1 == None
    ...
    exists_replica_1_1 = check_exists(
        zk,
        "/datastore/tables/test1/{shard}/replicated/test_table/replicas/{replica}".format(
            shard=1, replica="node_1_1"
        ),
    )
    assert exists_replica_1_1 == None
    ...
    node_1_3.query(
        "SYSTEM DROP REPLICA 'node_1_1' FROM ZKPATH '/datastore/tables/test3/{shard}/replicated/test_table'".format(
            shard=1
        )
    )
    exists_replica_1_1 = check_exists(
    ...
    node_1_1.query(
        "SYSTEM DROP REPLICA 'node_1_2' FROM ZKPATH '/datastore/tables/test4/{shard}/replicated/test_table'".format(
            shard=1
        )
    )
    exists_replica_1_2 = check_exists(
        zk,
        "/datastore/tables/test4/{shard}/replicated/test_table/replicas/{replica}".format(
