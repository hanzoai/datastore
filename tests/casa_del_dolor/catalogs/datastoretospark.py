...
class ClickHouseTypeMapper:
    """Maps between Datastore and Spark and Iceberg data types with string representations."""

    def __init__(self):
        # Basic type mappings from Datastore to Spark SQL strings
        self.field_id = 1
    ...
    def clickhouse_to_spark(
        ...
        """
        Convert Datastore type to Spark or Iceberg type with string representation.

        Args:
            ch_type: Datastore type string (e.g., 'Int32', 'Nullable(String)', 'Array(Int32)')
    ...
    def _split_named_element(self, element: str) -> Tuple[Optional[str], str]:
        ...
        - "`quoted_name` Type"  (backtick-quoted identifiers from Datastore)
        """
    ...
    def generate_random_clickhouse_type(
        ...
        """Generate a random Datastore SQL type string."""
        ...
        if roll <= 60 or current_depth >= max_depth or not allow_complex:
            ...
            # Map keys must be primitive in Datastore
