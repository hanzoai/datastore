...
LIBRARY_BRANCH = "Datastore-docker-library"
...
def update_docs(repos: LibraryRepos, dry_run: bool = True) -> None:
    ...
    repos.docs.parent.create_pull(
        ...
        head=pr_head,
        title=f"Update {ch_docs.name} docs according to Datastore/Datastore",
...
def update_library_images(repos: LibraryRepos, dry_run: bool = True) -> None:
    ...
    - If has:
        ...
        - Push changes if there are
    - Create directories in Datastore/docker-library via
    ...
    - Update Datastore/docker-library-official-images to the upstream
    - Create a branch in Datastore/docker-library-official-images
    - Copy `Datastore/docker-library/datastore` to
        `Datastore/docker-library-official-images/datastore`
    - Commit, create a PR
    """
    org = repos.ldf.owner.name
...
if __name__ == "__main__":
    ...
    if os.getenv("ROBOT_DATASTORE_SSH_KEY", ""):
        with SSHKey("ROBOT_DATASTORE_SSH_KEY"):
            main()
    else:
        main()
