...
"""
    ...
    def _assign_new_pr(self, new_pr: PullRequest) -> None:
        ...
        assignees = [
            a
            for a in assignees
            if "robot-datastore" not in str(a) and "datastore-gh" not in str(a)
        ]
        logging.info(
...
class BackportPRs:
    ...
    def remote_line(self) -> str:
        if not self._remote_line:
            # lines of "origin	git@github.com:Datastore/Datastore.git (fetch)"
        ...
        if is_general_backport:
            ...
            branches = [
                ReleaseBranch(
                    (
                        br
                        if self._repo_name == "Datastore/Datastore"
                        else f"release/{br}"
                    ),
                    pr,
                    self.repo,
