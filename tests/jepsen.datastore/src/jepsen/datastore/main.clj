...
(ns jepsen.datastore.main
  (:require [jepsen.datastore.keeper.main]
            [jepsen.datastore.server.main]))

(defn -main
  [f & args]
  (cond
   (= f "keeper") (apply jepsen.datastore.keeper.main/main args)
   (= f "server") (apply jepsen.datastore.server.main/main args)
   (some #(= f %) ["test" "test-all"]) (apply jepsen.datastore.keeper.main/main f args) ;; backwards compatibility
