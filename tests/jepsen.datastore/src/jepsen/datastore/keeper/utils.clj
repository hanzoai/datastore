(ns jepsen.datastore.keeper.utils
...
(defn zk-connect
  ...
  (let [conn (zk/connect (str host ":" port) :timeout-msec timeout)]
    (if with-auth
      (do
        (zk/add-auth-info conn "digest" "datastore:withauth")
        conn)
      conn)))
...
(defn start-datastore!
  ...
  (chu/start-datastore!
    ...
    :--
    :--logger.log (str logs-dir "/datastore-keeper.log")
    :--logger.errorlog (str logs-dir "/datastore-keeper.err.log")
