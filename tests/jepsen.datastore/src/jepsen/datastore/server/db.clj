...
(ns jepsen.datastore.keeper.db)

(ns jepsen.datastore.server.db
  (:require [clojure.tools.logging :refer :all]
            ...
            [jepsen.datastore.keeper.utils :as keeperutils]
...
(defn keeper
  ...
  (chu/db version reuse-binary keeperutils/start-datastore! install-keeper-configs))
         ...
         (do
           ...
           (if (or (not (cu/exists? binary-path)) (not reuse-binary))
             (do (info "Downloading datastore")
                 (let [datastore-path (chu/download-datastore version)]
                   (chu/install-downloaded-datastore datastore-path)))
             (info "Binary already exsist on path" binary-path "skipping download"))
           (extra-setup test node)
           (info "Waiting for Keeper")
           (chu/wait-datastore-alive! keeper-node test keeper-alive?)
