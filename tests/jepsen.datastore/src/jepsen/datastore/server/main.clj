(ns jepsen.datastore.server.main
...
(defn datastore-func-tests
  ...
  (let [quorum (boolean (:quorum opts))
    ...
    (merge tests/noop-test
           opts
           {:name (str "datastore-server-"  (name (:workload opts)) "-" (name (:nemesis opts)))
            :os ubuntu/os
            :db (get-db opts)
...
(defn main
  ...
  (cli/run! (merge (cli/single-test-cmd {:test-fn datastore-server-test
                                         :opt-spec cli-opts})
                   (cli/test-all-cmd {:tests-fn (partial all-tests datastore-server-test)
                                      :opt-spec cli-opts})
                   (cli/serve-cmd))
            args))
