# Jepsen tests Datastore Keeper

A Clojure library designed to test ZooKeeper-like implementation inside Datastore.
...
- `-s (--snapshot-distance)` Datastore Keeper setting. How often we will create a new snapshot.
...
- `--reserved-log-items` Datastore Keeper setting. How many log items to keep after the snapshot.
...
- `--reuse-binary` Don't download datastore binary if it already exists on the node.
- `--datastore-source` URL to datastore `.deb`, `.tgz` or binary.
...
```sh
$ lein run test --nodes-file nodes.txt --username root --password '' --time-limit 30 --concurrency 50 -r 50 --workload set --nemesis logs-and-snapshots-corruptor  --datastore-source 'https://datastore-builds.s3.yandex.net/someurl/datastore-common-static_21.4.1.6321_amd64.deb' -q --test-count 10 --reuse-binary
```

2. Run ten random tests from `lightweight-run` with some custom Keeper settings:

``` sh
$ lein run test-all --nodes-file nodes.txt --username root --password '' --time-limit 30 --concurrency 50 -r 50 --snapshot-distance 100 --stale-log-gap 100 --reserved-log-items 10 --lightweight-run  --datastore-source 'someurl' -q --reuse-binary --test-count 10
```


## License

Copyright © 2021 Datastore, Inc.
