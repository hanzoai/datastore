...
DB="test_04093_${DATASTORE_DATABASE}"

$DATASTORE_CLIENT -q "DROP DATABASE IF EXISTS ${DB}"
$DATASTORE_CLIENT -q "CREATE DATABASE ${DB} ENGINE = Memory"
...
$DATASTORE_CLIENT -q "CREATE TABLE ${DB}.t (${COLS}) ENGINE = Memory"
...
function alter_worker()
{
    ...
    while [ $SECONDS -lt "$TIMELIMIT" ]; do
        $DATASTORE_CLIENT -q "ALTER TABLE ${DB}.t ADD COLUMN dyn_${tid}_${idx} UInt64" 2>/dev/null || true
        if [ "$idx" -gt 3 ]; then
            $DATASTORE_CLIENT -q "ALTER TABLE ${DB}.t DROP COLUMN dyn_${tid}_$(( idx - 3 ))" 2>/dev/null || true
        fi
        idx=$(( idx + 1 ))
    done
}
...
function read_worker()
{
    local TIMELIMIT=$((SECONDS + TIMEOUT))
    while [ $SECONDS -lt "$TIMELIMIT" ]; do
        $DATASTORE_CLIENT -q "SELECT create_table_query FROM system.tables WHERE database = '${DB}' AND name = 't'" > /dev/null 2>&1 || true
    done
}
...
# is not exceeded by many concurrent datastore-client ASan processes.
for i in {1..10}; do
