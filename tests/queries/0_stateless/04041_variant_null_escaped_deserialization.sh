...
${DATASTORE_CURL} -sS "${DATASTORE_URL}&param_var=%5CN" \
...
${DATASTORE_CURL} -sS "${DATASTORE_URL}&param_var=42" \
...
${DATASTORE_CLIENT} -q "SELECT val, val IS NULL FROM format(TSV, 'val Variant(UInt64, Int32)', '\\\\N\n42\n-1') FORMAT TabSeparated"
