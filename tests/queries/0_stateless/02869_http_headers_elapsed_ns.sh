...
CURL_OUTPUT=$(echo 'SELECT number FROM numbers(10)' | \
  ${DATASTORE_CURL} -vsS "${DATASTORE_URL}&max_block_size=1&send_progress_in_http_headers=1&http_headers_progress_interval_ms=0&output_format_parallel_formatting=0" --data-binary @- 2>&1)

ELAPSED_NS_PROGRESS="$(echo "${CURL_OUTPUT}" | \
  grep 'X-Datastore-Progress' | grep -v 'Access-Control-Expose-Headers' | \
  awk '{print $3}' | \
