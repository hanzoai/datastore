...
${DATASTORE_CLIENT} --query "DROP TABLE IF EXISTS t_corrupted_substreams"
...
${DATASTORE_CLIENT} --query "INSERT INTO t_corrupted_substreams SELECT number, [number, number + 1] FROM numbers(10)"

echo "Data before corruption:"
${DATASTORE_CLIENT} --query "SELECT count(), sum(id), sum(length(arr)) FROM t_corrupted_substreams"

# Get the data path of the active part.
DATA_PATH=$(${DATASTORE_CLIENT} --query "SELECT path FROM system.parts WHERE database = currentDatabase() AND table = 't_corrupted_substreams' AND active")

# Detach the table so we can modify files on disk.
${DATASTORE_CLIENT} --query "DETACH TABLE t_corrupted_substreams"
...
${DATASTORE_CLIENT} --query "ATTACH TABLE t_corrupted_substreams" 2>/dev/null
...
${DATASTORE_CLIENT} --query "CHECK TABLE t_corrupted_substreams SETTINGS check_query_single_value_result = 1"

# DETACH/ATTACH partition should also work.
${DATASTORE_CLIENT} --query "ALTER TABLE t_corrupted_substreams DETACH PARTITION tuple()"
${DATASTORE_CLIENT} --query "ALTER TABLE t_corrupted_substreams ATTACH PARTITION tuple()" 2>/dev/null

echo "Data after partition reattach:"
${DATASTORE_CLIENT} --query "SELECT count(), sum(id), sum(length(arr)) FROM t_corrupted_substreams"

${DATASTORE_CLIENT} --query "DROP TABLE t_corrupted_substreams"

# ---- Test 2: JSON column (dynamic substreams, exercises enumerate_dynamic_streams = false fallback) ----

${DATASTORE_CLIENT} --query "DROP TABLE IF EXISTS t_corrupted_substreams_json"
...
${DATASTORE_CLIENT} --query "INSERT INTO t_corrupted_substreams_json VALUES (1, '{\"a\": 1, \"b\": \"hello\"}'), (2, '{\"a\": 2, \"c\": [1, 2, 3]}')"

echo "JSON data before corruption:"
${DATASTORE_CLIENT} --query "SELECT id, data.a FROM t_corrupted_substreams_json ORDER BY id"

# Get the data path of the active part.
DATA_PATH_JSON=$(${DATASTORE_CLIENT} --query "SELECT path FROM system.parts WHERE database = currentDatabase() AND table = 't_corrupted_substreams_json' AND active")

# Detach the table so we can modify files on disk.
${DATASTORE_CLIENT} --query "DETACH TABLE t_corrupted_substreams_json"
...
${DATASTORE_CLIENT} --query "ATTACH TABLE t_corrupted_substreams_json" 2>/dev/null

echo "JSON data after attach with corrupted file:"
${DATASTORE_CLIENT} --query "SELECT id, data.a FROM t_corrupted_substreams_json ORDER BY id"

echo "JSON CHECK TABLE result:"
${DATASTORE_CLIENT} --query "CHECK TABLE t_corrupted_substreams_json SETTINGS check_query_single_value_result = 1"

# DETACH/ATTACH partition should also work.
${DATASTORE_CLIENT} --query "ALTER TABLE t_corrupted_substreams_json DETACH PARTITION tuple()"
${DATASTORE_CLIENT} --query "ALTER TABLE t_corrupted_substreams_json ATTACH PARTITION tuple()" 2>/dev/null

echo "JSON data after partition reattach:"
${DATASTORE_CLIENT} --query "SELECT id, data.a FROM t_corrupted_substreams_json ORDER BY id"

${DATASTORE_CLIENT} --query "DROP TABLE t_corrupted_substreams_json"
