...
DB="db_04117_${DATASTORE_DATABASE}"
SELECT_QID="select_04117_${DATASTORE_DATABASE}"
DETACH_QID="detach_04117_${DATASTORE_DATABASE}"
...
cleanup()
{
    ...
    $DATASTORE_CLIENT --query "KILL QUERY WHERE query_id = '$SELECT_QID' SYNC FORMAT Null" 2>/dev/null || true
    $DATASTORE_CLIENT --query "KILL QUERY WHERE query_id = '$DETACH_QID' SYNC FORMAT Null" 2>/dev/null || true
    $DATASTORE_CLIENT --database_atomic_wait_for_drop_and_detach_synchronously=0 --query "ATTACH DATABASE IF NOT EXISTS \`$DB\`" 2>/dev/null || true
    $DATASTORE_CLIENT --database_atomic_wait_for_drop_and_detach_synchronously=0 --query "DROP DATABASE IF EXISTS \`$DB\` SYNC" 2>/dev/null || true
}
trap cleanup EXIT
...
wait_for_query_to_start()
{
    ...
    while [[ "$($DATASTORE_CLIENT --query "SELECT count() FROM system.processes WHERE query_id = '$qid'")" == "0" ]]; do
...
$DATASTORE_CLIENT --query "DROP DATABASE IF EXISTS \`$DB\` SYNC"
$DATASTORE_CLIENT --query "CREATE DATABASE \`$DB\` ENGINE = Atomic"
$DATASTORE_CLIENT --query "CREATE TABLE \`$DB\`.t (x UInt64) ENGINE = MergeTree ORDER BY x"
$DATASTORE_CLIENT --query "INSERT INTO \`$DB\`.t SELECT number FROM numbers(100)"
...
$DATASTORE_CLIENT --query_id "$SELECT_QID" \
    --max_threads=1 \
...
$DATASTORE_CLIENT --query_id "$DETACH_QID" \
...
# "DETACH cancelled".
$DATASTORE_CLIENT --query "KILL QUERY WHERE query_id = '$DETACH_QID' ASYNC FORMAT Null" >/dev/null
