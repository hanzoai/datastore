...
cat ${CUR_DIR}/wasm/host_api.wasm | ${DATASTORE_CLIENT} --query "INSERT INTO system.webassembly_modules (name, code) SELECT 'test_host_api', code FROM input('code String') FORMAT RawBlob"
...
${DATASTORE_CLIENT}  --allow_experimental_analyzer=1 --allow_repeated_settings --send_logs_level=debug --query \
    "SELECT test_host_api(materialize(0) :: UInt32) SETTINGS log_comment = '03208_wasm_import_export_ok' FORMAT Null" 2>&1 | grep -o "Hello, Datastore"

${DATASTORE_CLIENT}  --allow_experimental_analyzer=1 --query \
    "SELECT test_host_api(materialize(1) :: UInt32) SETTINGS log_comment = '03208_wasm_import_export_err' FORMAT Null" 2>&1 | grep 'DB::Exception' | grep -o "WebAssembly UDF terminated with error: Goodbye, Datastore" | head -1
...
${DATASTORE_CLIENT} --allow_experimental_analyzer=1 --allow_repeated_settings --send_logs_level=warning --query \
...
${DATASTORE_CLIENT} --allow_experimental_analyzer=1 --query "SELECT test_log(100 :: UInt32)"

cat ${CUR_DIR}/wasm/import_unknown.wasm | ${DATASTORE_CLIENT} --query "INSERT INTO system.webassembly_modules (name, code) SELECT 'import_unknown', code FROM input('code String') FORMAT RawBlob"
...
cat ${CUR_DIR}/wasm/import_non_env.wasm | ${DATASTORE_CLIENT} --query "INSERT INTO system.webassembly_modules (name, code) SELECT 'import_non_env', code FROM input('code String') FORMAT RawBlob"
...
cat ${CUR_DIR}/wasm/import_incorrect.wasm | ${DATASTORE_CLIENT} --query "INSERT INTO system.webassembly_modules (name, code) SELECT 'import_incorrect', code FROM input('code String') FORMAT RawBlob"
...
cat ${CUR_DIR}/wasm/export_incorrect_malloc.wasm | ${DATASTORE_CLIENT} --query "INSERT INTO system.webassembly_modules (name, code) SELECT 'export_incorrect_malloc', code FROM input('code String') FORMAT RawBlob"
...
cat ${CUR_DIR}/wasm/export_faulty_malloc.wasm | ${DATASTORE_CLIENT} --query "INSERT INTO system.webassembly_modules (name, code) SELECT 'export_faulty_malloc', code FROM input('code String') FORMAT RawBlob"
