...
$DATASTORE_CLIENT -q "CREATE TABLE test_sort_proj (x UInt32, y UInt32, PROJECTION p (SELECT x, y ORDER BY y)) ENGINE = MergeTree ORDER BY x SETTINGS index_granularity=8192, index_granularity_bytes='10Mi'"
$DATASTORE_CLIENT -q "insert into test_sort_proj select number, toUInt32(-number - 1) from numbers(100)"

echo "select where x < 10"

echo "optimize_move_to_prewhere = 0, optimize_use_projections = 0"
$DATASTORE_CLIENT -q "SELECT * FROM test_sort_proj WHERE x < 10 order by x
...
echo "optimize_move_to_prewhere = 0, optimize_use_projections = 0"
$DATASTORE_CLIENT -q "SELECT * FROM test_sort_proj WHERE x < 10 order by x FORMAT JSON
...
echo "optimize_move_to_prewhere = 0, optimize_use_projections = 0"
$DATASTORE_CLIENT -q "SELECT * FROM test_sort_proj WHERE y > 4294967286 order by x
                       SETTINGS optimize_move_to_prewhere = 0, optimize_use_projections = 0"
...
echo "optimize_move_to_prewhere = 0, optimize_use_projections = 0"
$DATASTORE_CLIENT -q "SELECT * FROM test_sort_proj WHERE y > 4294967286 order by x FORMAT JSON
                       SETTINGS optimize_move_to_prewhere = 0, optimize_use_projections = 0" | grep rows_read
...
$DATASTORE_CLIENT -q "ALTER TABLE test_sort_proj DELETE WHERE x % 2 = 0 SETTINGS mutations_sync=2;"
$DATASTORE_CLIENT -q "SELECT count() from test_sort_proj;"

$DATASTORE_CLIENT -q "DROP TABLE test_sort_proj"
