...
TABLE_PREFIX="test_race_drop_undrop_${DATASTORE_TEST_UNIQUE_NAME}"

function cleanup()
{
    for i in {1..4}; do
        $DATASTORE_CLIENT --query "DROP TABLE IF EXISTS ${TABLE_PREFIX}_$i SYNC" 2>/dev/null ||:
    done
}
...
for iter in {1..3}; do
    ...
    for i in {1..4}; do
        $DATASTORE_CLIENT --query "CREATE TABLE IF NOT EXISTS ${TABLE_PREFIX}_$i (x Int32) ENGINE = Memory" 2>/dev/null ||:
        $DATASTORE_CLIENT --query "INSERT INTO ${TABLE_PREFIX}_$i VALUES ($i)" 2>/dev/null ||:
    done
    ...
    for i in {1..4}; do
        $DATASTORE_CLIENT --query "DROP TABLE IF EXISTS ${TABLE_PREFIX}_$i" \
    ...
    for i in {1..4}; do
        $DATASTORE_CLIENT --query "UNDROP TABLE ${TABLE_PREFIX}_$i" 2>/dev/null &
    done
    wait
