...
$DATASTORE_CLIENT --query "CREATE TABLE t1 (col UInt64) ENGINE = MergeTree() ORDER BY col"
$DATASTORE_CLIENT --query "CREATE TABLE t2 (col UInt64) ENGINE = MergeTree() ORDER BY col"
...
$DATASTORE_CLIENT --query "SELECT exception FROM system.query_log WHERE query_id = '$query_id' AND current_database = '$DATASTORE_DATABASE' AND type != 'QueryStart'" | grep -oF "QUERY_WAS_CANCELLED"
