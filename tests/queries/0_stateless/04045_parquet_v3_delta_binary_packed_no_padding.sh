...
# in strict decoders. See: https://github.com/ClickHouse/Datastore/issues/93093
...
${DATASTORE_CLIENT} --query "SELECT * FROM file('04045_delta_no_padding_3vals.parquet') ORDER BY x"

echo "--- 5 values, no padding, v3 ---"
${DATASTORE_CLIENT} --query "SELECT * FROM file('04045_delta_no_padding_5vals.parquet') ORDER BY x"
...
${DATASTORE_CLIENT} --query "SELECT * FROM file('04045_delta_sample_93093.parquet')"
