...
# https://github.com/ClickHouse/Datastore/issues/101221

CURDIR=$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)
# shellcheck source=../shell_config.sh
. "$CURDIR"/../shell_config.sh

${DATASTORE_CLIENT} --query "DROP TABLE IF EXISTS test_async_result_rows"
${DATASTORE_CLIENT} --query "CREATE TABLE test_async_result_rows (x String) ENGINE = MergeTree ORDER BY x"

# Insert 3 rows via HTTP with async insert, capture X-Datastore-Summary
query_id="ASYNC_RESULT_ROWS_$RANDOM$RANDOM"
summary=$(${DATASTORE_CURL} -vsS "${DATASTORE_URL}&async_insert=1&wait_for_async_insert=1&query_id=$query_id" \
...
sync_summary=$(${DATASTORE_CURL} -vsS "${DATASTORE_URL}&async_insert=0&query_id=$sync_query_id" \
...
log_result_rows=$($DATASTORE_CLIENT --query "
...
${DATASTORE_CLIENT} --query "DROP TABLE test_async_result_rows"
