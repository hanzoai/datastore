...
if [ $($DATASTORE_LOCAL -q "SELECT value LIKE '%-g0%' FROM system.build_options WHERE name = 'CXX_FLAGS'") -eq 1 ]; then
    echo "@@SKIP@@: No debug symbols"
    exit 0
fi
# MacOS does not have file:line (backtrace_symbols() does not provide this info)
if [ $($DATASTORE_LOCAL -q "SELECT value = 'Darwin' FROM system.build_options WHERE name = 'SYSTEM'") -eq 1 ]; then
...
$DATASTORE_LOCAL --stacktrace -q 'select throwIf(1)' |& grep -q -P '(Common/Exception.cpp|libcxx/include/vector):[0-9]+:[0-9]+: DB::Exception::Exception' && echo 1 || $DATASTORE_LOCAL --stacktrace -q 'select throwIf(1)'
