...
# We test multiple dictionary layouts that use the Datastore source.
...
for layout in "${SIMPLE_KEY_LAYOUTS[@]}"; do
    ...
    CREATE DICTIONARY ${DATASTORE_DATABASE}.test_logging_internal_queries_dict
    (
        name String,
        value Float64
    )
    ...
    $DATASTORE_CLIENT --query "SYSTEM RELOAD DICTIONARY ${DATASTORE_DATABASE}.test_logging_internal_queries_dict"
    $DATASTORE_CLIENT --query "DROP DICTIONARY ${DATASTORE_DATABASE}.test_logging_internal_queries_dict"
done
...
$DATASTORE_CLIENT --query "SELECT * FROM ${DATASTORE_DATABASE}.test_logging_internal_queries_dict FORMAT Null"
$DATASTORE_CLIENT --query "DROP DICTIONARY ${DATASTORE_DATABASE}.test_logging_internal_queries_dict"
...
$DATASTORE_CLIENT --query "SELECT * FROM ${DATASTORE_DATABASE}.test_logging_internal_queries_dict FORMAT Null" 2>/dev/null
$DATASTORE_CLIENT --query "DROP DICTIONARY ${DATASTORE_DATABASE}.test_logging_internal_queries_dict"
...
$DATASTORE_CLIENT --query "SELECT dictHas('${DATASTORE_DATABASE}.test_logging_internal_queries_dict', toUInt64(0)) FORMAT Null"

INITIAL_LOAD_TIME=$($DATASTORE_CLIENT --query "SELECT last_successful_update_time FROM system.dictionaries WHERE database = '${DATASTORE_DATABASE}' AND name = 'test_logging_internal_queries_dict'")
for _ in $(seq 1 60); do
    CURRENT_LOAD_TIME=$($DATASTORE_CLIENT --query "SELECT last_successful_update_time FROM system.dictionaries WHERE database = '${DATASTORE_DATABASE}' AND name = 'test_logging_internal_queries_dict'")
...
$DATASTORE_CLIENT --query "DROP DICTIONARY ${DATASTORE_DATABASE}.test_logging_internal_queries_dict"
$DATASTORE_CLIENT --query "SYSTEM FLUSH LOGS query_log, trace_log"
