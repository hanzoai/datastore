...
$DATASTORE_CLIENT -m <<EOL
drop table if exists dist_01247;
drop table if exists data_01247;

create table data_01247 engine=Memory() as select * from numbers(2);
create table dist_01247 as data_01247 engine=Distributed(test_cluster_two_shards, '$DATASTORE_DATABASE', data_01247, number);
...
for ((i = 0; i < 100; ++i)); do
    network_errors_before=$($DATASTORE_CLIENT -q "SELECT value FROM system.errors WHERE name = 'NETWORK_ERROR'")
    ...
    $DATASTORE_CLIENT "${opts[@]}" --format CSV -m -q "select count(), * from dist_01247 group by number order by number limit 1 format Null"

    # expect zero new network errors
    network_errors_after=$($DATASTORE_CLIENT -q "SELECT value FROM system.errors WHERE name = 'NETWORK_ERROR'")
...
$DATASTORE_CLIENT -q "drop table data_01247"
$DATASTORE_CLIENT -q "drop table dist_01247"
