...
start_time=$($DATASTORE_CLIENT -q "SELECT now()")

# system.error_log is created lazily, flush logs query makes it sure that the table is created.
$DATASTORE_CLIENT -q "SYSTEM FLUSH LOGS error_log;"

# Get the previous number of errors for 111, 222 and 333
errors_111=$($DATASTORE_CLIENT -q "SELECT sum(value) FROM system.error_log WHERE event_date >= yesterday() AND event_time >= '$start_time' AND code = 111")
errors_222=$($DATASTORE_CLIENT -q "SELECT sum(value) FROM system.error_log WHERE event_date >= yesterday() AND event_time >= '$start_time' AND code = 222")
errors_333=$($DATASTORE_CLIENT -q "SELECT sum(value) FROM system.error_log WHERE event_date >= yesterday() AND event_time >= '$start_time' AND code = 333")
