...
${DATASTORE_CLIENT} -q "DROP TABLE IF EXISTS lazy_mark_test;"
...
${DATASTORE_CLIENT} -q "SYSTEM STOP MERGES lazy_mark_test"
${DATASTORE_CLIENT} -q "INSERT INTO lazy_mark_test select number, number % 3, number % 5, number % 10, number % 13, number % 15, number % 17, number % 18, number % 22, number % 25 from numbers(1000000)"
...
${DATASTORE_CLIENT} --log_queries=1 --query_id "${QUERY_ID}" -q "SELECT * FROM lazy_mark_test WHERE n3==11 SETTINGS load_marks_asynchronously=0, max_threads=1"
${DATASTORE_CLIENT} -q "SYSTEM FLUSH LOGS query_log"

# Expect 2 open files: n3 marks and n3 data.
${DATASTORE_CLIENT} -q "select ProfileEvents['FileOpen'] from system.query_log where event_date >= yesterday() AND event_time >= now() - 600 AND query_id = '${QUERY_ID}' and type = 'QueryFinish' and current_database = currentDatabase()"
