...
$DATASTORE_CLIENT -q "DROP TABLE IF EXISTS cancel_huge_mutation_subquery"
...
$DATASTORE_CLIENT --mutations_sync=2 -n -q "ALTER TABLE cancel_huge_mutation_subquery DELETE WHERE key IN (select number % 2 from numbers(100000000) where sleep(1) == 0)"   2>/dev/null &

# wait until mutation started
i=0
while [ "$($DATASTORE_CLIENT -q "SELECT count() FROM system.mutations WHERE table = 'cancel_huge_mutation_subquery' and database='${DATASTORE_DATABASE}' AND is_done = 0")" -ne 1 ]; do
    sleep 0.5
    i=$((i + 1))
    if [ $i -gt 100 ]; then
...
$DATASTORE_CLIENT  --query "SYSTEM STOP MERGES cancel_huge_mutation_subquery"
...
$DATASTORE_CLIENT --query "KILL MUTATION WHERE database='${DATASTORE_DATABASE}' AND table='cancel_huge_mutation_subquery'" > /dev/null
