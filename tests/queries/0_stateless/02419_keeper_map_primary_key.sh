...
$DATASTORE_CLIENT -q "DROP TABLE IF EXISTS 02419_test SYNC;"

test_primary_key()
{
    $DATASTORE_CLIENT -m -q "
    CREATE TABLE 02419_test (key UInt64, value Float64) Engine=KeeperMap('/' || currentDatabase() || '/test2419', 3) PRIMARY KEY($1);
