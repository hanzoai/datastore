...
WORKING_FOLDER_01528="${DATASTORE_TMP}/01528_clickhouse_local_prepare_parts"
...
## Checks scenario of preparing parts offline by datastore-local
...
## feed the table
${DATASTORE_LOCAL} --query "INSERT INTO local.test SELECT * FROM local.data_csv;" --path="${WORKING_FOLDER_01528}"

## check the parts were created
${DATASTORE_LOCAL} --query "SELECT * FROM local.test WHERE id < 10 ORDER BY id;" --path="${WORKING_FOLDER_01528}"
...
cat <<EOF | ${DATASTORE_LOCAL} --query "INSERT INTO local.test SELECT * FROM local.stdin;" --path="${WORKING_FOLDER_01528}"
...
${DATASTORE_LOCAL} --query "SELECT * FROM local.test WHERE id BETWEEN 10 AND 19 ORDER BY id;" --path="${WORKING_FOLDER_01528}"

#################

echo "Option 3. Prepare parts from from table with Engine=File defined via command line, read from stdin (pipe)"

cat <<EOF | ${DATASTORE_LOCAL} --query "INSERT INTO local.test SELECT * FROM table;" -S "id UInt64, d Date, s String" --input-format=CSV --path="${WORKING_FOLDER_01528}"
...
${DATASTORE_LOCAL} --query "SELECT * FROM local.test WHERE id BETWEEN 20 AND 29 ORDER BY id;" --path="${WORKING_FOLDER_01528}"

#################

echo "Possibility to run optimize on prepared parts before sending parts to server"

${DATASTORE_LOCAL} --query "OPTIMIZE TABLE local.test FINAL;" --path="${WORKING_FOLDER_01528}"

# ensure we have one part per partition
${DATASTORE_LOCAL} --query "SELECT toYYYYMM(d) m, uniqExact(_part) FROM local.test GROUP BY m ORDER BY m" --path="${WORKING_FOLDER_01528}"
