...
$DATASTORE_CLIENT --query "SELECT * FROM test WHERE _partition_id = '202501' FORMAT Null" --enable_analyzer=$enable_analyzer --print-profile-events 2>&1 \
...
$DATASTORE_CLIENT --query "SELECT * FROM test WHERE d = '2025-01-01' FORMAT Null" --enable_analyzer=$enable_analyzer --print-profile-events 2>&1 \
    | grep -q FilterPartsByVirtualColumnsMicroseconds && echo "enable_analyzer: $enable_analyzer, should not filter by virtual columns"
$DATASTORE_CLIENT --query "SELECT * FROM test WHERE d = '2025-01-01' AND 1 FORMAT Null" --enable_analyzer=$enable_analyzer --print-profile-events 2>&1 \
    | grep -q FilterPartsByVirtualColumnsMicroseconds && echo "enable_analyzer: $enable_analyzer, should not filter by virtual columns"
done
