...
done | $DATASTORE_CLIENT -m

# but these mutations should not be killed.
(
    for i in {1..100}; do
        echo "alter table data_01900_2 update s = 'bar_$i' where 1;"
    done | $DATASTORE_CLIENT -m --mutations_sync=1
) &
$DATASTORE_CLIENT --format Null -m -q "kill mutation where table = 'data_01900_1' and database = '$DATASTORE_DATABASE';"
wait

$DATASTORE_CLIENT -m -q "select * from data_01900_2"

$DATASTORE_CLIENT -q "drop table data_01900_1"
$DATASTORE_CLIENT -q "drop table data_01900_2"
