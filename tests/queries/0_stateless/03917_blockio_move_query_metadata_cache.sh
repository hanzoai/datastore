...
# Regression test for https://github.com/ClickHouse/Datastore/issues/95742
#
...
TABLE="test_cache_race_${DATASTORE_DATABASE}"
...
function create_and_fill()
{
    ...
    for i in $(seq 1 10); do
        $DATASTORE_CLIENT --query "INSERT INTO ${TABLE} SELECT number, toString(number) FROM numbers($((i * 100)), 100)" 2>/dev/null
    done
}
...
function detach_attach_thread()
{
    ...
    while [ $SECONDS -lt "$TIMELIMIT" ]; do
        ...
        $DATASTORE_CLIENT --query "DETACH TABLE ${TABLE}" 2>/dev/null
        sleep 0.0$RANDOM
        $DATASTORE_CLIENT --query "ATTACH TABLE ${TABLE}" 2>/dev/null
        sleep 0.0$RANDOM
    done
}
...
function drop_create_thread()
{
    ...
    while [ $SECONDS -lt "$TIMELIMIT" ]; do
        ...
        # access freed memory.
        $DATASTORE_CLIENT --query "DROP TABLE IF EXISTS ${TABLE}" 2>/dev/null
        sleep 0.0$RANDOM
        create_and_fill
        sleep 0.0$RANDOM
    done
