...
$DATASTORE_CLIENT -q "create table rmt1 (n int) engine=ReplicatedMergeTree('/test/02369/$DATASTORE_TEST_ZOOKEEPER_PREFIX/{database}', '1') order by n
...
$DATASTORE_CLIENT -q "create table rmt2 (n int) engine=ReplicatedMergeTree('/test/02369/$DATASTORE_TEST_ZOOKEEPER_PREFIX/{database}', '2') order by n"

$DATASTORE_CLIENT -q "system stop replicated sends rmt2"
$DATASTORE_CLIENT --insert_keeper_fault_injection_probability=0 -q "insert into rmt2 values (0);"
...
$DATASTORE_CLIENT -q "system sync replica rmt1 pull;"
...
(sleep 5 && $DATASTORE_CLIENT -q "system start replicated sends rmt2") &

$DATASTORE_CLIENT --optimize_throw_if_noop=1 -q "optimize table rmt1;"
$DATASTORE_CLIENT -q "system sync replica rmt1;"

$DATASTORE_CLIENT -q "select 1, *, _part from rmt1 order by n;"

path=$($DATASTORE_CLIENT -q "select path from system.parts where database='$DATASTORE_DATABASE' and table='rmt1' and name='all_1_2_1'")
...
$DATASTORE_CLIENT -q "select * from rmt1;" 2>&1 | grep LOGICAL_ERROR
$DATASTORE_CLIENT --min_bytes_to_use_direct_io=1 --local_filesystem_read_method=pread_threadpool -q "select * from rmt1;" 2>&1 | grep LOGICAL_ERROR

$DATASTORE_CLIENT -q "select sleep(0.1) from numbers($(($RANDOM % 30))) settings max_block_size=1 format Null"

$DATASTORE_CLIENT -q "detach table rmt1;"
...
$DATASTORE_CLIENT -q "system sync replica rmt1;"
$DATASTORE_CLIENT -q "select 3, *, _part from rmt1 order by n;"
