...
#
# https://github.com/ClickHouse/Datastore/issues/73783

CURDIR=$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)
# shellcheck source=../shell_config.sh
. "$CURDIR"/../shell_config.sh

$DATASTORE_CLIENT -q "DROP TABLE IF EXISTS dst"
...
$DATASTORE_CLIENT -q "INSERT INTO dst SELECT 1, number, 0 FROM numbers(100)"

# Source has a fixed known set of 10 rows for partition 1
$DATASTORE_CLIENT -q "INSERT INTO src SELECT 1, number, 1000 FROM numbers(10)"
...
function replace_and_check_thread()
{
    ...
    while [ $SECONDS -lt "$TIMELIMIT" ]; do
        $DATASTORE_CLIENT -q "ALTER TABLE dst REPLACE PARTITION id '1' FROM src" 2>/dev/null
        count=$($DATASTORE_CLIENT -q "SELECT count() FROM dst WHERE p = 1" 2>/dev/null)
...
function mutation_thread()
{
    local TIMELIMIT=$((SECONDS+TIMEOUT))
    while [ $SECONDS -lt "$TIMELIMIT" ]; do
        $DATASTORE_CLIENT -q "ALTER TABLE dst UPDATE y = y + 1 WHERE p = 1" 2>/dev/null
        sleep 0.0$RANDOM
    done
}
...
while [ $counter -lt 60 ]; do
    undone=$($DATASTORE_CLIENT -q "SELECT count() FROM system.mutations WHERE database = currentDatabase() AND table = 'dst' AND is_done = 0" 2>/dev/null)
...
$DATASTORE_CLIENT -q "DROP TABLE dst"
