...
$DATASTORE_CLIENT -q "CREATE ROW POLICY ${row_policy_a} ON test FOR SELECT USING x % 2 = 0"
$DATASTORE_CLIENT -q "CREATE ROW POLICY ${row_policy_b} ON test FOR SELECT USING x % 2 = 1"

echo 'with row policy a:'
$DATASTORE_CLIENT -q "CREATE USER ${user_name}"
$DATASTORE_CLIENT -q "GRANT SELECT ON test, REMOTE ON *.* TO ${user_name}"
$DATASTORE_CLIENT -q "ALTER ROW POLICY ${row_policy_a} ON test TO ${user_name}"
...
$DATASTORE_CLIENT -q "CREATE USER ${user_name}"
$DATASTORE_CLIENT -q "GRANT SELECT ON test, REMOTE ON *.* TO ${user_name}"
$DATASTORE_CLIENT -q "ALTER ROW POLICY ${row_policy_b} ON test TO ${user_name}"
$DATASTORE_CLIENT --user ${user_name} -q "SELECT * FROM cluster('test_shard_localhost', currentDatabase(), test)"
$DATASTORE_CLIENT -q "DROP USER ${user_name}"
