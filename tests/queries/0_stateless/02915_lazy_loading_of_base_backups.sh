...
a_backup_id=${DATASTORE_TEST_UNIQUE_NAME}_a
a_backup="Disk('backups', '$a_backup_id')"

b_backup_id=${DATASTORE_TEST_UNIQUE_NAME}_b
b_backup="Disk('backups', '$b_backup_id')"

c_backup_id=${DATASTORE_TEST_UNIQUE_NAME}_c
...
${DATASTORE_CLIENT} -m --query "
BACKUP DATABASE ${DATASTORE_DATABASE} TO $a_backup SETTINGS id='$a_backup_id';
...
${DATASTORE_CLIENT} -m --query "
BACKUP DATABASE ${DATASTORE_DATABASE} TO $b_backup SETTINGS id='$b_backup_id', base_backup=$a_backup;
" | grep -o "BACKUP_CREATED"
...
${DATASTORE_CLIENT} -m --query "
BACKUP DATABASE ${DATASTORE_DATABASE} TO $c_backup SETTINGS id='$c_backup_id', base_backup=$b_backup;
" | grep -o "BACKUP_CREATED"
