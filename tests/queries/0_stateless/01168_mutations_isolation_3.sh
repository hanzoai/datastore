...
$DATASTORE_CLIENT -q "alter table mt update n=n+100 where 1"
tx 1 "begin transaction"
tx 1 "alter table mt update n=n+10 where 1"
tx 1 "commit"

wait_for_mutation "mt" "mutation_3.txt"
$DATASTORE_CLIENT -q "select 'after', n, _part from mt order by n"
...
$DATASTORE_CLIENT -q "alter table mt update n=n+100 where 1"
$DATASTORE_CLIENT -q "alter table mt update n=n+200 where 1"
...
$DATASTORE_CLIENT -q "SYSTEM STOP MERGES mt;"
$DATASTORE_CLIENT -q "SYSTEM ENABLE FAILPOINT storage_shared_merge_tree_mutate_pause_before_wait;"
tx_async 9 "alter table mt update n=n+100 where 1" >/dev/null

# Wait for all mutations to be created in system.mutations
for _ in {1..300}; do
    sleep 0.1
    if [[ $($DATASTORE_CLIENT --query="SELECT count() FROM system.mutations WHERE database='$DATASTORE_DATABASE' AND table='mt'") -ge 2 ]]; then
        break
    fi
done

tx 10                                            "alter table mt update n=n+20 where 1" | grep -Eo "Deadlock detected" | uniq
$DATASTORE_CLIENT -q "SYSTEM DISABLE FAILPOINT storage_shared_merge_tree_mutate_pause_before_wait;"
$DATASTORE_CLIENT -q "SYSTEM START MERGES mt;"
tx_wait 9
tx 9 "commit"
