...
$DATASTORE_CLIENT --query "CREATE TABLE ${DB}.t1 (x UInt32) ENGINE = Memory;"
$DATASTORE_CLIENT --query "CREATE TABLE ${DB}.t2 (x UInt32) ENGINE = Memory;"

# --- Case A: global SHOW_TABLES grant ---
$DATASTORE_CLIENT --query "DROP USER IF EXISTS ${DB}_global_user;"
$DATASTORE_CLIENT --query "CREATE USER ${DB}_global_user IDENTIFIED WITH no_password;"
$DATASTORE_CLIENT --query "GRANT SHOW TABLES ON *.* TO ${DB}_global_user;"

echo "=== Global grant: SELECT name (fast path) shows both tables ==="
$DATASTORE_CLIENT --user="${DB}_global_user" \
...
$DATASTORE_CLIENT --query "DROP USER IF EXISTS ${DB}_db_user;"
$DATASTORE_CLIENT --query "CREATE USER ${DB}_db_user IDENTIFIED WITH no_password;"
$DATASTORE_CLIENT --query "GRANT SHOW TABLES ON ${DB}.* TO ${DB}_db_user;"

echo "=== DB-level grant: SELECT name (fast path) shows both tables ==="
$DATASTORE_CLIENT --user="${DB}_db_user" \
    --query "SELECT name FROM system.tables WHERE database = '${DB}' ORDER BY name;"
...
$DATASTORE_CLIENT --query "DROP USER IF EXISTS ${DB}_table_user;"
$DATASTORE_CLIENT --query "CREATE USER ${DB}_table_user IDENTIFIED WITH no_password;"
$DATASTORE_CLIENT --query "GRANT SHOW TABLES ON ${DB}.t1 TO ${DB}_table_user;"

echo "=== Per-table grant: SELECT name (slow path) shows only t1 ==="
$DATASTORE_CLIENT --user="${DB}_table_user" \
    --query "SELECT name FROM system.tables WHERE database = '${DB}' ORDER BY name;"
