...
query_id="${DATASTORE_DATABASE}_parquet_group_group_prune_profile_$RANDOM"

# A parquet file with 10 row groups from 1 to 100, 10 rows per row group.
${DATASTORE_CLIENT} "${opts[@]}" --query_id="${query_id}" --query="SELECT * FROM file('${DATA_FILE_USER_PATH}', Parquet) WHERE col_int > 70 format Null" --format Null
