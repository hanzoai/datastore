...
if ! echo "" | nc -w 1 "${DATASTORE_HOST}" "${DATASTORE_PORT_SSH}" >/dev/null 2>&1; then
    echo "@@SKIP@@: SSH port ${DATASTORE_PORT_SSH} is not available, skipping test"
    exit 0
fi
...
SSH_USER_KEY="${DATASTORE_TMP}/ssh_user_ed25519_key_${DATASTORE_TEST_UNIQUE_NAME}"
cp "$SSH_USER_KEY_ORIG" "$SSH_USER_KEY"
chmod 600 "$SSH_USER_KEY"

SSH_STDERR="${DATASTORE_TMP}/ssh_stderr_${DATASTORE_TEST_UNIQUE_NAME}"

# Extract the base64 public key from the .pub file
SSH_USER_PUBKEY=$(awk '{print $2}' "$SSH_USER_KEY_ORIG.pub")

SSH_USER="ssh_user_${DATASTORE_TEST_UNIQUE_NAME}"
...
SSH_OPTS=(
    ...
    -i "$SSH_USER_KEY"
    -p "${DATASTORE_PORT_SSH}"
)

# Create a user with SSH key authentication
${DATASTORE_CLIENT} --query "DROP USER IF EXISTS ${SSH_USER}"
${DATASTORE_CLIENT} --query "CREATE USER ${SSH_USER} IDENTIFIED WITH ssh_key BY KEY '${SSH_USER_PUBKEY}' TYPE 'ssh-ed25519'"
${DATASTORE_CLIENT} --query "GRANT ALL ON ${DATASTORE_DATABASE}.* TO ${SSH_USER}"

# Run queries via SSH, capturing stderr for diagnostics on failure
ssh "${SSH_OPTS[@]}" "${SSH_USER}@${DATASTORE_HOST}" "SELECT 1" 2>"$SSH_STDERR" || cat "$SSH_STDERR" >&2

ssh "${SSH_OPTS[@]}" "${SSH_USER}@${DATASTORE_HOST}" "SELECT currentUser() = '${SSH_USER}'" 2>"$SSH_STDERR" || cat "$SSH_STDERR" >&2
