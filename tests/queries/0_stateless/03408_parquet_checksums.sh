...
F="${DATASTORE_TMP}/03408_parquet_checksums.parquet"

corrupt_file() {
    # Get column chunk offset and length.
    r=`${DATASTORE_LOCAL} -q "select row_groups[1].file_offset, row_groups[1].total_compressed_size from file('$F', ParquetMetadata)"`
