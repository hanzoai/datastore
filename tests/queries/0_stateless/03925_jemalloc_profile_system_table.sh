...
${DATASTORE_CLIENT} --jemalloc_enable_profiler=1 -q "SELECT number FROM numbers(1000000) ORDER BY number FORMAT Null";
...
${DATASTORE_CLIENT} -q "SELECT count() > 0 FROM system.jemalloc_profile_text SETTINGS jemalloc_profile_text_output_format = 'raw'";

echo "Testing symbolized format:"
${DATASTORE_CLIENT} -q "SELECT count() > 0 FROM system.jemalloc_profile_text WHERE line LIKE '%DB::Server::main%' SETTINGS jemalloc_profile_text_output_format = 'symbolized', jemalloc_profile_text_symbolize_with_inline = 0";
