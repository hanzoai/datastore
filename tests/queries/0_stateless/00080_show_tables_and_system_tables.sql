...
CREATE TABLE {DATASTORE_DATABASE:Identifier}.A (A UInt8) ENGINE = TinyLog;
CREATE TABLE {DATASTORE_DATABASE:Identifier}.B (A UInt8) ENGINE = TinyLog;
...
CREATE TABLE {DATASTORE_DATABASE:Identifier}.test_log(id UInt64) ENGINE = Log;
CREATE MATERIALIZED VIEW {DATASTORE_DATABASE:Identifier}.test_materialized ENGINE = Log AS SELECT * FROM {DATASTORE_DATABASE:Identifier}.test_log;
...
CREATE DATABASE {DATASTORE_DATABASE:Identifier} ENGINE = Memory;
CREATE TABLE {DATASTORE_DATABASE:Identifier}.A (A UInt8) ENGINE = Null;

SELECT sum(ignore(*, metadata_modification_time, engine_full, create_table_query)) FROM system.tables WHERE database = '{DATASTORE_DATABASE:String}';
