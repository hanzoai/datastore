...
DATASTORE_CLIENT="`echo "$DATASTORE_CLIENT --allow_materialized_view_with_bad_select=0 --session_timezone Etc/UTC"`"
...
$DATASTORE_CLIENT -q "
    ...
    drop table src;"
while [ "`$DATASTORE_CLIENT -q "select exception == '' from refreshes -- $LINENO" | xargs`" != '0' ]
do
    sleep 0.5
done
...
$DATASTORE_CLIENT -q "
    ...
    system stop view e;"
while [ "`$DATASTORE_CLIENT -q "select status from refreshes -- $LINENO" | xargs`" != 'Disabled' ]
do
    sleep 0.5
done
...
$DATASTORE_CLIENT -q "
    ...
    system start view e;"
while [ "`$DATASTORE_CLIENT -q "select status from refreshes -- $LINENO" | xargs`" != 'Running' ]
do
    sleep 0.5
done
# Rename.
...
while [ "`$DATASTORE_CLIENT -q "select status from refreshes -- $LINENO" | xargs`" != 'Scheduled' ]
do
    sleep 0.5
done
...
while [ "`$DATASTORE_CLIENT -q "select status from refreshes where view = 'f' -- $LINENO" | xargs`" != 'Running' ]
do
    sleep 0.5
done
# Drop.
...
while [ "`$DATASTORE_CLIENT -q "select next_refresh_time > '2049-01-01' from refreshes -- $LINENO" | xargs`" != '1' ]
do
    sleep 0.5
done
$DATASTORE_CLIENT -q "
...
$DATASTORE_CLIENT -q "
    ...
    system wait view h;
    system stop view h;"
while [ "`$DATASTORE_CLIENT -q "select status from refreshes where view = 'h' -- $LINENO" | xargs`" != 'Disabled' ]
do
    sleep 0.5
done
$DATASTORE_CLIENT -q "
    ...
    system start view h;"
while [ "`$DATASTORE_CLIENT -q "select count() from dest -- $LINENO" | xargs`" != '2' ]
do
    sleep 0.5
done
...
$DATASTORE_CLIENT -q "system stop view h;"
while [ "`$DATASTORE_CLIENT -q "select status from refreshes where view = 'h' -- $LINENO" | xargs`" != 'Disabled' ]
do
    sleep 0.5
done
...
$DATASTORE_CLIENT -q "system wait view h2; -- { serverError REFRESH_FAILED }"
$DATASTORE_CLIENT -q "
    ...
    drop table src2;"
while [ "`$DATASTORE_CLIENT -nq "select status, retry from refreshes where view = 'h2' -- $LINENO" | xargs`" != 'Scheduled 0' ]
do
    sleep 0.5
done
$DATASTORE_CLIENT -q "
...
while [ "`$DATASTORE_CLIENT -q "select sum(last_success_time is null) from refreshes -- $LINENO" | xargs`" == '2' ]
do
    sleep 0.5
done
$DATASTORE_CLIENT -q "
...
$DATASTORE_CLIENT --create_table_empty_primary_key_by_default 0 -q "
