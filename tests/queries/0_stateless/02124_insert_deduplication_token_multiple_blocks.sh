...
$DATASTORE_CLIENT --query="DROP TABLE IF EXISTS block_dedup_token SYNC"
$DATASTORE_CLIENT --query="CREATE TABLE block_dedup_token (id Int32) ENGINE=MergeTree() ORDER BY id SETTINGS non_replicated_deduplication_window=0xFFFFFFFF;"
$DATASTORE_CLIENT --query="SYSTEM STOP MERGES block_dedup_token;"

$DATASTORE_CLIENT --query="SELECT 'insert 2 blocks with dedup token, 1 row per block'"
DEDUP_TOKEN='dedup1'
echo 'INSERT INTO block_dedup_token VALUES (1), (2)' | ${DATASTORE_CURL} -sS "${DATASTORE_URL}&$INSERT_BLOCK_SETTINGS&insert_deduplication_token='$DEDUP_TOKEN'&query=" --data-binary @-
...
echo 'INSERT INTO block_dedup_token VALUES (1), (2), (3), (4)' | ${DATASTORE_CURL} -sS "${DATASTORE_URL}&$INSERT_BLOCK_SETTINGS&insert_deduplication_token='$DEDUP_TOKEN'&query=" --data-binary @-
$DATASTORE_CLIENT --query "$QUERY_COUNT_ORIGIN_BLOCKS;$QUERY_SELECT_FROM_TABLE_ORDERED"
...
$DATASTORE_CLIENT --query="DROP TABLE block_dedup_token SYNC"
