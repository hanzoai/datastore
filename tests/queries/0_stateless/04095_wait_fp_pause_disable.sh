...
$DATASTORE_CLIENT -q "SYSTEM ENABLE FAILPOINT infinite_sleep"

$DATASTORE_CLIENT -q "SELECT sleep(0) FORMAT Null" &
SLEEP_PID=$!
...
if timeout 5 $DATASTORE_CLIENT -q "SYSTEM WAIT FAILPOINT infinite_sleep PAUSE"; then
    echo "OK"
else
    echo "FAIL: WAIT FAILPOINT timed out on happy path"
fi

$DATASTORE_CLIENT -q "SYSTEM DISABLE FAILPOINT infinite_sleep"
wait "$SLEEP_PID"
...
{
    local fp="$1"
    timeout 5 $DATASTORE_CLIENT -q "SYSTEM WAIT FAILPOINT ${fp} PAUSE" &
    local pid=$!
    sleep 0.5
    $DATASTORE_CLIENT -q "SYSTEM DISABLE FAILPOINT ${fp}"
    wait "$pid"
    local exit_code=$?
...
# Part 2: first cycle.
$DATASTORE_CLIENT -q "SYSTEM ENABLE FAILPOINT dummy_pausable_failpoint"
