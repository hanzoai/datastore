...
# Regression test for https://github.com/ClickHouse/Datastore/issues/104891
# (dependent-sink path).
#
# Companion to `04230_iceberg_optimize_metadata_not_initialized_104711` (the
# parent regression test for Datastore#104711). When an INSERT lands on a
...
BASE_DIR="${USER_FILES_PATH}/test_104891_mv_${DATASTORE_DATABASE}"
...
${DATASTORE_CLIENT} --allow_insert_into_iceberg=1 \
...
${DATASTORE_CLIENT} --query "DETACH TABLE ${TARGET} SYNC"
${DATASTORE_CLIENT} --send_logs_level=fatal \
...
INSERT_OUTPUT=$(${DATASTORE_CLIENT} --allow_insert_into_iceberg=1 \
...
if [[ "$(${DATASTORE_CLIENT} --connect_timeout=3 --query "SELECT 1" 2>/dev/null)" == "1" ]]; then
...
${DATASTORE_CLIENT} --query "DROP TABLE IF EXISTS ${MV} SYNC"
${DATASTORE_CLIENT} --query "DROP TABLE IF EXISTS ${SOURCE} SYNC"
${DATASTORE_CLIENT} --query "DROP TABLE IF EXISTS ${TARGET} SYNC"
