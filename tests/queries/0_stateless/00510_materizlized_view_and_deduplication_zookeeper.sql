...
CREATE TABLE with_deduplication(x UInt32)
    ENGINE ReplicatedMergeTree('/datastore/tables/{database}/test_00510/with_deduplication', 'r1') ORDER BY x;
CREATE TABLE without_deduplication(x UInt32)
    ENGINE ReplicatedMergeTree('/datastore/tables/{database}/test_00510/without_deduplication', 'r1') ORDER BY x SETTINGS replicated_deduplication_window = 0, replicated_deduplication_window_for_async_inserts = 0;

CREATE MATERIALIZED VIEW with_deduplication_mv UUID '00000510-1000-4000-8000-000000000001'
    ENGINE = ReplicatedAggregatingMergeTree('/datastore/tables/{database}/test_00510/with_deduplication_mv', 'r1') ORDER BY dummy
...
CREATE MATERIALIZED VIEW without_deduplication_mv UUID '00000510-1000-4000-8000-000000000002'
    ENGINE = ReplicatedAggregatingMergeTree('/datastore/tables/{database}/test_00510/without_deduplication_mv', 'r1') ORDER BY dummy
