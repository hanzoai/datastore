...
# Regression test for https://github.com/ClickHouse/Datastore/issues/104509 on the Change Data
...
#
# Written as a `.sh` test through `datastore-local`: on the unfixed binary the process aborts, so
# stdout is empty and mismatches the reference (clean FAIL); on the fixed binary the guarded panic
# is reported and grep matches (PASS). Running in `datastore-local` keeps a crash on the unfixed
...
$DATASTORE_LOCAL --query "SELECT * FROM deltaLake('https://datastore-public-datasets.s3.amazonaws.com/delta_lake/hits/', '\0', 'x') SETTINGS allow_experimental_delta_kernel_rs = 1, delta_lake_snapshot_start_version = 0" 2>&1 \
