...
" | $DATASTORE_LOCAL --table test --input-format Native -q "select d, dynamicType(d), isDynamicElementInSharedData(d) from test"
...
select * from test format Native settings output_format_native_use_flattened_dynamic_and_json_serialization=1;
" | $DATASTORE_LOCAL --table test --input-format Native -q "select json, JSONDynamicPathsWithTypes(json), JSONSharedDataPathsWithTypes(json) from test settings output_format_json_quote_64bit_integers=0"
...
select * from test format Native settings output_format_native_use_flattened_dynamic_and_json_serialization=1;
" | $DATASTORE_LOCAL --table test --input-format Native -q "select json, JSONDynamicPathsWithTypes(json.a[][1]), JSONSharedDataPathsWithTypes(json.a[][1]) from test settings output_format_json_quote_64bit_integers=0"
