...
${DATASTORE_CLIENT} "${opts[@]}" -q "SELECT groupArray(repeat('a', if(_shard_num == 2, 100000, 1))), number%100000 k from remote('127.{2,3}', system.numbers) GROUP BY k LIMIT 10e6" |& {
    # the query should fail earlier on 127.3 and 127.2 should not even go to the memory limit exceeded error.
    grep -F -q "DB::Exception: Received from 127.3:${DATASTORE_PORT_TCP}. DB::Exception: Query memory limit exceeded:"
