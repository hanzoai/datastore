...
${CLICKHOUSE_CURL} -sS -D - "${CLICKHOUSE_URL}" --data-binary "SELECT 1" | grep -v "Access-Control-Expose-Headers" | grep "X-Datastore-Exception-Tag" | head -1 | cut -d':' -f1 | sed 's/$/:/g'

echo "Test 2: Verify exception tag is alphanumeric"
TAG=$(${CLICKHOUSE_CURL} -sS -D - "${CLICKHOUSE_URL}" --data-binary "SELECT 1" | grep -v "Access-Control-Expose-Headers" | grep "X-Datastore-Exception-Tag" | cut -d':' -f2 | tr -d ' \r\n')
