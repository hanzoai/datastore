...
echo "Test 1: Check that X-Datastore-Exception-Tag header is present"
${DATASTORE_CURL} -sS -D - "${DATASTORE_URL}" --data-binary "SELECT 1" | grep -v "Access-Control-Expose-Headers" | grep "X-Datastore-Exception-Tag" | head -1 | cut -d':' -f1 | sed 's/$/:/g'

echo "Test 2: Verify exception tag is alphanumeric"
TAG=$(${DATASTORE_CURL} -sS -D - "${DATASTORE_URL}" --data-binary "SELECT 1" | grep -v "Access-Control-Expose-Headers" | grep "X-Datastore-Exception-Tag" | cut -d':' -f2 | tr -d ' \r\n')
...
RESPONSE=$(${DATASTORE_CURL} -sS -D - "${DATASTORE_URL}&$SETTINGS" --data-binary "SELECT number, throwIf(number=10, 'there is a exception') FROM system.numbers" 2>&1 || true)
TAG=$(echo "$RESPONSE" | grep -v "Access-Control-Expose-Headers" | grep "X-Datastore-Exception-Tag" | cut -d':' -f2 | tr -d ' \r\n')
...
RESPONSE=$(${DATASTORE_CURL} -sS -D - "${DATASTORE_URL}&$SETTINGS" --data-binary "SELECT number, throwIf(number=10, '${LONG_MSG}') FROM system.numbers" 2>&1 || true)
...
${DATASTORE_CURL} -sS --raw "${DATASTORE_URL}&$SETTINGS" --data-binary "SELECT throwIf(number=3, 'exception') FROM system.numbers" 2>&1 | grep -Fq 'transfer closed with outstanding read data remaining' && echo "No final zero chunk on exception: OK"

echo "Test 7: Verify no final zero chunk on exception with gzip compression"
# Same test but with gzip compression enabled
${DATASTORE_CURL} -H "Accept-Encoding: gzip" "${DATASTORE_URL}&$SETTINGS" --compressed -v --data-binary "SELECT randomFixedString(1000), throwIf(number=3000, 'exception') FROM system.numbers" 2>&1 | grep -Faq 'transfer closed with outstanding read data remaining' && echo 'No final zero chunk on exception with compression: OK'
...
${DATASTORE_CURL} -sS --raw -H "Accept-Encoding: gzip" "${DATASTORE_URL}&$SETTINGS" --compressed -v --data-binary "SELECT number FROM system.numbers LIMIT 3" 2>&1 | grep -Fvq 'transfer closed with outstanding read data remaining' && echo 'Final zero chunk present on success: OK'
