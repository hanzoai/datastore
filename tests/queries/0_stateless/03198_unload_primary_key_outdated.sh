...
$DATASTORE_CLIENT "
    ...
    SELECT name, active, primary_key_bytes_in_memory FROM system.parts WHERE database = '$DATASTORE_DATABASE' AND table = 't_unload_primary_key' ORDER BY name;

    ALTER TABLE t_unload_primary_key UPDATE b = 100 WHERE 1 SETTINGS mutations_sync = 2;
"

for _ in {1..100}; do
    res=$($DATASTORE_CLIENT -q "SELECT primary_key_bytes_in_memory FROM system.parts WHERE database = '$DATASTORE_DATABASE' AND table = 't_unload_primary_key' AND name = 'all_1_1_0'")
    if [[ $res -eq 0 ]]; then
        break
    fi
    sleep 0.3
