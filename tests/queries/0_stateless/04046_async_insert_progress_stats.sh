...
# https://github.com/ClickHouse/Datastore/issues/99758

CURDIR=$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)
# shellcheck source=../shell_config.sh
. "$CURDIR"/../shell_config.sh

${DATASTORE_CLIENT} --query "DROP TABLE IF EXISTS test_async_insert_progress"
${DATASTORE_CLIENT} --query "CREATE TABLE test_async_insert_progress (x String) ENGINE = MergeTree ORDER BY x"

# Test 1: TCP protocol (datastore-client uses TCP)
query_tcp_values_id="ASYNC_INSERT_TCP_VALUES_$RANDOM$RANDOM"
${DATASTORE_CLIENT} --progress err --query_id="$query_tcp_values_id" --async_insert 1 --wait_for_async_insert 1 --query \
...
echo -e "one\n two\n three" | ${DATASTORE_CLIENT} --progress err --query_id="$query_tcp_data_id" --async_insert 1 --wait_for_async_insert 1 --query \
...
${DATASTORE_CURL} -vsS "${DATASTORE_URL}&async_insert=1&wait_for_async_insert=1&query_id=$query_http_id" \
    -d "INSERT INTO test_async_insert_progress VALUES ('four'), ('five'), ('six')" 2>&1 \
    | grep "X-Datastore-Summary" | grep -v "Access-Control-Expose-Headers" | sed 's/,\"elapsed_ns[^}]*//' | sed 's/,\"memory_usage[^}]*//'
...
${DATASTORE_CLIENT} --query "SELECT 'total_rows', count() FROM test_async_insert_progress"

${DATASTORE_CLIENT} --query "DROP TABLE test_async_insert_progress"
