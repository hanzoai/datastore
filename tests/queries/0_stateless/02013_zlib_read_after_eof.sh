...
$DATASTORE_CLIENT -q "DROP TABLE IF EXISTS graphite;"

$DATASTORE_CLIENT -q "CREATE TABLE graphite(\`Path\` String, \`Value\` Float64, \`Time\` UInt32, \`Date\` Date, \`Timestamp\` UInt32) \
...
$DATASTORE_CLIENT -q "SELECT count() FROM graphite;"

$DATASTORE_CLIENT -q "drop table graphite;"
