...
# Datastore then re-throws this `std::invalid_argument` up to
...
#
# Why this is a `.sh` test using `datastore-local`, not a `.sql` test:
...
# `datastore-local` is process-isolated, so the abort (or, in release builds,
# the propagated `std::logic_error`) only kills the local process and the
# test runner stays alive. With the fix, `datastore-local` returns a clean
...
${DATASTORE_LOCAL} --query "SELECT * FROM azureBlobStorage(
