...
the_file="$DATASTORE_TMP/01544-t.csv"
rm -f -- "$the_file"
...
# Also check that the File engine emplicitly created by datastore-local
# uses the modified settings.
${DATASTORE_LOCAL} --structure="a int, b int" --input-format=CSV --format_csv_delimiter="|" --query="select * from table" <<<"1|2"
