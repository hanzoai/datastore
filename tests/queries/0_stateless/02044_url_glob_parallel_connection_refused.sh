...
# proper fix should be implemented in https://github.com/ClickHouse/Datastore/issues/66656
while [[ $i -lt $retries ]]; do
    timeout 5s ${DATASTORE_CLIENT} --max_threads 10 --http_max_tries 2 --query "SELECT * FROM url('http://128.0.0.{1..10}:${DATASTORE_PORT_HTTP}/?query=SELECT+sleep(1)', TSV, 'x UInt8')" --format Null 2>/dev/null
    ((++i))
done
