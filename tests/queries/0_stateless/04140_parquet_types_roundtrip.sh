...
OUT="${DATASTORE_TMP:-/tmp}/${DATASTORE_TEST_UNIQUE_NAME}.parquet"
...
${DATASTORE_LOCAL} --query "SELECT count() FROM file('${OUT}', 'Parquet')"
${DATASTORE_LOCAL} --query "DESCRIBE TABLE file('${OUT}', 'Parquet')" | sort

echo '--- full readback ---'
${DATASTORE_LOCAL} --query "SELECT * FROM file('${OUT}', 'Parquet') ORDER BY i32 FORMAT Vertical"
...
for codec in NONE SNAPPY ZSTD LZ4 LZ4_HC GZIP BROTLI; do
    echo "--- codec: ${codec} ---"
    ${DATASTORE_LOCAL} --query "SELECT number AS n FROM numbers(100) FORMAT Parquet" \
        --output_format_parquet_compression_method="$(echo ${codec} | tr A-Z a-z)" > "${OUT}" 2>&1 || { echo "skip ${codec}"; continue; }
    ${DATASTORE_LOCAL} --query "SELECT count(), sum(n) FROM file('${OUT}', 'Parquet')"
done
...
${DATASTORE_LOCAL} --print-profile-events --query "
...
${DATASTORE_LOCAL} --query "SELECT count() FROM file('${OUT}', 'Parquet') WHERE n BETWEEN 100 AND 200"
${DATASTORE_LOCAL} --query "SELECT min(n), max(n) FROM file('${OUT}', 'Parquet') WHERE n < 50"

echo '--- filter pushdown: result identical with pushdown off ---'
${DATASTORE_LOCAL} --query "SELECT count() FROM file('${OUT}', 'Parquet') WHERE n BETWEEN 100 AND 200 SETTINGS input_format_parquet_filter_push_down = 0"
...
" > "${OUT}"
echo '--- column pruning ---'
${DATASTORE_LOCAL} --query "SELECT sum(b) FROM file('${OUT}', 'Parquet')"
${DATASTORE_LOCAL} --query "SELECT sum(a), sum(c) FROM file('${OUT}', 'Parquet')"
...
ONE=$(${DATASTORE_LOCAL} --print-profile-events --query "SELECT sum(b) FROM file('${OUT}', 'Parquet')" 2>&1 \
    | awk '/ParquetDecodingTasks:/ { sum += $(NF-1) } END { print sum+0 }')
ALL=$(${DATASTORE_LOCAL} --print-profile-events --query "SELECT * FROM file('${OUT}', 'Parquet') FORMAT Null" 2>&1 \
    | awk '/ParquetDecodingTasks:/ { sum += $(NF-1) } END { print sum+0 }')
...
${DATASTORE_LOCAL} --query "SELECT countIf(n IS NULL), countIf(s IS NULL) FROM file('${OUT}', 'Parquet')"

# -----------------------------------------------------------------------------
# 6. Schema inference.
# -----------------------------------------------------------------------------
${DATASTORE_LOCAL} --query "SELECT 1::Int32 AS a, 'x' AS s, [1, 2]::Array(Int32) AS arr FORMAT Parquet" > "${OUT}"
...
# -----------------------------------------------------------------------------
${DATASTORE_LOCAL} --query "SELECT 1::Int64 AS n WHERE 0 FORMAT Parquet" > "${OUT}"
echo '--- empty parquet: count ---'
${DATASTORE_LOCAL} --query "SELECT count() FROM file('${OUT}', 'Parquet', 'n Int64')"
