...
CH_URL="${DATASTORE_PORT_HTTP_PROTO}://${DATASTORE_HOST}:${DATASTORE_PORT_HTTP}/"

query="SELECT 'OK'"

echo "buffer_size=WRONG -- should fail"
${DATASTORE_CURL} -sS "${CH_URL}?buffer_size=WRONG" -d "$query" | grep -o "CANNOT_PARSE_INPUT_ASSERTION_FAILED"
echo "buffer_size=WRONG&buffer_size=0 -- WRONG value is overrided, last value is used"
${DATASTORE_CURL} -sS "${CH_URL}?buffer_size=WRONG&buffer_size=0" -d "$query"
...
${DATASTORE_CURL} -qsS "${CH_URL}&wait_end_of_query=0" -d "$query" | sed "s/(version .*)//" | sed 's/DB::Exception://g'
...
${DATASTORE_CURL} -qsS "${CH_URL}&wait_end_of_query=0&http_wait_end_of_query=1" -d "$query" | sed "s/(version .*)//" | sed 's/DB::Exception://g'
