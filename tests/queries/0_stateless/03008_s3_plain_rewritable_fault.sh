...
{
    local table=$1
    local database=${2:-$DATASTORE_DATABASE}
    local timeout=${3:-20}
    ...
    while [[ timeout -gt 0 ]]
    do
        res=$(${DATASTORE_CLIENT} --query="$query")
        [[ $res -eq 1 ]] && return 0
...
${DATASTORE_CLIENT} --query "DROP TABLE IF EXISTS test_s3_mt_fault"
...
active_count=$(${DATASTORE_CLIENT} --query "
select countIf(active) from system.parts
where database = '${DATASTORE_DATABASE}' and table = 'test_s3_mt_fault'")
...
${DATASTORE_CLIENT} --query "SYSTEM ENABLE FAILPOINT storage_merge_tree_background_clear_old_parts_pause;"
...
inactive_count=$(${DATASTORE_CLIENT} --query "
select count() from system.parts
where database = '${DATASTORE_DATABASE}' and table = 'test_s3_mt_fault' and $STATE_CONDITION")
...
${DATASTORE_CLIENT} --query "SYSTEM ENABLE FAILPOINT plain_object_storage_write_fail_on_directory_move;"
${DATASTORE_CLIENT} --query "SYSTEM DISABLE FAILPOINT storage_merge_tree_background_clear_old_parts_pause;"

wait_for_part_remove_rolled_back test_s3_mt_fault

inactive_count=$(${DATASTORE_CLIENT} --query "
select count() from system.parts
where database = '${DATASTORE_DATABASE}' and table = 'test_s3_mt_fault' and $STATE_CONDITION and $REMOVAL_STATE_CONDITION")
if [[ $inactive_count -eq 0 ]]
then
...
${DATASTORE_CLIENT} --query "SYSTEM DISABLE FAILPOINT plain_object_storage_write_fail_on_directory_move;"
...
${DATASTORE_CLIENT} --query "DROP TABLE test_s3_mt_fault SYNC" 2>&1 | grep -v 'Removing temporary directory' ||:
