...
${DATASTORE_CURL} -s -D "$TMP_DIR/headers" -o "$TMP_DIR/body" "$URL" --data-binary "SELECT 1" || true
...
GOOD_URL="${DATASTORE_PORT_HTTP_PROTO}://default:@${DATASTORE_HOST}:${DATASTORE_PORT_HTTP}/"
${DATASTORE_CURL} -s -D "$TMP_DIR/headers2" -o "$TMP_DIR/body2" "$GOOD_URL" --data-binary "SELECT * FROM no_such_table" || true
