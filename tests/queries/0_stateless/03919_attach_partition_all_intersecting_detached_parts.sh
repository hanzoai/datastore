...
$DATASTORE_CLIENT -q "DROP TABLE IF EXISTS t_intersect"

$DATASTORE_CLIENT -q "
    CREATE TABLE t_intersect (x UInt64)
    ENGINE = ReplicatedMergeTree('/datastore/tables/{database}/t_intersect', 'r1')
    ORDER BY x
"
...
$DATASTORE_CLIENT -q "SYSTEM STOP MERGES t_intersect"
for i in $(seq 1 10); do
    $DATASTORE_CLIENT -q "INSERT INTO t_intersect VALUES ($i)"
done
$DATASTORE_CLIENT -q "SYSTEM START MERGES t_intersect"
$DATASTORE_CLIENT -q "OPTIMIZE TABLE t_intersect FINAL"
$DATASTORE_CLIENT -q "SYSTEM SYNC REPLICA t_intersect"

# Get the ZK path before detaching.
ZK_PATH=$($DATASTORE_CLIENT -q "SELECT zookeeper_path FROM system.replicas WHERE database = currentDatabase() AND table = 't_intersect'")

# Detach the table first — this releases the ZK session and ephemeral nodes,
# allowing us to drop the replica metadata from ZK.
$DATASTORE_CLIENT -q "DETACH TABLE t_intersect"

# Drop the replica's ZK metadata while the table is detached.
# This bypasses the "can't drop local replica" check.
$DATASTORE_CLIENT -q "SYSTEM DROP REPLICA 'r1' FROM ZKPATH '$ZK_PATH'"

# Re-attach — without the ZK replica node, the table enters readonly mode.
$DATASTORE_CLIENT -q "ATTACH TABLE t_intersect"

# Wait for the replica to become readonly.
for _ in $(seq 1 100); do
    is_readonly=$($DATASTORE_CLIENT -q "SELECT is_readonly FROM system.replicas WHERE database = currentDatabase() AND table = 't_intersect'")
...
$DATASTORE_CLIENT -q "SYSTEM RESTORE REPLICA t_intersect"
$DATASTORE_CLIENT -q "SYSTEM SYNC REPLICA t_intersect"
...
$DATASTORE_CLIENT -q "ALTER TABLE t_intersect ATTACH PARTITION ALL"

# Verify we can read data.
$DATASTORE_CLIENT -q "SELECT count() > 0 FROM t_intersect"

$DATASTORE_CLIENT -q "DROP TABLE t_intersect"
