...
SELECT hex(c0) FROM file(currentDatabase() || '04006_false.parquet', 'Parquet', 'c0 FixedString(5)');

-- FixedString(4) should accept "true" (4 chars)
SELECT hex(c0) FROM file(currentDatabase() || '04006_true.parquet', 'Parquet', 'c0 FixedString(4)');

-- FixedString(1) should reject "false" (too large)
SELECT c0 FROM file(currentDatabase() || '04006_false.parquet', 'Parquet', 'c0 FixedString(1)'); -- {serverError TOO_LARGE_STRING_SIZE}

-- FixedString(3) should reject "true" (too large)
SELECT c0 FROM file(currentDatabase() || '04006_true.parquet', 'Parquet', 'c0 FixedString(3)'); -- {serverError TOO_LARGE_STRING_SIZE}
