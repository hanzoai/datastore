...
${DATASTORE_CURL} -sS "${DATASTORE_URL}&compatibility=24.12" -d "$QUERY_SKIP"

echo "-- explicit =1 with compatibility=24.12 (compat first in URL)"
${DATASTORE_CURL} -sS "${DATASTORE_URL}&compatibility=24.12&use_skip_indexes_if_final=1" -d "$QUERY_SKIP"

echo "-- explicit =1 with compatibility=24.12 (setting first in URL)"
${DATASTORE_CURL} -sS "${DATASTORE_URL}&use_skip_indexes_if_final=1&compatibility=24.12" -d "$QUERY_SKIP"

echo "-- explicit =0 with compatibility=24.12 (not affected by bug)"
${DATASTORE_CURL} -sS "${DATASTORE_URL}&compatibility=24.12&use_skip_indexes_if_final=0" -d "$QUERY_SKIP"

echo "-- TCP: explicit =1 with compatibility=24.12"
${DATASTORE_CLIENT} --compatibility=24.12 --use_skip_indexes_if_final=1 \
    -q "$QUERY_SKIP"
