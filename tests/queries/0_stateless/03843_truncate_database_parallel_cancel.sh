...
DB="${DATASTORE_DATABASE}_trunc_parallel"
...
for i in $(seq 1 8); do
    ${DATASTORE_CLIENT} -q "INSERT INTO ${DB}.t1 SELECT number FROM numbers(10000)"
    ${DATASTORE_CLIENT} -q "INSERT INTO ${DB}.t2 SELECT number FROM numbers(10000)"
    ${DATASTORE_CLIENT} -q "INSERT INTO ${DB}.t3 SELECT number FROM numbers(10000)"
    ${DATASTORE_CLIENT} -q "INSERT INTO ${DB}.t4 SELECT number FROM numbers(10000)"
    ${DATASTORE_CLIENT} -q "INSERT INTO ${DB}.skip_me SELECT number FROM numbers(10000)"
done
...
${DATASTORE_CLIENT} -q "TRUNCATE TABLES FROM ${DB} LIKE 't%'"

# All matched tables must be empty.
${DATASTORE_CLIENT} -q "SELECT count() FROM ${DB}.t1"
${DATASTORE_CLIENT} -q "SELECT count() FROM ${DB}.t2"
${DATASTORE_CLIENT} -q "SELECT count() FROM ${DB}.t3"
${DATASTORE_CLIENT} -q "SELECT count() FROM ${DB}.t4"

# The unmatched table must still have its data.
${DATASTORE_CLIENT} -q "SELECT count() > 0 FROM ${DB}.skip_me"
