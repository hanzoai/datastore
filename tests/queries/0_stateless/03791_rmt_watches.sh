...
read -r event_time_timestamp query_duration_ms < <($DATASTORE_CLIENT -nm -q "
  system flush logs query_log;
  select toUnixTimestamp(event_time), query_duration_ms from system.query_log where event_date >= yesterday() AND event_time >= now() - 600 AND current_database = '$DATASTORE_DATABASE' and query_kind = 'Drop' and type = 'QueryFinish';
")
$DATASTORE_CLIENT -nm -q "
