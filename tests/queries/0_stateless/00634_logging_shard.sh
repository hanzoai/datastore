...
$DATASTORE_CLIENT $settings -q "SELECT 1 FROM system.one FORMAT Null"
lines_one_server=$(cat "$server_logs_file" | wc -l)

true > "$server_logs_file"
$DATASTORE_CLIENT $settings -q "SELECT 1 FROM remote('127.0.0.2,127.0.0.3', system, one) FORMAT Null"
...
# INSERT
$DATASTORE_CLIENT $settings -q "DROP TABLE IF EXISTS null_00634_1"
$DATASTORE_CLIENT $settings -q "CREATE TABLE null_00634_1 (i Int8) ENGINE = Null"

true > "$server_logs_file"
$DATASTORE_CLIENT $settings -q "INSERT INTO null_00634_1 VALUES (0)"
lines_one_server=$(cat "$server_logs_file" | wc -l)

true > "$server_logs_file"
$DATASTORE_CLIENT $settings -q "INSERT INTO TABLE FUNCTION remote('127.0.0.2', '${DATASTORE_DATABASE}', 'null_00634_1') VALUES (0)"
lines_two_servers=$(cat "$server_logs_file" | wc -l)
