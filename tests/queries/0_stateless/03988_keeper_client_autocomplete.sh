...
path="/test-keeper-autocomplete-$DATASTORE_DATABASE"
$DATASTORE_KEEPER_CLIENT -q "rmr '$path'" >& /dev/null

$DATASTORE_KEEPER_CLIENT -q "create '$path' 'x'"
$DATASTORE_KEEPER_CLIENT -q "create '$path/dir_node' 'x'"
$DATASTORE_KEEPER_CLIENT -q "create '$path/dir_node/child1' 'x'"
$DATASTORE_KEEPER_CLIENT -q "create '$path/dir_node/child2' 'x'"
$DATASTORE_KEEPER_CLIENT -q "create '$path/leaf_node' 'x'"
$DATASTORE_KEEPER_CLIENT -q "create '$path/unique_prefix_xyz' 'x'"

# Create a znode with a space in the name (using quoted path)
$DATASTORE_KEEPER_CLIENT -q "create '$path/space node' 'x'"
$DATASTORE_KEEPER_CLIENT -q "create '$path/space node/inner' 'x'"

# Create a znode with a double quote in the name
$DATASTORE_KEEPER_CLIENT -q "create '$path/has\"quote' 'dqval'"
$DATASTORE_KEEPER_CLIENT -q "create '$path/has\"quote/deep' 'dqdeep'"

# Create a znode with a unique prefix and an embedded double quote for completion tests
$DATASTORE_KEEPER_CLIENT -q "create '$path/dq\"node' 'dqn'"
$DATASTORE_KEEPER_CLIENT -q "create '$path/dq\"node/inner' 'dqni'"

# Test bare (unquoted) path parsing: create and get without any quoting
echo -n "bare_path_parse: "
$DATASTORE_KEEPER_CLIENT -q "create $path/bare_test test_value"
result=$($DATASTORE_KEEPER_CLIENT -q "get $path/bare_test")
if [ "$result" = "test_value" ]; then echo "OK"; else echo "FAIL ($result)"; fi

# Test quoted path parsing still works
echo -n "quoted_path_parse: "
$DATASTORE_KEEPER_CLIENT -q "create \"$path/quoted_test\" quoted_value"
result=$($DATASTORE_KEEPER_CLIENT -q "get \"$path/quoted_test\"")
...
stderr=$($DATASTORE_KEEPER_CLIENT -q "create $path/garbage_test val garbage" 2>&1 >/dev/null)
if ! echo "$stderr" | grep -q "Syntax error"; then
    echo "FAIL (no syntax error, stderr: $stderr)"
elif $DATASTORE_KEEPER_CLIENT -q "exists $path/garbage_test" 2>/dev/null | grep -q "1"; then
...
$DATASTORE_KEEPER_CLIENT -q "create $path/multi_a aa; create $path/multi_b bb"
result_a=$($DATASTORE_KEEPER_CLIENT -q "get $path/multi_a")
result_b=$($DATASTORE_KEEPER_CLIENT -q "get $path/multi_b")
...
result=$($DATASTORE_KEEPER_CLIENT -q "get $path/space\ node")
...
$DATASTORE_KEEPER_CLIENT -q "create $path/bs_node\\\\ bs_value"
# Use quoted path for get to avoid bare-path escaping issues
result=$($DATASTORE_KEEPER_CLIENT -q "get \"$path/bs_node\\\\\"")
...
$DATASTORE_KEEPER_CLIENT -q "create '$path/trailing_sp ' 'old'"
$DATASTORE_KEEPER_CLIENT -q "set $path/trailing_sp\  new_value"
result=$($DATASTORE_KEEPER_CLIENT -q "get '$path/trailing_sp '")
...
$DATASTORE_KEEPER_CLIENT -q "create '$path/x\\\\y' 'bsonly'"
# ls emits: x\\y  (backslash-escaped). Extract with grep -oF.
token=$($DATASTORE_KEEPER_CLIENT -q "ls '$path'" | grep -oF $'x\\\\y')
result=$($DATASTORE_KEEPER_CLIENT -q "get $path/$token")
...
$DATASTORE_KEEPER_CLIENT -q "create '${path}/has''quote d' 'x'"
...
token=$($DATASTORE_KEEPER_CLIENT -q "ls '$path'" | grep -oF $'\'has\\\'quote d\'')
result=$($DATASTORE_KEEPER_CLIENT -q "get $path/$token")
...
$DATASTORE_KEEPER_CLIENT -q "create '${path}/it''s' 'quoteval'"
# ls emits 'it\'s' — use grep -oF with the literal bytes via $'...' ANSI-C quoting
token=$($DATASTORE_KEEPER_CLIENT -q "ls '$path'" | grep -oF $'\'it\\\'s\'')
result=$($DATASTORE_KEEPER_CLIENT -q "get $path/$token")
...
result=$($DATASTORE_KEEPER_CLIENT -q "ls '$path'")
...
$DATASTORE_KEEPER_CLIENT -q "create '$path/a;b' 'semival'"
token=$($DATASTORE_KEEPER_CLIENT -q "ls '$path'" | grep -oF $'\'a;b\'')
result=$($DATASTORE_KEEPER_CLIENT -q "get $path/$token")
...
# '\t' inside single-quoted string is interpreted as a tab by Datastore's
# ParserStringLiteral (which handles C-style escape sequences, unlike standard SQL).
$DATASTORE_KEEPER_CLIENT -q "create '$path/a\tb' 'tabval'"
