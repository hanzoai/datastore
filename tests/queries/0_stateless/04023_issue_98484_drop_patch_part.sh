...
${DATASTORE_CLIENT} --query "INSERT INTO t_98484 VALUES (1, 'hello', 10)"
${DATASTORE_CLIENT} --query "INSERT INTO t_98484 VALUES (2, 'world', 20)"
${DATASTORE_CLIENT} --query "INSERT INTO t_98484 VALUES (3, 'test', 30)"

# Create patch parts via lightweight delete
${DATASTORE_CLIENT} --query "SET lightweight_delete_mode = 'lightweight_update_force'; DELETE FROM t_98484 WHERE c0 = 1"

# Wait for mutations to complete
for _ in $(seq 1 30); do
    result=$(${DATASTORE_CLIENT} --query "SELECT count() FROM system.parts WHERE database = currentDatabase() AND table = 't_98484' AND name LIKE 'patch-%' AND active = 1")
    if [ "$result" -ge 1 ]; then
        break
    fi
    sleep 0.5
...
${DATASTORE_CLIENT} --query "ALTER TABLE t_98484 ADD COLUMN c9 Nullable(Bool)"

# Get the first active patch part name
PATCH_PART=$(${DATASTORE_CLIENT} --query "
...
${DATASTORE_CLIENT} --query "ALTER TABLE t_98484 DROP PART '$PATCH_PART'" 2>&1

# Verify server is still alive
${DATASTORE_CLIENT} --query "SELECT 1" > /dev/null 2>&1
if [ $? -ne 0 ]; then
...
${DATASTORE_CLIENT} --query "DROP TABLE t_98484"
