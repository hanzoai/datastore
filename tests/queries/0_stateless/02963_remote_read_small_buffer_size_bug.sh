...
$DATASTORE_CLIENT -m --query_id "$query_id" --query "
WITH RANDOM_SET AS (
...
WHERE event_date >= yesterday() AND event_time >= now() - 600 AND current_database = '$DATASTORE_DATABASE' AND query_id = '$query_id' AND type = 'QueryFinish'
