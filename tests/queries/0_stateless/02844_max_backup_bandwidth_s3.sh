...
# and Datastore never reads/writes the data, so throttling cannot apply.
# Checking wall clock time for native copy is unreliable — S3 server-side copy latency is unpredictable.
#
query_id=$(random_str 10)
$DATASTORE_CLIENT "${client_opts[@]}" --query_id "$query_id" -q "backup table data to S3(s3_conn, 'backups/$DATASTORE_DATABASE/data/backup3') SETTINGS allow_s3_native_copy=0" --max_backup_bandwidth=1M > /dev/null
$DATASTORE_CLIENT "${client_opts[@]}" -m -q "
