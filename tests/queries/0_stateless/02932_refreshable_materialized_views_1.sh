...
DATASTORE_CLIENT="`echo "$DATASTORE_CLIENT" | sed 's/--session_timezone[= ][^ ]*//g'`"
DATASTORE_CLIENT="`echo "$DATASTORE_CLIENT --session_timezone Etc/UTC"`"

$DATASTORE_CLIENT -q "create view refreshes as select * from system.view_refreshes where database = '$DATASTORE_DATABASE' order by view"
...
while [ "`$DATASTORE_CLIENT -q "select last_success_time is null from refreshes -- $LINENO" | xargs`" != '0' ]
do
    sleep 0.5
done
start_time="`$DATASTORE_CLIENT -q "select reinterpret(now64(), 'Int64')"`"
# Check table contents.
$DATASTORE_CLIENT -q "select '<2: refreshed>', count(), sum(x=0), sum(x=1) from a"
# Wait for table contents to change.
res1="`$DATASTORE_CLIENT -q 'select * from a order by x format Values'`"
while :
do
    res2="`$DATASTORE_CLIENT -q 'select * from a order by x format Values -- $LINENO'`"
    [ "$res2" == "$res1" ] || break
    sleep 0.5
done
...
while :
do
    res3="`$DATASTORE_CLIENT -q 'select * from a order by x format Values -- $LINENO'`"
    [ "$res3" == "$res2" ] || break
    sleep 0.5
done
...
while [ "`$DATASTORE_CLIENT -q "select last_success_time, status from refreshes -- $LINENO" | xargs`" != '2052-02-03 04:05:06 Scheduled' ]
do
    sleep 0.5
done
$DATASTORE_CLIENT -q "
...
while [ "`$DATASTORE_CLIENT -q "select status from refreshes where view = 'b' -- $LINENO" | xargs`" != 'WaitingForDependencies' ]
do
    sleep 0.5
done
...
$DATASTORE_CLIENT -q "
    ...
    drop table src;
    system refresh view a;"
$DATASTORE_CLIENT -q "system wait view a; -- { serverError REFRESH_FAILED }"
$DATASTORE_CLIENT -q "
...
while [ "`$DATASTORE_CLIENT -q "select status from refreshes where view = 'b' -- $LINENO" | xargs`" != 'Scheduled' ]
do
    sleep 0.5
done
...
while [ "`$DATASTORE_CLIENT -q "select next_refresh_time from refreshes where view = 'b' -- $LINENO" | xargs`" != '2062-01-01 00:00:00' ]
do
    sleep 0.5
done
$DATASTORE_CLIENT -q "
...
while [ "`$DATASTORE_CLIENT -q "select status, last_refresh_time from refreshes where view = 'b' -- $LINENO" | xargs`" != 'Scheduled 2062-03-03 03:03:03' ]
do
    sleep 0.5
done
$DATASTORE_CLIENT -q "
