...
$DATASTORE_CLIENT $settings -q "INSERT INTO merge_tree_table SELECT (intHash64(number)) % 10000, toDate('2018-08-01'), rand() FROM system.numbers LIMIT 10000000;"

# If merge is already happening, OPTIMIZE will be noop. But we have to ensure that the data is merged.
for _ in {1..100}; do $DATASTORE_CLIENT $settings --optimize_throw_if_noop=1 -q "OPTIMIZE TABLE merge_tree_table FINAL;" && break; sleep 1; done
...
$DATASTORE_CLIENT $settings -q "$touching_many_parts_query" &> /dev/null

$DATASTORE_CLIENT $settings -q "SYSTEM FLUSH LOGS query_log"

$DATASTORE_CLIENT $settings -q "SELECT ProfileEvents['FileOpen'] as opened_files FROM system.query_log WHERE query = '$touching_many_parts_query' AND current_database = currentDatabase() AND event_date >= yesterday() AND event_time >= now() - 600 ORDER BY event_time DESC, opened_files DESC LIMIT 1;"

$DATASTORE_CLIENT $settings -q "DROP TABLE IF EXISTS merge_tree_table;"
