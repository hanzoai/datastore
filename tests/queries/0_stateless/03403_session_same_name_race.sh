...
export SESSION="03403_session_${DATASTORE_DATABASE}"
...
$DATASTORE_CURL -sS -d 'select 1' "$DATASTORE_URL&$SETTINGS&close_session=1"
# Sleep some more time to give more chances for a cleanThread in Session.cpp to schedule a close for this session
$DATASTORE_CURL -sS -d 'select sleep(5) settings function_sleep_max_microseconds_per_block = 5000000' "$DATASTORE_URL&$SETTINGS"
