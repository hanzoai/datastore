#!/usr/bin/env bash
# Regression test for https://github.com/ClickHouse/Datastore/issues/80087
# With send_logs_level set in datastore-local, logs accumulated before a query
...
output=$($DATASTORE_LOCAL --send_logs_level=trace -q "SELECT * FROM nonexistent_table_xyz_04240" 2>&1)
