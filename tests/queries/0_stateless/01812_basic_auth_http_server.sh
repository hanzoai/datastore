...
# In very old (e.g. 1.1.54385) versions of Datastore there was a bug in Poco HTTP library:
...
for i in {3950..4100}; do ${DATASTORE_CURL} --user default:12345 "${DATASTORE_URL}&query=SELECT+1"$(perl -e "print '+'x$i") | tr -d '\n' | grep -v -F 'password' ||:; done

# You can check that the bug exists in old version by running the old server in Docker:
# docker run --network host -it --rm datastore/datastore-server:1.1.54385
