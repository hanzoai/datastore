...
echo "First query"
$DATASTORE_CURL -v -sS "$DATASTORE_URL&query_id=${DATASTORE_DATABASE}_q1&use_query_cache=1&query_cache_ttl=600&query_cache_tag=${TAG}&query=SELECT+'Test+03711'" |& grep -o -P '< (Age|Expires):'

sleep 1 # needed for QueryCacheAgeSeconds below

echo "Second query"
$DATASTORE_CURL -v -sS "$DATASTORE_URL&query_id=${DATASTORE_DATABASE}_q2&use_query_cache=1&query_cache_ttl=600&query_cache_tag=${TAG}&query=SELECT+'Test+03711'" |& grep -o -P '< (Age|Expires):'
...
for _ in $(seq 1 60); do
    ...
    count=$($DATASTORE_CLIENT --query "SELECT count() FROM system.query_log WHERE current_database = currentDatabase() AND event_date >= yesterday() AND event_time >= now() - 600 AND type = 'QueryFinish' AND query_id IN ('${DATASTORE_DATABASE}_q1', '${DATASTORE_DATABASE}_q2') AND Settings['query_cache_tag'] = '$TAG'")
    [ "$count" -ge 2 ] && break
    sleep 0.5
done
...
WHERE
    ...
    AND query_id IN ('${DATASTORE_DATABASE}_q1', '${DATASTORE_DATABASE}_q2')
