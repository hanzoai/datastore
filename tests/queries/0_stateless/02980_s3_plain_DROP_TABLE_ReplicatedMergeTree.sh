...
# suppress output
$DATASTORE_CLIENT -q "backup table data_read to S3('http://localhost:11111/test/s3_plain/backups/$DATASTORE_DATABASE', 'test', 'testtest')" > /dev/null
...
path=$($DATASTORE_CLIENT -q "SELECT replace(data_paths[1], 's3_plain', '') FROM system.tables WHERE database = '$DATASTORE_DATABASE' AND table = 'data_read'")
...
datastore-disks -C "$config" --disk s3_plain_disk --query "list --recursive $path" | tail -n+2 | sed 's/all_[^_]*_[^_]*_0/all_X_X_X/g'
