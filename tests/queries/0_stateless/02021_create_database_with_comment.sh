...
{
    $DATASTORE_CLIENT --query="SHOW CREATE DATABASE ${DB_NAME};"
    $DATASTORE_CLIENT --query="SELECT 'comment=', comment FROM system.databases WHERE name='${DB_NAME}'"
    echo # just a newline
}
...
function test_db_comments()
{
    ...
    $DATASTORE_CLIENT --allow_deprecated_database_ordinary=1 -m <<EOF
...
# test_db_comments "Replicated('/datastore/$DATASTORE_TEST_ZOOKEEPER_PREFIX', '1') ORDER BY k"
