...
DATASTORE_CLIENT="${DATASTORE_CLIENT} --database_replicated_allow_explicit_uuid 3"
...
CREATE MATERIALIZED VIEW mv UUID '$uuid' ENGINE = Log AS SELECT sleepEachRow(0.0000002) FROM tab_00738;" | ${DATASTORE_CLIENT} -n

${DATASTORE_CLIENT} --query_id insert_$DATASTORE_DATABASE --query "INSERT INTO tab_00738 SELECT number FROM numbers(10000000)" &

function drop_inner_id()
{
    ${DATASTORE_CLIENT} --query "DROP TABLE \`.inner_id.$uuid\`" -n
}

function wait_for_query_to_start()
{
    while [[ $(${DATASTORE_CLIENT} --query "SELECT count() FROM system.processes WHERE query_id = 'insert_$DATASTORE_DATABASE'") == 0 ]]; do sleep 0.001; done
    ...
    if [[ $(${DATASTORE_CLIENT} --query "SELECT count() FROM system.processes WHERE query_id = 'insert_$DATASTORE_DATABASE'") == 0 ]]; then
        return 2
    fi
}
...
DROP TABLE IF EXISTS mv;" | ${DATASTORE_CLIENT} -n
}
