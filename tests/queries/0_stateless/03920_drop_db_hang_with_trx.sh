...
DB="${DATASTORE_DATABASE}_dropdb_hang"

function cleanup()
{
    ${DATASTORE_CLIENT} -q "SYSTEM DISABLE FAILPOINT drop_database_before_exclusive_ddl_lock" 2>/dev/null ||:
...
${DATASTORE_CLIENT} -q "SYSTEM ENABLE FAILPOINT drop_database_before_exclusive_ddl_lock"
...
${DATASTORE_CLIENT} -q "SYSTEM WAIT FAILPOINT drop_database_before_exclusive_ddl_lock PAUSE"
...
${DATASTORE_CLIENT} -q "DROP TABLE IF EXISTS ${DB}.a_mem"
${DATASTORE_CLIENT} -q "DROP TABLE IF EXISTS ${DB}.z_mt"
...
${DATASTORE_CLIENT} -q "SYSTEM NOTIFY FAILPOINT drop_database_before_exclusive_ddl_lock"
...
${DATASTORE_CLIENT} -q "SELECT count() FROM system.databases WHERE name = '${DB}'"
