...
$DATASTORE_CLIENT -q "create database $db engine=Replicated('zookeeper2:/test/$DATASTORE_DATABASE/rdb', 's1', 'r1')"
...
$DATASTORE_CLIENT -q "system drop database replica 's1|r1' from zkpath 'zookeeper2:/test/$DATASTORE_DATABASE/rdb'" 2>&1| grep -Fac "There is a local database"
$DATASTORE_CLIENT -q "system drop database replica 's1|r1' from zkpath 'zookeeper2:/test/$DATASTORE_DATABASE/rdb/'" 2>&1| grep -Fac "There is a local database"
$DATASTORE_CLIENT -q "system drop database replica 'r1' from shard 's1' from zkpath 'zookeeper2:/test/$DATASTORE_DATABASE/rdb/'" 2>&1| grep -Fac "There is a local database"

$DATASTORE_CLIENT -q "system drop database replica 's1|r1' from zkpath 'zookeeper2:/test/$DATASTORE_DATABASE/'" 2>&1| grep -Fac "does not look like a path of Replicated database"
$DATASTORE_CLIENT -q "system drop database replica 's2|r1' from zkpath 'zookeeper2:/test/$DATASTORE_DATABASE/rdb'" 2>&1| grep -Fac "does not exist"
$DATASTORE_CLIENT -q "system drop database replica 's1' from shard 'r1' from zkpath 'zookeeper2:/test/$DATASTORE_DATABASE/rdb'" 2>&1| grep -Fac "does not exist"
$DATASTORE_CLIENT -q "system drop database replica 's1|r1' from shard 's1' from zkpath 'zookeeper2:/test/$DATASTORE_DATABASE/rdb'" 2>&1| grep -Fac "does not exist"
$DATASTORE_CLIENT -q "system drop database replica 's2/r1' from zkpath 'zookeeper2:/test/$DATASTORE_DATABASE/rdb'" 2>&1| grep -Fac "Invalid replica name"

db2="${db}_2"
db3="${db}_3"
$DATASTORE_CLIENT -q "create database $db2 engine=Replicated('zookeeper2:/test/$DATASTORE_DATABASE/rdb', 's1', 'r2')"
$DATASTORE_CLIENT -q "create database $db3 engine=Replicated('zookeeper2:/test/$DATASTORE_DATABASE/rdb', 's2', 'r1')"
...
$DATASTORE_CLIENT -q "create database $db9 engine=Replicated('zookeeper2:/test/${DATASTORE_DATABASE}/rdb', 's9', 'r9')"
$DATASTORE_CLIENT -q "detach database $db9"
$DATASTORE_CLIENT -q "insert into system.zookeeper(name, path, value, zookeeperName) values ('active', '/test/${DATASTORE_DATABASE}/rdb/replicas/s9|r9', '${REPLICA_UUID}', 'zookeeper2')"
...
$DATASTORE_CLIENT -q "system drop database replica 'r1' from shard 's1' from zkpath 'zookeeper2:/test/$DATASTORE_DATABASE/rdb/'"
...
db4="${db}_4"
$DATASTORE_CLIENT -q "create database $db4 engine=Replicated('zookeeper2:/test/$DATASTORE_DATABASE/rdb', 's1', 'r1')"
...
$DATASTORE_CLIENT -q "system drop database replica 'zookeeper2:doesntexist$DATASTORE_DATABASE' from shard 'doesntexist'"
...
$DATASTORE_CLIENT -q "system drop replica 'zookeeper2:doesntexist$DATASTORE_DATABASE' from database $db4"
$DATASTORE_CLIENT -q "system drop replica 'zookeeper2:doesntexist$DATASTORE_DATABASE'"
