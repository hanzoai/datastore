...
CREATE TABLE {DATASTORE_DATABASE_1:Identifier}.cross_db_target (id UInt32) ENGINE = MergeTree ORDER BY id;
-- Create alias in DATASTORE_DATABASE_1 while current database is DATASTORE_DATABASE
-- The non-qualified 'cross_db_target' should resolve to DATASTORE_DATABASE_1, not current db
CREATE TABLE {DATASTORE_DATABASE_1:Identifier}.cross_db_alias ENGINE = Alias('cross_db_target');

SET check_referential_table_dependencies = 1;
DROP TABLE {DATASTORE_DATABASE_1:Identifier}.cross_db_target; -- { serverError HAVE_DEPENDENT_OBJECTS }

-- Clean up
DROP TABLE {DATASTORE_DATABASE_1:Identifier}.cross_db_alias;
DROP TABLE {DATASTORE_DATABASE_1:Identifier}.cross_db_target;
