...
${DATASTORE_CLIENT} -q "DROP TABLE IF EXISTS index_corruption"

${DATASTORE_CLIENT} -n -q "CREATE TABLE IF NOT EXISTS index_corruption
(
...
${DATASTORE_CLIENT} -q 'insert into index_corruption FORMAT TSVWithNamesAndTypes' < $CURDIR/data_zstd/test_03755.tsv.zst

${DATASTORE_CLIENT} -q 'ALTER TABLE index_corruption (MODIFY TTL toDateTime(38C3868AC) + toIntervalYear(140));'

${DATASTORE_CLIENT} -q "SELECT * FROM index_corruption WHERE (7AF = 'xyz');"
