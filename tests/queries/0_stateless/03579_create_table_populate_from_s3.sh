...
INSERT INTO FUNCTION s3(
    'http://localhost:11111/test/$DATASTORE_DATABASE/03579.tsv',
    'TSV',
...
do
    ...
    CREATE OR REPLACE TABLE table (x UInt32, y UInt32, z UInt32)
        ...
        ORDER BY x
        AS SELECT *
            FROM s3('http://localhost:11111/test/$DATASTORE_DATABASE/03579.tsv', NOSIGN, 'TSV')
            LIMIT 100;
    ...
    CREATE OR REPLACE TABLE table_s3Cluster (x UInt32, y UInt32, z UInt32)
        ...
        ORDER BY x
        AS SELECT *
            FROM s3Cluster('test_cluster_one_shard_three_replicas_localhost', 'http://localhost:11111/test/$DATASTORE_DATABASE/03579.tsv', 'TSV')
            LIMIT 100;
...
replicated_db="${DATASTORE_DATABASE}_replicated"
cleanup_replicated_database() {
    set +e
    $DATASTORE_CLIENT -q "DROP DATABASE IF EXISTS ${replicated_db};"
    $DATASTORE_CLIENT -q "DROP DATABASE IF EXISTS ${replicated_db}_second;"
}
trap cleanup_replicated_database EXIT

$DATASTORE_CLIENT <<EOF
CREATE DATABASE IF NOT EXISTS ${replicated_db} ENGINE = Replicated('/datastore/{database}/replicated_db', 'shard_1');
CREATE DATABASE IF NOT EXISTS ${replicated_db}_second ENGINE = Replicated('/datastore/{database}/replicated_db', 'shard_2');
EOF
...
CREATE OR REPLACE TABLE ${replicated_db}.table_s30 (x UInt32, y UInt32, z UInt32)
    engine=Memory
    AS SELECT *
        FROM s3('http://localhost:11111/test/$DATASTORE_DATABASE/03579.tsv', 'TSV', 'x UInt32, y UInt32, z UInt32')
        LIMIT 100;
...
CREATE OR REPLACE TABLE ${replicated_db}.table_s3Cluster (x UInt32, y UInt32, z UInt32)
    engine=Memory
    AS SELECT *
        FROM s3Cluster('test_cluster_one_shard_three_replicas_localhost', 'http://localhost:11111/test/$DATASTORE_DATABASE/03579.tsv', 'TSV', 'x UInt32, y UInt32, z UInt32')
        LIMIT 100;
