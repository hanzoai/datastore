...
$DATASTORE_CLIENT -n -q "EXPLAIN indexes = 1 SELECT * FROM t WHERE a = 0;" | grep -o "Search Algorithm: binary search"
$DATASTORE_CLIENT -n -q "EXPLAIN indexes = 1 SELECT * FROM t WHERE b = 0;" | grep -o "Search Algorithm: generic exclusion search"
