...
CREATE TABLE partslost_0 (x String) ENGINE=ReplicatedMergeTree('/datastore/table/{database}_02067_lost/partslost', '0') ORDER BY tuple()
...
CREATE TABLE partslost_1 (x String) ENGINE=ReplicatedMergeTree('/datastore/table/{database}_02067_lost/partslost', '1') ORDER BY tuple()
    SETTINGS min_rows_for_wide_part = 0, min_bytes_for_wide_part = 0, old_parts_lifetime = 1,
    cleanup_delay_period = 1, cleanup_delay_period_random_add = 1, cleanup_thread_preferred_points_per_iteration=0,
    index_granularity = 8192, index_granularity_bytes = '10Mi';

CREATE TABLE partslost_2 (x String) ENGINE=ReplicatedMergeTree('/datastore/table/{database}_02067_lost/partslost', '2') ORDER BY tuple()
    SETTINGS min_rows_for_wide_part = 0, min_bytes_for_wide_part = 0, old_parts_lifetime = 1,
    cleanup_delay_period = 1, cleanup_delay_period_random_add = 1, cleanup_thread_preferred_points_per_iteration=0,
    index_granularity = 8192, index_granularity_bytes = '10Mi';
