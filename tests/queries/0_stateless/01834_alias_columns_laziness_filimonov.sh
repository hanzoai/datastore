...
# In very old Datastore versions alias column was calculated for every row.
...
${DATASTORE_CLIENT} --profile-events-delay-ms=-1 --print-profile-events --query "SELECT x, y FROM aliases_lazyness WHERE x = 1 FORMAT Null" |& grep -o -e "SleepFunctionMicroseconds.*" -e "SleepFunctionCalls.*"

${DATASTORE_CLIENT} --query "drop table aliases_lazyness"
