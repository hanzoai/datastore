...
CREATE TABLE t_04612 (k UInt64) ENGINE = MergeTree ORDER BY intHash32(k) SAMPLE BY intHash32(k);
INSERT INTO t_04612 SELECT number FROM numbers(10000);
...
SELECT count() FROM cluster('test_shard_localhost', merge(currentDatabase(), '^t_04612$'));
SELECT count() FROM cluster('test_shard_localhost', merge(currentDatabase(), '^t_04612$')) SAMPLE 1/2;
SELECT count() FROM cluster('test_shard_localhost', merge(currentDatabase(), '^t_04612$')) SAMPLE 1/2 OFFSET 1/2;
