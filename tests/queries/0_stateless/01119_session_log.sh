...
where event_date >= yesterday() AND event_time >= now() - 600 AND user in ('default', 'nonexistsnt_user_1119', '   ', ' INTERSERVER SECRET ')
...
for _ in {1..10}
do
    if [ "`$DATASTORE_CLIENT -q "$session_log_query_prefix and type = 'Logout'" | wc -l`" -eq 3 ]
    then
        break
    fi
    sleep 2
...
$DATASTORE_CLIENT -q "$session_log_query_prefix and type = 'Logout' order by user, interface"
