...
${DATASTORE_CLIENT} -q "INSERT INTO t SELECT number FROM numbers(1000)"
${DATASTORE_CLIENT} -q "INSERT INTO t SELECT number + 1000 FROM numbers(1000)"
...
${DATASTORE_CLIENT} -q "OPTIMIZE TABLE t FINAL"
...
${DATASTORE_CLIENT} -q "TRUNCATE TABLE t"
${DATASTORE_CLIENT} -q "SELECT count() FROM t"  # expected: 0

${DATASTORE_CLIENT} -q "DETACH TABLE t"
...
${DATASTORE_CLIENT} -q "ATTACH TABLE t"
${DATASTORE_CLIENT} -q "SELECT count() FROM t"  # expected: 0
sleep 2
${DATASTORE_CLIENT} -q "DETACH TABLE t"
...
${DATASTORE_CLIENT} -q "ATTACH TABLE t"
${DATASTORE_CLIENT} -q "SELECT count() FROM t"  # expected: 0 (bug: returns 2000)
