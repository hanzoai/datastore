...
READ_USER="u_04103_r_$DATASTORE_DATABASE"
WRITE_USER="u_04103_w_$DATASTORE_DATABASE"
$DATASTORE_CLIENT --format Null -q "CREATE USER $READ_USER, $WRITE_USER"
...
$DATASTORE_CLIENT -m -q "
    ...
    GRANT SELECT, INSERT ON ${DATASTORE_DATABASE}.data_04103 TO $READ_USER, $WRITE_USER;
"

write_query_id="04103_w_$DATASTORE_DATABASE"
read_query_id="04103_r_$DATASTORE_DATABASE"
...
$DATASTORE_CLIENT --user "$WRITE_USER" --query_id "$write_query_id" -q "
    INSERT INTO ${DATASTORE_DATABASE}.data_04103 SELECT number+1e6 FROM numbers(1e6)
    SETTINGS max_network_bandwidth_for_user = 1000000
" &
$DATASTORE_CLIENT --user "$READ_USER" --query_id "$read_query_id" -q "
    SELECT * FROM ${DATASTORE_DATABASE}.data_04103 WHERE key < 1e6 FORMAT Null
    SETTINGS max_network_bandwidth_for_user = 1000000
" &
wait

# FIXME: some issues with query plan serialization
$DATASTORE_CLIENT --serialize_query_plan=0 -m -q "
...
$DATASTORE_CLIENT --format Null -q "
