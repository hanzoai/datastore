...
${DATASTORE_CLIENT} -q "DROP TABLE IF EXISTS t_async_insert_bad_data"
${DATASTORE_CLIENT} -q "CREATE TABLE t_async_insert_bad_data (d Date, s String) ENGINE = Memory"

${DATASTORE_CURL} -sS "${DATASTORE_URL}&async_insert=1" -X POST \
...
${DATASTORE_CLIENT} -q "SELECT s FROM t_async_insert_bad_data ORDER BY s"
${DATASTORE_CLIENT} -q "DROP TABLE IF EXISTS t_async_insert_bad_data"
