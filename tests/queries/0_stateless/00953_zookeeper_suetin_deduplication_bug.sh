...
$DATASTORE_CLIENT --query="DROP TABLE IF EXISTS elog;"
...
ENGINE = ReplicatedMergeTree('/datastore/tables/$DATASTORE_TEST_ZOOKEEPER_PREFIX/elog/{shard}', '{replica}')
PARTITION BY date
ORDER BY (engine_id)
...
$DATASTORE_CLIENT --query="INSERT INTO elog VALUES (toDate('2018-10-01'), 1, 'hello')"
$DATASTORE_CLIENT --query="INSERT INTO elog VALUES (toDate('2018-10-01'), 2, 'hello')"
$DATASTORE_CLIENT --query="INSERT INTO elog VALUES (toDate('2018-10-01'), 3, 'hello')"

$DATASTORE_CLIENT --query="SELECT count(*) from elog" # 3 rows

# Get the resolved ZK table path (with macros expanded) to query both directories.
zk_path=$($DATASTORE_CLIENT --query="SELECT replica_path FROM system.replicas WHERE database = currentDatabase() AND table = 'elog'" | sed 's|/replicas/.*||')
...
$DATASTORE_CLIENT --query="SELECT count(*) from elog" # 4 rows
...
$DATASTORE_CLIENT --query="SELECT count(*) from elog" # 5 rows
...
$DATASTORE_CLIENT --query="SELECT count(*) from elog" # still 5 rows

$DATASTORE_CLIENT -q "DROP TABLE elog"
