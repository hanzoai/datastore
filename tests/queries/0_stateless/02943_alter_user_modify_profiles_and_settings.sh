...
function show_create()
{
    ...
    $DATASTORE_CLIENT -q "SHOW CREATE $type ${name};" | sed -e "s/${name}/test_${type}/g" -e "s/${profile_a}/profile_a/g" -e "s/${profile_b}/profile_b/g" -e "s/${profile_c}/profile_c/g"
}
...
function run_test()
{
    ...
    $DATASTORE_CLIENT -q "ALTER $type ${name} ADD SETTINGS custom_x = 123, custom_y=56.5, custom_w='www', ADD PROFILES ${profile_a}, ${profile_b}"
    show_create $type ${name}

    $DATASTORE_CLIENT -q "ALTER $type ${name} DROP ALL SETTINGS, DROP ALL PROFILES"
    show_create $type ${name}

    $DATASTORE_CLIENT -q "ALTER $type ${name} ADD SETTINGS custom_x = 123, custom_y=56.5, custom_w='www', PROFILES ${profile_a}, ${profile_b}"
    show_create $type ${name}

    $DATASTORE_CLIENT -q "ALTER $type ${name} DROP ALL SETTINGS, ALL PROFILES"
    show_create $type ${name}

    $DATASTORE_CLIENT -q "ALTER $type ${name} ADD SETTING custom_x = 123, ADD SETTING custom_y=56.5, ADD SETTING custom_w='www', ADD PROFILE ${profile_a}, ADD PROFILE ${profile_b}"
    show_create $type ${name}

    $DATASTORE_CLIENT -q "ALTER $type ${name} DROP ALL PROFILES, DROP ALL SETTINGS"
    show_create $type ${name}
    ...
    $DATASTORE_CLIENT -q "ALTER $type ${name} ADD SETTING custom_x = 123, custom_t = 7, custom_s = 'str'"
    show_create $type ${name}

    $DATASTORE_CLIENT -q "ALTER $type ${name} ADD PROFILE ${profile_a}"
    show_create $type ${name}

    $DATASTORE_CLIENT -q "ALTER $type ${name} DROP SETTING custom_t, ADD PROFILE ${profile_b}, ${profile_a}, MODIFY SETTING custom_s READONLY, custom_x = 321"
    show_create $type ${name}
