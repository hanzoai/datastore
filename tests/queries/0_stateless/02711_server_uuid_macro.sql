...
CREATE TABLE test (x UInt8) ENGINE = ReplicatedMergeTree('/datastore/tables/{database}/test/{shard}', 'replica-{server_uuid}') ORDER BY x;
...
SELECT count() > 0 FROM system.zookeeper WHERE path = '/datastore/tables/' || currentDatabase() || '/test/s1/replicas/' AND name LIKE 'replica-' || serverUUID()::String || '%';

-- An attempt to create a second table with the same UUID results in error.
CREATE TABLE test2 (x UInt8) ENGINE = ReplicatedMergeTree('/datastore/tables/{database}/test/{shard}', 'replica-{server_uuid}') ORDER BY x; -- { serverError REPLICA_ALREADY_EXISTS }
