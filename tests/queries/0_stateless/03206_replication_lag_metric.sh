...
ZK_PATH="/test/test_replication_lag_metric/${DATASTORE_TEST_UNIQUE_NAME}"
...
for i in $(seq 1 30); do
    DB1="rdb1_${DATASTORE_TEST_UNIQUE_NAME}_${i}"
    DB2="rdb2_${DATASTORE_TEST_UNIQUE_NAME}_${i}"

    $DATASTORE_CLIENT --query "CREATE DATABASE ${DB1} ENGINE = Replicated('${ZK_PATH}/${i}', 'shard1', 'replica1')"
    $DATASTORE_CLIENT --query "CREATE DATABASE ${DB2} ENGINE = Replicated('${ZK_PATH}/${i}', 'shard1', 'replica2')"

    $DATASTORE_CLIENT --distributed_ddl_task_timeout 0 --query \
        "CREATE TABLE ${DB1}.t (id UInt32) ENGINE = ReplicatedMergeTree ORDER BY id"

    lag1=$($DATASTORE_CLIENT --query "SELECT replication_lag FROM system.clusters WHERE cluster = '${DB1}' AND replica_num = 1")
    lag2=$($DATASTORE_CLIENT --query "SELECT replication_lag FROM system.clusters WHERE cluster = '${DB1}' AND replica_num = 2")

    $DATASTORE_CLIENT --query "DROP DATABASE ${DB1}"
    $DATASTORE_CLIENT --query "DROP DATABASE ${DB2}"
