...
function run_with_progress_and_match_total_rows()
{
    CURL_RESPONSE=$(echo "$1" | \
            ${DATASTORE_CURL} -vsS "${DATASTORE_URL}&http_wait_end_of_query=1&max_block_size=1&send_progress_in_http_headers=1&http_headers_progress_interval_ms=0&output_format_parallel_formatting=0" --data-binary @- 2>&1)
