...
# Corrupt a byte inside column 'a' data region of the single data.packed archive. The archive index
# and the checksums stay intact, so the part is still loadable but its contents no longer match the
# checksums it advertises.
A_OFFSET=$(${CLICKHOUSE_BINARY} packed-io -i "${DATA_PATH}data.packed" --list 2>/dev/null | awk '$1 == "a.bin" { print $3 }')
printf '\xAA\xBB\xCC\xDD' | dd of="${DATA_PATH}data.packed" bs=1 seek=$((A_OFFSET + 2)) count=4 conv=notrunc 2>/dev/null
