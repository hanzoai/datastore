...
${DATASTORE_CLIENT} -q "DROP TABLE IF EXISTS ghdata"
${DATASTORE_CLIENT} -q "CREATE TABLE ghdata (data JSON) ENGINE = MergeTree ORDER BY tuple() SETTINGS index_granularity = 8192, index_granularity_bytes = '10Mi'" --enable_json_type 1

cat $CUR_DIR/data_json/ghdata_sample.json | ${DATASTORE_CLIENT} \
  --max_memory_usage 10G --max_execution_time 300 --query "INSERT INTO ghdata FORMAT JSONAsObject"

${DATASTORE_CLIENT} -q "ALTER TABLE ghdata MODIFY column data JSON(max_dynamic_paths=10) SETTINGS mutations_sync=1" --enable_json_type 1

${DATASTORE_CLIENT} -q "SELECT count() FROM ghdata WHERE NOT ignore(*)"
...
${DATASTORE_CLIENT} -q "SELECT max(data.payload.pull_request.assignees[].size0) FROM ghdata"
