...
backup_name="${DATASTORE_DATABASE}_03799_backup_security"
user_name="test_03799_user_${DATASTORE_DATABASE}"

function cleanup()
{
    $DATASTORE_CLIENT -q "DROP TABLE IF EXISTS test_backup_security"
    $DATASTORE_CLIENT -q "DROP TABLE IF EXISTS test_restored"
...
# We use DATASTORE_CLIENT_BINARY directly to avoid test framework injecting settings that conflict with readonly mode.
$DATASTORE_CLIENT --readonly=1 --backup_restore_s3_retry_attempts=0 -q "RESTORE TABLE test_backup_security AS test_restored FROM S3('http://localhost:11111/test/backups/${backup_name}', 'INVALID_ACCESS_KEY', 'INVALID_SECRET')" 2>&1 | grep -q "ACCESS_DENIED" && echo "Test 1 OK" || echo "Test 1 FAIL"
# Verify that the table was not created
$DATASTORE_CLIENT -q "SELECT count() FROM system.tables WHERE database = currentDatabase() AND name = 'test_restored'"
...
$DATASTORE_CLIENT -q "GRANT SELECT ON ${DATASTORE_DATABASE}.test_backup_security TO $user_name"
# User has SELECT but not BACKUP permission - should get ACCESS_DENIED, not S3_ERROR
$DATASTORE_CLIENT --user=$user_name -m -q "
BACKUP TABLE test_backup_security TO S3('http://localhost:11111/test/backups/${DATASTORE_DATABASE}/no_permission_backup', 'INVALID_ACCESS_KEY', 'INVALID_SECRET') SETTINGS backup_restore_s3_retry_attempts=0; -- { serverError ACCESS_DENIED }
"
$DATASTORE_CLIENT -q "DROP USER IF EXISTS $user_name"
