...
DB="${DATASTORE_DATABASE}_repl"
ZK_PATH="/test/restart_replica_digest_mismatch/${DATASTORE_TEST_UNIQUE_NAME}"
...
${DATASTORE_CLIENT} --distributed_ddl_output_mode=none \
...
${DATASTORE_CLIENT} -q "SYSTEM ENABLE FAILPOINT restart_replica_fail_after_detach"
...
${DATASTORE_CLIENT} --max_execution_time 2 \
    -q "SYSTEM RESTART REPLICA ${DB}.t1" 2>&1 \
    | grep -o "TIMEOUT_EXCEEDED" | head -1

${DATASTORE_CLIENT} -q "SYSTEM DISABLE FAILPOINT restart_replica_fail_after_detach"
...
${DATASTORE_CLIENT} -q "SYSTEM ENABLE FAILPOINT database_replicated_force_metadata_digest_check"
...
${DATASTORE_CLIENT} -q "SYSTEM DISABLE FAILPOINT database_replicated_force_metadata_digest_check"

echo "OK"

${DATASTORE_CLIENT} -q "DROP DATABASE ${DB} SYNC" 2>/dev/null || true
