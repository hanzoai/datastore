...
for check_query in "SELECT value FROM system.settings WHERE name = 'alter_sync';" "SELECT getSetting('alter_sync');"; do
    ...
    ${DATASTORE_CURL} -sS "$DATASTORE_URL&replication_alter_partitions_sync=0" -d "$check_query"
    ${DATASTORE_CURL} -sS "$DATASTORE_URL&replication_alter_partitions_sync=2" -d "$check_query"

    echo 'Using client options'
    $DATASTORE_CLIENT --replication_alter_partitions_sync=0 -q "$check_query"
    $DATASTORE_CLIENT --replication_alter_partitions_sync=2 -q "$check_query"
done
...
for setting_name in "replication_alter_partitions_sync" "alter_sync"; do
    ...
    $DATASTORE_CLIENT --replication_alter_partitions_sync=0 -q "$query"
    $DATASTORE_CLIENT --replication_alter_partitions_sync=2 -q "$query"
done
