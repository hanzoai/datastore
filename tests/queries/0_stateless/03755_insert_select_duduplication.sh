...
DATASTORE_CLIENT="${DATASTORE_CLIENT} --async_insert=0 --insert_deduplicate=1"

echo "deduplicate_insert_select=force_enable with non-stable select, exception expected"
${DATASTORE_CLIENT} --deduplicate_insert_select=force_enable -q "
...
CH_CLIENT=$(echo ${DATASTORE_CLIENT} | sed 's/--send_logs_level=[^ ]*//')
...
${DATASTORE_CLIENT} --deduplicate_insert_select='enable_when_possible' -q "
    INSERT INTO dst SELECT number AS id, toString(number) AS data FROM numbers(10) ORDER BY all;
    INSERT INTO dst SELECT number AS id, toString(number) AS data FROM numbers(10) ORDER BY all;
    SELECT count() FROM dst;
    TRUNCATE TABLE dst;
...
${DATASTORE_CLIENT} --deduplicate_insert_select=disable -q "
    INSERT INTO dst SELECT number AS id, toString(number) AS data FROM numbers(10) ORDER BY all;
    INSERT INTO dst SELECT number AS id, toString(number) AS data FROM numbers(10) ORDER BY all;
    SELECT count() FROM dst;
    TRUNCATE TABLE dst;
