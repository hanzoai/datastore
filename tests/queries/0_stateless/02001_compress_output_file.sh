...
${DATASTORE_CLIENT} --query "SELECT * FROM (SELECT 'Hello, World! From client.') INTO OUTFILE '${DATASTORE_TMP}/test_compression_of_output_file_from_client.gz'"
...
${DATASTORE_LOCAL} --query "SELECT * FROM (SELECT 'Hello, World! From local.') INTO OUTFILE '${DATASTORE_TMP}/test_compression_of_output_file_from_local.gz'"
