#!/usr/bin/env bash
# Tags: no-async-insert, no-fasttest
# no-fasttest: Too slow for fast test (~20s), covered by regular stateless runs.
# no-async-insert: Test expects new part after connection drop

CURDIR=$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)
# shellcheck source=../shell_config.sh
. "$CURDIR"/../shell_config.sh

DATASTORE_TABLE="test_insert_on_connection_drop"
SETTINGS="input_format_connection_handling=1,min_insert_block_size_bytes=0,min_insert_block_size_rows=0"
DATASTORE_INSERT_URL="${DATASTORE_URL}&max_query_size=1000&query=INSERT%20INTO%20${DATASTORE_TABLE}%20SETTINGS%20${SETTINGS//,/%2C}%20FORMAT%20CSV"

echo "DROP TABLE IF EXISTS ${DATASTORE_TABLE}" | \
    curl -sS -d@- "$DATASTORE_URL"

echo "CREATE TABLE ${DATASTORE_TABLE} (id UInt64, data String, ts UInt64, value UInt64) ENGINE = MergeTree ORDER BY id" | \
    curl -sS -d@- "$DATASTORE_URL"

(
i=1
while true; do
    ts=$(date +%s)
    echo "$i,hello-$i,$ts,3" || exit 0
    ((i++))
done
) | curl -sS -N --no-buffer \
    -T - \
    -X POST \
    -H "Content-Type: text/csv" \
    -H "Transfer-Encoding: chunked" \
    "$DATASTORE_INSERT_URL" 2>&1 &

PIPELINE_PID=$!

sleep 15

# Temporarily redirect the shell's own stderr to suppress expected
# "Broken pipe" and "Killed" job notification messages from bash.
exec {_stderr}>&2 2>/dev/null
kill -9 $PIPELINE_PID
wait $PIPELINE_PID
exec 2>&$_stderr {_stderr}>&-


for _ in $(seq 1 60); do
    $DATASTORE_CLIENT -q "SYSTEM FLUSH LOGS part_log;"
    parts_count=$(${DATASTORE_CLIENT} --query "
        SELECT count()
        FROM system.part_log
        WHERE table = '${DATASTORE_TABLE}'
          AND event_type = 'NewPart'
          AND database = currentDatabase()
          AND event_time > (now() - 120)
    ")
    [ "${parts_count}" -ge 1 ] && break
    sleep 0.5
done

echo "Number of parts created: ${parts_count}"

echo "DROP TABLE IF EXISTS ${DATASTORE_TABLE}" | \
    curl -sS -d@- "$DATASTORE_URL"

