...
$DATASTORE_CLIENT --query="
    ...
    PRIMARY KEY id
    SOURCE(DATASTORE(HOST 'localhost' PORT tcpPort() USER 'default' TABLE 'source_table'))
    LIFETIME(MIN 1 MAX 1000)
    LAYOUT(SSD_CACHE(BLOCK_SIZE 4096 FILE_SIZE 8192 PATH '$DATASTORE_USER_FILES/${DATASTORE_DATABASE}_ssd_dic'));
