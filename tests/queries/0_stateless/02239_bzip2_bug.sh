...
${DATASTORE_CLIENT} --query "CREATE TABLE file (x UInt64) ENGINE = File(TSV, '${DATASTORE_DATABASE}/data.tsv.bz2')"
${DATASTORE_CLIENT} --query "TRUNCATE TABLE file"
${DATASTORE_CLIENT} --query "INSERT INTO file SELECT * FROM numbers(1000000)"
${DATASTORE_CLIENT} --max_read_buffer_size=2 --query "SELECT count(), max(x) FROM file"
