...
$DATASTORE_CLIENT --query-id="$same_query_not_finish" -q "SELECT sleep(120) SETTINGS function_sleep_max_microseconds_per_block=1000000000, query_metric_log_interval=100, replace_running_query=0 FORMAT Null; -- { serverError QUERY_WAS_CANCELLED}" &

# In this case, we let the first one running a little bit and run the second one so that it cancels the first one.
$DATASTORE_CLIENT --query-id="$same_query_finish" -q "SELECT sleep(120) SETTINGS function_sleep_max_microseconds_per_block=1000000000, query_metric_log_interval=0, replace_running_query=0 FORMAT Null; -- { serverError QUERY_WAS_CANCELLED}" &

# Ensure both queries have started
while true; do
    $DATASTORE_CLIENT -q "SYSTEM FLUSH LOGS system.query_log;"
    count=$($DATASTORE_CLIENT -q """
        SELECT
            count()
        FROM
            system.query_log
...
$DATASTORE_CLIENT --query-id="$same_query_finish" -q "SELECT sleep(2) SETTINGS query_metric_log_interval=100, replace_running_query=1, replace_running_query_max_wait_ms=30000 FORMAT Null;" &
$DATASTORE_CLIENT --query-id="$same_query_not_finish" -q "SELECT 'a' SETTINGS query_metric_log_interval=0, replace_running_query=0 FORMAT Null;" 2> /dev/null

# Kill the initial query because the second one didn't replace it
$DATASTORE_CLIENT -q "KILL QUERY WHERE query_id = '$same_query_not_finish' SYNC FORMAT Null" &

wait

$DATASTORE_CLIENT -q "SYSTEM FLUSH LOGS query_log, query_metric_log"
