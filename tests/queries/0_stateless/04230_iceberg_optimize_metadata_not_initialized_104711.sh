...
# Regression test for https://github.com/ClickHouse/Datastore/issues/104711:
...
${DATASTORE_CLIENT} --allow_insert_into_iceberg=1 --output_format_compression_level=11 \
...
${DATASTORE_CLIENT} --query "DETACH TABLE ${TABLE} SYNC"
${DATASTORE_CLIENT} --send_logs_level=fatal --query "ATTACH TABLE ${TABLE}" 2>/dev/null
...
${DATASTORE_CLIENT} --allow_experimental_iceberg_compaction=1 \
...
${DATASTORE_CLIENT} --query "ALTER TABLE ${TABLE} ADD COLUMN c1 Int32" 2>&1 \
...
${DATASTORE_CLIENT} --query "DROP TABLE IF EXISTS ${TABLE} SYNC"
