...
$DATASTORE_CLIENT --allow_deprecated_database_ordinary=1 -m -q "create database ordinary_$DATASTORE_DATABASE engine=Ordinary"

# MergeTree
$DATASTORE_CLIENT -m -q """
    use ordinary_$DATASTORE_DATABASE;
...
$DATASTORE_CLIENT -m -q """
    ...
    Engine=ReplicatedMergeTree('/datastore/tables/$DATASTORE_TEST_ZOOKEEPER_PREFIX/rep_data_01810', '1')
    order by key
    partition by key%100
    settings concurrent_part_removal_threshold=99, min_bytes_for_wide_part=0;
...
$DATASTORE_CLIENT -m -q "drop database ordinary_$DATASTORE_DATABASE"
