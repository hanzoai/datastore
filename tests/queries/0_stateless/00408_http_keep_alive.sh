...
${DATASTORE_CURL} -vsS "${URL}" --data-binary @- <<< "SELECT 1" 2>&1 | sed -r 's/(keep-alive: timeout=10, max=)[0-9]+/\1?/I' | grep -i 'keep-alive';
${DATASTORE_CURL} -vsS "${URL}" --data-binary @- <<< " error here " 2>&1 | sed -r 's/(keep-alive: timeout=10, max=)[0-9]+/\1?/I' | grep -i 'keep-alive';
${DATASTORE_CURL} -vsS "${URL}"ping  2>&1 | perl -lnE 'print if /Keep-Alive/' | sed -r 's/(keep-alive: timeout=10, max=)[0-9]+/\1?/I' | grep -i 'keep-alive';

# no keep-alive:
${DATASTORE_CURL} -vsS "${URL}"404/not/found/ 2>&1 | perl -lnE 'print if /Keep-Alive/';
...
tmp_prefix=00408_${DATASTORE_DATABASE}
clean() {
    rm -f "$tmp_prefix"*
}
trap clean EXIT
...
do
    ...
    ${DATASTORE_CURL} -vsS "${DATASTORE_URL}&${settings}&query_id=00408-wait-$wait_for_async_insert-$file_1" -d "INSERT INTO async_inserts FORMAT CSV
    1,\"a-$wait_for_async_insert\"
    2,\"b-$wait_for_async_insert\"" |& grep 'Connection:' > $file_1 &

    ${DATASTORE_CURL} -vsS "${DATASTORE_URL}&${settings}&query_id=00408-wait-$wait_for_async_insert-$file_2" -d "INSERT INTO async_inserts FORMAT CSV
    qqqqqqqqqqq-$wait_for_async_insert" |& grep 'Connection:' > $file_2  &

    ${DATASTORE_CURL} -vsS "${DATASTORE_URL}&${settings}&query_id=00408-wait-$wait_for_async_insert-$file_3" -d "INSERT INTO async_inserts FORMAT CSV
