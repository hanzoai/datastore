...
# Regression test for https://github.com/ClickHouse/Datastore/issues/97309
#
...
$DATASTORE_CLIENT --query "DROP TABLE IF EXISTS ${TABLE_R1} SYNC"
$DATASTORE_CLIENT --query "DROP TABLE IF EXISTS ${TABLE_R2} SYNC"

$DATASTORE_CLIENT --query "
    CREATE TABLE ${TABLE_R1} (key UInt64, value String)
    ENGINE = ReplicatedMergeTree('/datastore/tables/$DATASTORE_TEST_ZOOKEEPER_PREFIX/${TABLE_PREFIX}', '1')
    ORDER BY key
"

$DATASTORE_CLIENT --query "
    CREATE TABLE ${TABLE_R2} (key UInt64, value String)
    ENGINE = ReplicatedMergeTree('/datastore/tables/$DATASTORE_TEST_ZOOKEEPER_PREFIX/${TABLE_PREFIX}', '2')
    ORDER BY key
"

$DATASTORE_CLIENT --insert_keeper_fault_injection_probability=0 --query "
    INSERT INTO ${TABLE_R1} SELECT number, toString(number) FROM numbers(1000)
"

$DATASTORE_CLIENT --query "SYSTEM SYNC REPLICA ${TABLE_R2}"
...
function drop_replica_thread()
{
    local TIMELIMIT=$((SECONDS + TIMEOUT))
    while [ $SECONDS -lt "$TIMELIMIT" ]; do
        $DATASTORE_CLIENT --query "DROP TABLE IF EXISTS ${TABLE_R2} SYNC" >/dev/null 2>&1 ||:
        sleep 0.$((RANDOM % 3))
        $DATASTORE_CLIENT --query "
            CREATE TABLE ${TABLE_R2} (key UInt64, value String)
            ENGINE = ReplicatedMergeTree('/datastore/tables/$DATASTORE_TEST_ZOOKEEPER_PREFIX/${TABLE_PREFIX}', '2')
...
$DATASTORE_CLIENT --query "DROP TABLE IF EXISTS ${TABLE_R2} SYNC" 2>/dev/null
$DATASTORE_CLIENT --query "
    CREATE TABLE ${TABLE_R2} (key UInt64, value String)
    ENGINE = ReplicatedMergeTree('/datastore/tables/$DATASTORE_TEST_ZOOKEEPER_PREFIX/${TABLE_PREFIX}', '2')
    ORDER BY key
...
$DATASTORE_CLIENT --query "SYSTEM SYNC REPLICA ${TABLE_R1}" 2>/dev/null ||:
$DATASTORE_CLIENT --query "SYSTEM SYNC REPLICA ${TABLE_R2}" 2>/dev/null ||:

# Verify the server is alive and data is readable.
$DATASTORE_CLIENT --query "SELECT count() > 0 FROM ${TABLE_R1}"
