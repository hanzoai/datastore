...
$DATASTORE_CLIENT --max_execution_time 300 --max_rows_to_read 50M --query "
...
LOG="$DATASTORE_TMP/err-$DATASTORE_DATABASE"
$DATASTORE_BENCHMARK --max_rows_to_read 51M --iterations 10 --max_threads 100 --min_bytes_to_use_direct_io 1 <<< "SELECT sum(UserID) FROM bug PREWHERE NOT ignore(Date)" 1>/dev/null 2>"$LOG"
