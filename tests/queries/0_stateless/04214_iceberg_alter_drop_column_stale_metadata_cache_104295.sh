...
# Regression test for https://github.com/ClickHouse/Datastore/issues/104295
#
...
#
# The reproducer runs in `datastore local` (rather than against the test
# server) because the underlying bug aborts the process with `SIGABRT`. With
# `datastore-client` the abort lands inside the long-running test server,
...
# `datastore local` contains the abort to a single short-lived subprocess:
...
# Isolated work dir for the `IcebergLocal` table. Passed to `datastore local`
# as `--user_files_path` so the `IcebergLocal` access check accepts the path.
WORK_DIR="${DATASTORE_TMP}/iceberg_alter_drop_column_104295_${DATASTORE_TEST_UNIQUE_NAME}"
rm -rf "${WORK_DIR}"
mkdir -p "${WORK_DIR}"
