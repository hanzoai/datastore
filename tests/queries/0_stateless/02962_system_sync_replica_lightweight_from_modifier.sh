...
for i in $(seq $TOTAL_REPLICAS); do
    $DATASTORE_CLIENT --query "DROP TABLE IF EXISTS test_table_$i"
    $DATASTORE_CLIENT --query "CREATE TABLE test_table_$i (key UInt64, value UInt8) ENGINE = ReplicatedMergeTree('/datastore/tables/$DATASTORE_TEST_ZOOKEEPER_PREFIX/test_table', '$i') ORDER BY key SETTINGS old_parts_lifetime=1"
done
...
function insert_thread() {
    ...
    while [ $SECONDS -lt "$TIMELIMIT" ]; do
        ...
        with_lock test_table_$REPLICA $DATASTORE_CLIENT --query "INSERT INTO test_table_$REPLICA VALUES ($RANDOM, $RANDOM % 255)"
        sleep 0.$RANDOM
    done
}
...
function sync_and_drop_replicas() {
    ...
    while [ $SECONDS -lt "$TIMELIMIT" ]; do
        for i in $(seq $REPLICAS_TO_DROP); do
            ...
            with_lock test_table_$i $DATASTORE_CLIENT --query "
...
function optimize_thread() {
    ...
    while [ $SECONDS -lt "$TIMELIMIT" ]; do
        REPLICA=$(($RANDOM % $TOTAL_REPLICAS + 1))
        $DATASTORE_CLIENT --query "OPTIMIZE TABLE test_table_$REPLICA FINAL"
        sleep 0.$RANDOM
    done
}
...
function mutations_thread() {
    ...
    while [ $SECONDS -lt "$TIMELIMIT" ]; do
        REPLICA=$(($RANDOM % $TOTAL_REPLICAS + 1))
        CONDITION="key % 2 = 0"
        $DATASTORE_CLIENT --query "ALTER TABLE test_table_$REPLICA DELETE WHERE $CONDITION"
        sleep 0.$RANDOM
    done
}
...
function consistency_table_sync_non_existent_replica() {
    ...
    SYNC_OUTPUT=$($DATASTORE_CLIENT --query "SYSTEM SYNC REPLICA test_table_1 LIGHTWEIGHT FROM '$NON_EXISTENT_REPLICA'" 2>&1)
    EXIT_CODE=$?
...
lost_parts_count=$($DATASTORE_CLIENT --query "SELECT SUM(lost_part_count) FROM system.replicas WHERE database=currentDatabase()")
