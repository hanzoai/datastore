...
query_id=${DATASTORE_DATABASE}_optimize_read_in_order_from_view_$RANDOM$RANDOM

$DATASTORE_CLIENT -q "$query" --query_id="$query_id" --log_queries=1 --optimize_read_in_order=1

$DATASTORE_CLIENT -q "SYSTEM FLUSH LOGS query_log"

read_rows=$($DATASTORE_CLIENT -q "SELECT read_rows FROM system.query_log WHERE event_date >= yesterday() AND event_time >= now() - 600 AND current_database = currentDatabase() AND query_id='${query_id}' AND type='QueryFinish'")
...
query_plan=$($DATASTORE_CLIENT -q "EXPLAIN actions=1 $query" --optimize_read_in_order=1)

echo "$query_plan" | grep -A 1 "ReadFromMergeTree" | sed 's/^[ \t]*//'

$DATASTORE_CLIENT -q "DROP TABLE view1"
