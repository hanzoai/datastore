...
create_optimize_partition() {
    ...
    """
    cnt=$(${DATASTORE_CLIENT} -q "SELECT count() FROM system.parts WHERE database = currentDatabase() AND table = 'with_lonely' AND partition = '$1' AND active")
    while [ "$cnt" -ne "1" ]; do
        ${DATASTORE_CLIENT} -q "OPTIMIZE TABLE with_lonely PARTITION '$1' FINAL;"
        cnt=$(${DATASTORE_CLIENT} -q "SELECT count() FROM system.parts WHERE database = currentDatabase() AND table = 'with_lonely' AND partition = '$1' AND active")
    done
}
...
DATASTORE_CLIENT="${DATASTORE_CLIENT} --do_not_merge_across_partitions_select_final 1"

# mix lonely parts and non-lonely parts
${DATASTORE_CLIENT} --max_bytes_to_read 2000 -q "SELECT max(val), count(*) FROM with_lonely FINAL;"
# only lonely parts
${DATASTORE_CLIENT} --max_bytes_to_read 1000 -q "SELECT max(val), count(*) FROM with_lonely FINAL WHERE dt < '2022-11-01';"
# only lonely parts but max_thread = 1, so reading lonely parts with in-order
${DATASTORE_CLIENT} --max_threads 1 --max_bytes_to_read 1000 -q "SELECT max(val), count(*) FROM with_lonely FINAL WHERE dt < '2022-11-01';"
