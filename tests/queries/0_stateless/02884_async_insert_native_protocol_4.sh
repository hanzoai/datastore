...
echo "(1)" | $DATASTORE_CLIENT_WITH_LOG $async_insert_options --async_insert_max_data_size 10 \
     -q 'INSERT INTO t_async_insert_native_4 FORMAT Values' 2>&1 \
     | grep -c "too much data"

echo "(2) (3) (4) (5)" | $DATASTORE_CLIENT_WITH_LOG $async_insert_options --async_insert_max_data_size 10 \
    -q 'INSERT INTO t_async_insert_native_4 FORMAT Values' 2>&1 \
    | grep -c "too much data"
...
$DATASTORE_CLIENT -q "
    ...
    WHERE event_date >= yesterday() AND event_time >= now() - 600 AND database = '$DATASTORE_DATABASE' AND table = 't_async_insert_native_4'
    ORDER BY format;
