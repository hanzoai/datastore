...
ENGINE = ReplicatedVersionedCollapsingMergeTree('/datastore/{database}/versioned_collapsing_table/{shard}', '{replica}', sign, version)
PARTITION BY d
ORDER BY (key1, key2);

INSERT INTO versioned_collapsing_table VALUES (toDate('2019-10-10'), 1, 1, 'Hello', -1, 1);

SELECT value FROM system.zookeeper WHERE path = '/datastore/' || currentDatabase() || '/versioned_collapsing_table/s1' and name = 'metadata';
