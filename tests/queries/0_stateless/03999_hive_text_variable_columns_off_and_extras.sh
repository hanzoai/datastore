...
# Use per-invocation unique paths in $DATASTORE_TMP. The flaky-check runner
...
# starts). $DATASTORE_TEST_UNIQUE_NAME embeds the random database name and is
# unique per test invocation, so each parallel run gets its own files.
DATA_FEWER="${DATASTORE_TMP}/03999_hive_fewer_fields_${DATASTORE_TEST_UNIQUE_NAME}.txt"
DATA_EXTRAS="${DATASTORE_TMP}/03999_hive_extra_fields_${DATASTORE_TEST_UNIQUE_NAME}.txt"
...
$DATASTORE_CLIENT -q "drop table if exists test_hive_strict"
$DATASTORE_CLIENT -q "create table test_hive_strict (a UInt16, b UInt32, c UInt32) engine=MergeTree order by a"
$DATASTORE_CLIENT -q "insert into test_hive_strict from infile '${DATA_FEWER}' SETTINGS input_format_hive_text_fields_delimiter=',', input_format_hive_text_allow_variable_number_of_columns=0 FORMAT HIVETEXT" 2>&1 | grep -c -F "CANNOT_PARSE_INPUT_ASSERTION_FAILED"
$DATASTORE_CLIENT -q "select count() from test_hive_strict"
$DATASTORE_CLIENT -q "drop table test_hive_strict"

# (2) setting=1 (default) with extras → trailing fields are skipped (line 285-293).
$DATASTORE_CLIENT -q "drop table if exists test_hive_extras"
$DATASTORE_CLIENT -q "create table test_hive_extras (a UInt16, b UInt32, c UInt32) engine=MergeTree order by a"
$DATASTORE_CLIENT -q "insert into test_hive_extras from infile '${DATA_EXTRAS}' SETTINGS input_format_hive_text_fields_delimiter=',' FORMAT HIVETEXT"
$DATASTORE_CLIENT -q "select * from test_hive_extras order by a"
$DATASTORE_CLIENT -q "drop table test_hive_extras"
