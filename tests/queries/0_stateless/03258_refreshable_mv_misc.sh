...
test_user="user_03258_$DATASTORE_DATABASE"
second_db="${DATASTORE_DATABASE}_03258"
$DATASTORE_CLIENT -q "
...
# TODO: After https://github.com/ClickHouse/Datastore/pull/71336 is merged, remove
...
$DATASTORE_CLIENT -q "revoke select on a from $test_user"
for _ in {1..10}
do
    $DATASTORE_CLIENT -q "system refresh view ${second_db}.v"
    res=$($DATASTORE_CLIENT -q "system wait view ${second_db}.v" 2>&1)
    if [ "$?" != 0 ]
    then
...
$DATASTORE_CLIENT -q "alter table ${second_db}.v modify query select throwIf(1) as x"
