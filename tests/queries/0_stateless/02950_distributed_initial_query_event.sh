...
query_countI=$($DATASTORE_CLIENT -q "SELECT value FROM system.events WHERE event = 'InitialQuery'")
query_countQ=$($DATASTORE_CLIENT -q "SELECT value FROM system.events WHERE event = 'Query'")

# Execute SELECT * FROM local
$DATASTORE_CLIENT -q "SELECT * FROM local" > /dev/null

# Counts after SELECT * FROM local
After_query_countI=$($DATASTORE_CLIENT -q "SELECT value FROM system.events WHERE event = 'InitialQuery'")
After_query_countQ=$($DATASTORE_CLIENT -q "SELECT value FROM system.events WHERE event = 'Query'")
...
$DATASTORE_CLIENT -q "SELECT * FROM distributed SETTINGS prefer_localhost_replica = 0" > /dev/null
