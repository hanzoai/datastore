...
$DATASTORE_CLIENT --receive_timeout=1 --interactive_delay=10000000 --query="SELECT sleep(3) FORMAT Null" >/dev/null 2>&1
echo $?

# Verify the timeout error message is correctly printed (capture stderr)
$DATASTORE_CLIENT --receive_timeout=1 --interactive_delay=10000000 --query="SELECT sleep(3) FORMAT Null" 2>&1 | grep -o "Timeout exceeded"
