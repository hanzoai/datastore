...
ENGINE = KeeperMap('/$table_name/$DATASTORE_DATABASE')
PRIMARY KEY(key)"
...
function assert_children_size()
{
    for _ in `seq 10`
    do
        children_size=$($DATASTORE_CLIENT --query="SELECT count() FROM system.zookeeper WHERE path = '$1'")
        if [ $children_size == $2 ]
        then
            return
        fi
...
{
    assert_children_size "/test_keeper_map/02704_keeper_map_zk_nodes/$DATASTORE_DATABASE" $1
}

function assert_data_children_size()
{
    assert_children_size "/test_keeper_map/02704_keeper_map_zk_nodes/$DATASTORE_DATABASE/data" $1
}

assert_root_children_size 2
assert_data_children_size 0

$DATASTORE_CLIENT --query="INSERT INTO $table_name VALUES (1, 11)"
...
$DATASTORE_CLIENT --query="INSERT INTO $table_name_another VALUES (1, 11)"

assert_root_children_size 2
assert_data_children_size 1

$DATASTORE_CLIENT --query="INSERT INTO $table_name_another VALUES (2, 22)"
...
$DATASTORE_CLIENT --query="DROP TABLE $table_name_another"
