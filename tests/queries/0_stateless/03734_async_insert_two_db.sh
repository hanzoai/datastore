...
CH_CLIENT=$(echo ${DATASTORE_CLIENT} | sed 's/'"--database=${DATASTORE_DATABASE}"'//g')
...
db1="test_async_insert_two_db_1_$DATASTORE_DATABASE"
db3="test_async_insert_two_db_2_$DATASTORE_DATABASE"
...
for db in $db1 $db3; do
    echo "Creating database ${db%$DATASTORE_DATABASE}"
    $DATASTORE_CLIENT --query "DROP DATABASE IF EXISTS $db"
    $DATASTORE_CLIENT --query "CREATE DATABASE IF NOT EXISTS $db"
    echo "Creating table in database ${db%$DATASTORE_DATABASE}"
    $CH_CLIENT --database=$db --query "$create_query"
done

for db in $db1 $db3; do
    echo "Inserting data into database ${db%$DATASTORE_DATABASE}"
