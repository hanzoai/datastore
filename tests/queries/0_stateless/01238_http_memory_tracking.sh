...
${DATASTORE_CLIENT} --format Null -q "CREATE USER $MISTER_USER"
...
${DATASTORE_CLIENT} --user ${MISTER_USER} --function_sleep_max_microseconds_per_block 5000000 --format Null <<<'SELECT sleepEachRow(1) FROM numbers(5)' &

# ignore "yes: standard output: Broken pipe"
yes 'SELECT 1' 2>/dev/null | {
    head -n1000
} | {
    xargs -I{} ${DATASTORE_CURL} -sS "${DATASTORE_URL}&user=${MISTER_USER}&http_wait_end_of_query=1&max_memory_usage_for_user=$((1<<30))" -d '{}'
} | grep -x -c 1

wait

# Reset max_memory_usage_for_user, so it will not affect other tests
${DATASTORE_CLIENT} --user ${MISTER_USER} --max_memory_usage_for_user=0 -q "SELECT 1 FORMAT Null"
