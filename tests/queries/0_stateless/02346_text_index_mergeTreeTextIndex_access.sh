...
user_name="${DATASTORE_DATABASE}_test_user_03917"
...
REVOKE SELECT ON $DATASTORE_DATABASE.tab FROM $user_name;
"

function check_access()
{
    local output
    output=$($DATASTORE_CLIENT --user "$user_name" --password "password" -q "$1" 2>&1)
    local rc=$?
    if [ $rc -eq 0 ]; then
        echo "OK"
...
$DATASTORE_CLIENT -q "GRANT SELECT (b) ON $DATASTORE_DATABASE.tab TO $user_name;"
...
## Row policy tests
$DATASTORE_CLIENT -q "GRANT SELECT ON $DATASTORE_DATABASE.tab TO $user_name;"

# Row policy on column `a`: idx_a and idx_ab denied, idx_b allowed
$DATASTORE_CLIENT -q "CREATE ROW POLICY p1_03917 ON $DATASTORE_DATABASE.tab FOR SELECT USING a = 'hello' TO $user_name;"

check_access "SELECT * FROM mergeTreeTextIndex(currentDatabase(), tab, idx_a)"
check_access "SELECT * FROM mergeTreeTextIndex(currentDatabase(), tab, idx_b)"
check_access "SELECT * FROM mergeTreeTextIndex(currentDatabase(), tab, idx_ab)"

$DATASTORE_CLIENT -q "DROP ROW POLICY p1_03917 ON $DATASTORE_DATABASE.tab;"
