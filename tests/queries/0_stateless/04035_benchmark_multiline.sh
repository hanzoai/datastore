...
echo "SELECT
1" | ${DATASTORE_BENCHMARK} --iterations 1 --concurrency 1 --queries-format=script \
...
2;
SELECT
3" | ${DATASTORE_BENCHMARK} --iterations 1 --concurrency 1 --queries-format=script \
    2>&1 | grep -q "Loaded 3 queries" && echo "OK: Multiple multiline queries"

${DATASTORE_CLIENT} --query "CREATE TABLE IF NOT EXISTS t (a UInt8) engine = Memory"

echo "INSERT INTO t FORMAT Values (1), (2);
SELECT 1;" | ${DATASTORE_BENCHMARK} --iterations 1 --concurrency 1 --queries-format=script \
...
1;
SELECT /* block comment; with semicolon */ 2" | ${DATASTORE_BENCHMARK} --iterations 1 --concurrency 1 --queries-format=script \
...
echo "SELECT 1
SELECT 2" | ${DATASTORE_BENCHMARK} --iterations 1 --concurrency 1 \
...
echo "SELECT 1
SELECT 2" | ${DATASTORE_BENCHMARK} --iterations 1 --concurrency 1 --queries-format=tsv \
    2>&1 | grep -q "Loaded 2 queries" && echo "OK: Explicit tsv mode"

# Unknown value must be rejected
echo "SELECT 1" | ${DATASTORE_BENCHMARK} --iterations 1 --concurrency 1 --queries-format=bogus \
