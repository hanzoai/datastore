...
echo '{"id": 1, "s": "aaa"} {"id": 2, "s": "bbb"}' | $DATASTORE_CLIENT $async_insert_options -q 'INSERT INTO t_async_insert_native_2 FORMAT JSONEachRow' &
echo "(3, 'ccc') (4, 'ddd') (5, 'eee')" | $DATASTORE_CLIENT $async_insert_options -q 'INSERT INTO t_async_insert_native_2 FORMAT Values' &
...
$DATASTORE_CLIENT -q "
    ...
    WHERE event_date >= yesterday() AND event_time >= now() - 600 AND database = '$DATASTORE_DATABASE' AND table = 't_async_insert_native_2'
    ORDER BY format;
