...
FILE_NAME="data.tmp"
FORM_DATA="${USER_FILES_PATH}/${DATASTORE_TEST_UNIQUE_NAME}/${FILE_NAME}"
mkdir -p ${USER_FILES_PATH}/${DATASTORE_TEST_UNIQUE_NAME}/
touch $FORM_DATA

# Simple tests
echo -ne "col1=42&col2=Hello%2C%20World%21" > $FORM_DATA
$DATASTORE_CLIENT -q "SELECT * from file('$FORM_DATA', Form, 'col1 UInt64, col2 String')"
$DATASTORE_CLIENT -q "SELECT * from file('$FORM_DATA', Form, 'col2 String')"
rm $FORM_DATA

# Schema reader test
touch $FORM_DATA
echo -ne "col1=42&col2=Hello%2C%20World%21&col3=%5B1%2C%202%2C%203%5D" > $FORM_DATA
$DATASTORE_CLIENT -q "DESC file('$FORM_DATA', Form)"
$DATASTORE_CLIENT -q "SELECT * FROM file('$FORM_DATA', Form)"
rm $FORM_DATA
...
$DATASTORE_CLIENT -q "SELECT * FROM file('$FORM_DATA', Form) FORMAT Vertical"
