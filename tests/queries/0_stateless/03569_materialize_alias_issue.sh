...
$DATASTORE_CLIENT --query "DROP TABLE IF EXISTS customer_dbt_materialize"

$DATASTORE_CLIENT -n --query "CREATE TABLE customer_dbt_materialize(
    key UInt64,
...
$DATASTORE_CLIENT --query "INSERT INTO customer_dbt_materialize SELECT number FROM numbers(1000)"
...
$DATASTORE_CLIENT --query "ALTER TABLE customer_dbt_materialize MODIFY COLUMN value Array(Tuple(transaction_hash String, instruction_sig_hash String)) ALIAS array((toString(key), toString(key))) SETTINGS mutations_sync = 2"

$DATASTORE_CLIENT --query "ALTER TABLE customer_dbt_materialize MODIFY COLUMN value Array(Tuple(transaction_hash String, transaction_index_data String)) MATERIALIZED array((toString(key), toString(key)))  SETTINGS mutations_sync = 2"

$DATASTORE_CLIENT --query "ALTER TABLE customer_dbt_materialize MATERIALIZE COLUMN value SETTINGS mutations_sync = 2, alter_sync = 2"

$DATASTORE_CLIENT --query "SYSTEM RESTART REPLICA customer_dbt_materialize"

$DATASTORE_CLIENT --query "CHECK TABLE customer_dbt_materialize SETTINGS check_query_single_value_result = 1"
