...
server_path=$(${DATASTORE_CLIENT} -q "SELECT path FROM system.disks WHERE name = 'default'")
...
$DATASTORE_CLIENT "${settings[@]}" -nm -q "
    ...
    SETTINGS disk = disk(
        ...
        max_size = '10Mi',
        path = '${DATASTORE_TEST_UNIQUE_NAME}/',
        disk = disk(type='local_blob_storage', path='${server_path}/disks/${DATASTORE_TEST_UNIQUE_NAME}/'));
