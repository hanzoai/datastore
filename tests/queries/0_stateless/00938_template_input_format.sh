...
CURDIR=$CURDIR/${DATASTORE_DATABASE}
mkdir -p $CURDIR

$DATASTORE_CLIENT --query="DROP TABLE IF EXISTS template1";
$DATASTORE_CLIENT --query="DROP TABLE IF EXISTS template2";
$DATASTORE_CLIENT --query="CREATE TABLE template1 (s1 String, s2 String, s3 String, s4 String, n UInt64, d Date) ENGINE = Memory";
$DATASTORE_CLIENT --query="CREATE TABLE template2 (s1 String, s2 String, s3 String, s4 String, n UInt64, d Date) ENGINE = Memory";
...
n:	789, s1:	zx\\ncv\\tbn m	, s2:	'qwe,rty', s3:	\"as\\\"df'gh\", s4:	\"\", d:	2016-01-04"$'\t'"
 $ suffix $" | $DATASTORE_CLIENT --query="INSERT INTO template1 SETTINGS \
...
$DATASTORE_CLIENT --query="SELECT * FROM template1 ORDER BY n FORMAT CSV";
...
$DATASTORE_CLIENT --query="SELECT * FROM template1 ORDER BY n FORMAT JSON" | $DATASTORE_CLIENT --query="INSERT INTO template2 SETTINGS \
format_template_resultset = '$CURDIR/00938_template_input_format_resultset.tmp', \
format_template_row = '$CURDIR/00938_template_input_format_row.tmp', \
format_template_rows_between_delimiter = ',' \
FORMAT TemplateIgnoreSpaces";

$DATASTORE_CLIENT --query="SELECT * FROM template2 ORDER BY n FORMAT CSV";
$DATASTORE_CLIENT --query="TRUNCATE TABLE template2";
...
$DATASTORE_CLIENT --query="TRUNCATE TABLE template1";
...
$DATASTORE_CLIENT --query="DROP TABLE template1";
$DATASTORE_CLIENT --query="DROP TABLE template2";
...
echo -ne "a,b\nc,d\n" | $DATASTORE_LOCAL --structure "a String, b String"  --input-format Template \
...
echo -ne "a,b\nc,d\n" | $DATASTORE_LOCAL --structure "a String"  --input-format Template \
    --format_template_row "$CURDIR"/00938_template_input_format_row.tmp --format_template_rows_between_delimiter '' \
    -q 'select * from table' 2>&1| grep -Fac "'Escaped' serialization requires delimiter"
