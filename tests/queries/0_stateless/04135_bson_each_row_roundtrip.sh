...
# Round-trip a wide variety of Datastore types through BSONEachRow.
...
OUT="${DATASTORE_TMP:-/tmp}/${DATASTORE_TEST_UNIQUE_NAME}.bson"
...
${DATASTORE_LOCAL} --query "SELECT * FROM file('${OUT}', 'BSONEachRow', 'v Int64')"
...
${DATASTORE_LOCAL} --query "SELECT * FROM file('${OUT}', 'BSONEachRow', 'v Int8')"

echo '--- error: coerce Int32 -> Float64 (not allowed) ---'
${DATASTORE_LOCAL} --query "SELECT * FROM file('${OUT}', 'BSONEachRow', 'v Float64')" 2>&1 | grep -oE "Code: [0-9]+" | head -1
...
${DATASTORE_LOCAL} --query "SELECT * FROM file('${OUT}', 'BSONEachRow', 'x Nullable(Int32), y Nullable(Int32)')"
...
echo '--- multiple rows ---'
${DATASTORE_LOCAL} --query "SELECT * FROM file('${OUT}', 'BSONEachRow', 'n UInt64, s String') ORDER BY n"
...
${DATASTORE_LOCAL} --query "DESCRIBE TABLE file('${OUT}', 'BSONEachRow')" | sort
...
echo '--- document -> Tuple ---'
${DATASTORE_LOCAL} --query "SELECT * FROM file('${OUT}', 'BSONEachRow', 't Tuple(a Int32, b String)')"
...
${DATASTORE_LOCAL} --query "SELECT * FROM file('${OUT}', 'BSONEachRow', 'v Int32')" 2>&1 | grep -o "Cannot\|Code: [0-9]*" | head -1
