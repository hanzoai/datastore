...
$DATASTORE_CLIENT --query="INSERT INTO test_constraints VALUES (3, 4), (1, 0), (3, 4), (6, 0);" 2>&1 \
...
$DATASTORE_CLIENT --query="INSERT INTO test_constraints VALUES (1, 2);" 2>&1 \
    | grep -q "$EXCEPTION_TEXT" && echo "$EXCEPTION_SUCCESS_TEXT" || echo "Did not thrown an exception"
$DATASTORE_CLIENT --query="SELECT * FROM test_constraints;"

# This one  must throw an exception
$DATASTORE_CLIENT --query="INSERT INTO test_constraints VALUES (5, 16), (10, 11), (9, 11), (8, 12);" 2>&1 \
    | grep -q "$EXCEPTION_TEXT" && echo "$EXCEPTION_SUCCESS_TEXT" || echo "Did not thrown an exception"
$DATASTORE_CLIENT --query="SELECT * FROM test_constraints;"

# This one must succeed
$DATASTORE_CLIENT --query="INSERT INTO test_constraints VALUES (7, 18), (0, 11);"
$DATASTORE_CLIENT --query="SELECT * FROM test_constraints;"
