...
CREATE TABLE replicated_with_sampling(x UInt8)
    ENGINE ReplicatedMergeTree('/datastore/tables/{database}/test_00509/replicated_with_sampling', 'r1')
    ORDER BY x
    SAMPLE BY x;
...
CREATE TABLE replicated_collapsing(d Date, x UInt32, sign Int8)
    ENGINE = ReplicatedCollapsingMergeTree('/datastore/tables/{database}/test_00509/replicated_collapsing', 'r1', sign)
...
CREATE TABLE replicated_versioned_collapsing(d Date, x UInt32, sign Int8, version UInt8)
    ENGINE = ReplicatedVersionedCollapsingMergeTree('/datastore/tables/{database}/test_00509/replicated_versioned_collapsing', 'r1', sign, version)
...
CREATE TABLE with_settings(x UInt32)
    ENGINE ReplicatedMergeTree('/datastore/tables/{database}/test_00509/with_settings', 'r1')
    ORDER BY x
