...
${DATASTORE_CLIENT} --allow_nondeterministic_mutations=1 << EOF
DROP TABLE IF EXISTS mutations_r1;
DROP TABLE IF EXISTS for_subquery;

CREATE TABLE mutations_r1(x UInt32, y UInt32) ENGINE ReplicatedMergeTree('/datastore/tables/$DATASTORE_TEST_ZOOKEEPER_PREFIX/mutations', 'r1') ORDER BY x;
...
${DATASTORE_CLIENT} --query="SELECT * FROM mutations_r1"

${DATASTORE_CLIENT} --query="DROP TABLE mutations_r1"
${DATASTORE_CLIENT} --query="DROP TABLE for_subquery"
