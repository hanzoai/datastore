...
${DATASTORE_CLIENT} -q "select 1 format Null" "--query_id=$query_id"
...
for _ in $(seq 1 60); do
    ...
    count=$(${DATASTORE_CLIENT} -q "select count() from system.query_log where current_database = currentDatabase() AND query_id = '$query_id' and type = 'QueryFinish'")
    [ "$count" -ge 2 ] && break
    sleep 0.5
done
