...
MY_DATASTORE_CLIENT="$DATASTORE_CLIENT --enable_parsing_to_custom_serialization 1 --parallel_replicas_for_cluster_engines 0 --optimize_trivial_insert_select 0 --server_logs_file=/dev/null"
...
filename="test_data_sparse_$DATASTORE_DATABASE.json"
...
for _ in {1..4}; do
    $MY_DATASTORE_CLIENT --query "INSERT INTO t_reference SELECT * FROM file('$filename', JSONEachRow)"
done;

$MY_DATASTORE_CLIENT --query "INSERT INTO t_insert_mem SELECT * FROM file('$filename', JSONEachRow)"
$MY_DATASTORE_CLIENT --query "INSERT INTO t_insert_mem SELECT * FROM file('$filename', JSONEachRow)"

$MY_DATASTORE_CLIENT --query "DETACH TABLE t_insert_mem"
$MY_DATASTORE_CLIENT --query "ATTACH TABLE t_insert_mem"

$MY_DATASTORE_CLIENT --query "INSERT INTO t_insert_mem SELECT * FROM s3(s3_conn, filename='$filename', format='JSONEachRow')"
$MY_DATASTORE_CLIENT --query "SELECT * FROM file('$filename', LineAsString) FORMAT LineAsString" | ${DATASTORE_CURL} -sS --max-time 240 "${DATASTORE_URL}&query=INSERT+INTO+t_insert_mem+FORMAT+JSONEachRow&enable_parsing_to_custom_serialization=1" --data-binary @-
...
$MY_DATASTORE_CLIENT --query "
    ...
    WHERE table = 't_insert_mem' AND database = '$DATASTORE_DATABASE'
...
for _ in $(seq 1 60); do
    $MY_DATASTORE_CLIENT --query "SYSTEM FLUSH LOGS query_log"
    count=$($MY_DATASTORE_CLIENT --query "SELECT count() FROM system.query_log WHERE event_date >= yesterday() AND event_time >= now() - 600 AND query LIKE 'INSERT INTO t_insert_mem%' AND current_database = '$DATASTORE_DATABASE' AND type = 'QueryFinish'")
    [ "$count" -ge 4 ] && break
    sleep 0.5
done
...
$MY_DATASTORE_CLIENT --query "
    ...
    WHERE event_date >= yesterday() AND event_time >= now() - 600 AND query LIKE 'INSERT INTO t_insert_mem%' AND current_database = '$DATASTORE_DATABASE' AND type = 'QueryFinish'
