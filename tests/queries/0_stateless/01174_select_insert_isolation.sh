...
LAST_ITERATION_FILE_1="${DATASTORE_TMP}/last_iteration_1_$$"
LAST_ITERATION_FILE_2="${DATASTORE_TMP}/last_iteration_2_$$"
touch $LAST_ITERATION_FILE_1
touch $LAST_ITERATION_FILE_2

$DATASTORE_CLIENT --query "DROP TABLE IF EXISTS mt"
$DATASTORE_CLIENT --query "CREATE TABLE mt (n Int8, m Int8) ENGINE=MergeTree ORDER BY n PARTITION BY 0 < n SETTINGS old_parts_lifetime=0"
...
function thread_insert_commit()
{
    for i in {1..50}; do
        ...
        INSERT INTO mt VALUES /* (-$i, $1) */ (-$i, $1);
        COMMIT;"
        echo $i > "${DATASTORE_TMP}/last_iteration_${1}_$$"
    done
}
