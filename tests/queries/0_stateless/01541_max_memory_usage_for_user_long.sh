...
${DATASTORE_CLIENT} -n --allow_introspection_functions 1 --query "DROP USER IF EXISTS test_01541; CREATE USER test_01541; GRANT ALL ON *.* TO test_01541;";
...
{
    for _ in {1..100}; do
        $DATASTORE_CURL -sS "$DATASTORE_URL&user=test_01541&max_memory_usage_for_user=100Mi&max_threads=1" -d@- <<<"$query" | grep -F DB::Exception:
    done
}
function execute_tcp()
{
    ...
    for _ in {1..20}; do
        ${DATASTORE_CLIENT} --user=test_01541 --max_memory_usage_for_user=100Mi --max_threads=1 -q "$query" | grep -F DB::Exception:
    done
}
function execute_tcp_one_session()
{
    for _ in {1..30}; do
        echo "$query;"
    done | ${DATASTORE_CLIENT} --user=test_01541 -nm --max_memory_usage_for_user=100Mi --max_threads=1 | grep -F DB::Exception:
}
...
${DATASTORE_CLIENT} --user=test_01541 --max_block_size=1 --format Null --query_id $query_id -q 'SELECT sleepEachRow(1) FROM numbers(600)' &
# trap
sleep_query_pid=$!
...
function cleanup()
{
    ...
    while ${DATASTORE_CLIENT} -q "SELECT query_id FROM system.processes WHERE query_id = '$query_id'" | grep -xq "$query_id"; do
        sleep 0.1
    done
}
trap cleanup EXIT
...
${DATASTORE_CLIENT} --query "DROP USER test_01541";

# Reset max_memory_usage_for_user, so it will not affect other tests
${DATASTORE_CLIENT} --max_memory_usage_for_user=0 -q "SELECT 1 FORMAT Null"
