...
function test()
{
    ...
    # Follow https://github.com/ClickHouse/Datastore/issues/17317 for more info about the issue
    where=$1

    uuid_1=$(cat /proc/sys/kernel/random/uuid)
    $DATASTORE_CLIENT --max_execution_time 300 --query="SELECT uniq(15Id) FROM test_extract $where SETTINGS max_threads=1" --query_id=$uuid_1
    uuid_2=$(cat /proc/sys/kernel/random/uuid)
    $DATASTORE_CLIENT --max_execution_time 300 --query="SELECT uniq($sql) FROM test_extract $where SETTINGS max_threads=1" --query_id=$uuid_2
    $DATASTORE_CLIENT --query="
