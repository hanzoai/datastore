...
$DATASTORE_CLIENT -q "DROP TABLE IF EXISTS cannot_kill_query"
$DATASTORE_CLIENT -q "CREATE TABLE cannot_kill_query (x UInt64) ENGINE = MergeTree ORDER BY x" &> /dev/null
$DATASTORE_CLIENT -q "INSERT INTO cannot_kill_query SELECT * FROM numbers(10000000)" &> /dev/null
...
$DATASTORE_CLIENT -q "$query_for_pending" &>/dev/null &
...
$DATASTORE_CLIENT -q "ALTER TABLE cannot_kill_query MODIFY COLUMN x UInt64" &>/dev/null &
...
$DATASTORE_CLIENT -q "$query_to_kill" &>/dev/null &
...
$DATASTORE_CLIENT -q "KILL QUERY WHERE query='$query_to_kill' ASYNC" &>/dev/null

sleep 1

# Kill $query_for_pending SYNC. This query is not blocker, so it should be killed fast.
timeout 20 ${DATASTORE_CLIENT} -q "KILL QUERY WHERE query='$query_for_pending' SYNC" &>/dev/null

# Both queries have to be killed, doesn't matter with SYNC or ASYNC kill
for _ in {1..15}
do
    sleep 1
    no_first_query=$($DATASTORE_CLIENT -q "SELECT count() FROM system.processes where query='$query_for_pending'")
    no_second_query=$($DATASTORE_CLIENT -q "SELECT count() FROM system.processes where query='$query_to_kill'")
...
$DATASTORE_CLIENT -q "DROP TABLE IF EXISTS cannot_kill_query" &>/dev/null
