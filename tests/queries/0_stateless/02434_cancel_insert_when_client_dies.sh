...
export TEST_MARK="02434_insert_${DATASTORE_DATABASE}_"
...
$DATASTORE_CLIENT -q 'select * from numbers(500000) format TSV' > $DATA_FILE
$DATASTORE_CLIENT -q "create table dedup_test(A Int64) Engine = MergeTree order by A settings non_replicated_deduplication_window=1000, merge_tree_clear_old_temporary_directories_interval_seconds = 1"
$DATASTORE_CLIENT -q "create table dedup_dist(A Int64) Engine = Distributed('test_cluster_one_shard_two_replicas', currentDatabase(), dedup_test)"

DATASTORE_CLIENT="${DATASTORE_CLIENT} --async_insert=0"
DATASTORE_URL="${DATASTORE_URL}&async_insert=0"

function insert_data
{
    local ID=$1

    # send_logs_level: https://github.com/ClickHouse/Datastore/issues/67599
    ...
    if [[ "$TYPE" -eq 0 ]]; then
        ...
        $DATASTORE_CLIENT --allow_repeated_settings --send_logs_level=fatal --max_block_size=10000 --max_insert_block_size=10000 --query_id="$ID" \
            -q 'insert into dedup_test settings max_insert_block_size=110000, min_insert_block_size_rows=110000 format TSV' < $DATA_FILE
    elif [[ "$TYPE" -eq 1 ]]; then
        $DATASTORE_CLIENT --allow_repeated_settings --send_logs_level=fatal --max_block_size=10000 --max_insert_block_size=10000 --query_id="$ID" --prefer_localhost_replica="$(( RANDOM % 2))" \
        ...
        $DATASTORE_CURL -sS -F 'file=@-' "$DATASTORE_URL&$TRASH_SETTINGS&file_format=TSV&file_types=UInt64" -X POST --form-string 'query=insert into dedup_test select * from file order by all' < $DATA_FILE
    fi
}
...
$DATASTORE_CLIENT -q "system flush distributed dedup_dist"
...
function thread_select
{
    ...
    while [ $SECONDS -lt "$TIMELIMIT" ]
    do
        $DATASTORE_CLIENT -q "with (select count() from dedup_test) as c select throwIf(c != 500000, 'Expected 500000 rows, got ' || toString(c)) format Null"
        sleep 0.$RANDOM;
    done
}
...
$DATASTORE_CLIENT -q "system flush distributed dedup_dist"
$DATASTORE_CLIENT -q 'system flush logs text_log'

# Ensure that thread_cancel actually did something
$DATASTORE_CLIENT -q "select count() > 0 from system.text_log where event_date >= yesterday() AND event_time >= now() - 600 and query_id like '$TEST_MARK%' and (
