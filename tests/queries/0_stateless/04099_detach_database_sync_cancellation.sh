...
DB_NAME="${DATASTORE_DATABASE}_detach_cancel"
LONG_QUERY_ID="long_select_detach_${DATASTORE_DATABASE}_$$"
DETACH_QUERY_ID="detach_db_cancel_${DATASTORE_DATABASE}_$$"
...
function cleanup()
{
    ...
    $DATASTORE_CLIENT --query "KILL QUERY WHERE query_id = '${DETACH_QUERY_ID}' SYNC FORMAT Null" 2>/dev/null ||:
    wait 2>/dev/null ||:
    # Database may or may not exist depending on where we failed
    $DATASTORE_CLIENT --query "ATTACH DATABASE IF NOT EXISTS ${DB_NAME}" 2>/dev/null ||:
...
for _ in $(seq 1 60); do
    result=$($DATASTORE_CLIENT --query "SELECT count() FROM system.processes WHERE query_id = '${DETACH_QUERY_ID}'")
    [ "$result" = "1" ] && break
    sleep 0.1
done
if [ "$result" != "1" ]; then
...
$DATASTORE_CLIENT --query "KILL QUERY WHERE query_id = '${DETACH_QUERY_ID}' SYNC FORMAT Null"
