...
$DATASTORE_CLIENT -q "drop table if exists data_01882"
$DATASTORE_CLIENT -q "create table data_01882 (key Int) Engine=MergeTree() partition by key order by key as select * from numbers(10)"
...
for _ in {1..30}; do
    $DATASTORE_CURL -vsS "${DATASTORE_URL}&max_threads=1&default_format=Null&send_progress_in_http_headers=1&http_headers_progress_interval_ms=1" --data-binary @- <<< "select * from data_01882" |& {
