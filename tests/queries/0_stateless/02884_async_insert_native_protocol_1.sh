...
echo '{"id": 1, "s": "aaa"} {"id": 2, "s": "bbb"}' | $DATASTORE_CLIENT $async_insert_options -q 'INSERT INTO t_async_insert_native_1 FORMAT JSONEachRow'
$DATASTORE_CLIENT $async_insert_options  -q 'INSERT INTO t_async_insert_native_1 FORMAT JSONEachRow {"id": 3, "s": "ccc"}'

# Mixed inlined and external data is not supported.
echo '{"id": 1, "s": "aaa"}' \
    | $DATASTORE_CLIENT $async_insert_options -q 'INSERT INTO t_async_insert_native_1 FORMAT JSONEachRow {"id": 2, "s": "bbb"}' 2>&1 \
    | grep -o "NOT_IMPLEMENTED"

$DATASTORE_CLIENT -q "
    SELECT sum(length(entries.bytes)) FROM system.asynchronous_inserts
    WHERE database = '$DATASTORE_DATABASE' AND table = 't_async_insert_native_1';
    ...
    WHERE event_date >= yesterday() AND event_time >= now() - 600 AND database = '$DATASTORE_DATABASE' AND table = 't_async_insert_native_1'
