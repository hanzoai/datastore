...
${DATASTORE_CLIENT} --query "select *, _filename, _offset from file_log order by  _filename, _offset settings stream_like_engine_allow_direct_select=1;"
...
${DATASTORE_CLIENT} --query "select * from file_log order by k settings stream_like_engine_allow_direct_select=1;" 2>&1 | grep -q "CANNOT_READ_ALL_DATA" && echo 'OK' || echo 'FAIL'
