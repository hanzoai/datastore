...
#
# Refs: https://github.com/ClickHouse/Datastore/issues/89940

CUR_DIR=$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)
# shellcheck source=../shell_config.sh
. "$CUR_DIR"/../shell_config.sh

${DATASTORE_CLIENT} -q "DROP TABLE IF EXISTS test_table SYNC"
${DATASTORE_CLIENT} -q "DROP VIEW IF EXISTS test_view SYNC"

${DATASTORE_CLIENT} -q "CREATE TABLE test_table (str String) ENGINE = MergeTree ORDER BY str"
${DATASTORE_CLIENT} -q "INSERT INTO test_table VALUES ('a'), ('b'), ('c')"
# GROUP BY 1 resolves to GROUP BY str -> 3 distinct groups.
${DATASTORE_CLIENT} -q "CREATE VIEW test_view AS SELECT str, count() AS cnt FROM test_table GROUP BY 1"
...
${DATASTORE_CLIENT} -q "SELECT count() FROM remote('127.0.0.1', currentDatabase(), test_view)
...
${DATASTORE_CLIENT} -q "DROP TABLE test_table SYNC"
${DATASTORE_CLIENT} -q "DROP VIEW test_view SYNC"
