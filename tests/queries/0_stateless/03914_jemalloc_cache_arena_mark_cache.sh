...
$DATASTORE_CLIENT -q "DROP TABLE IF EXISTS t_cache_arena_marks"
...
before_bytes=$($DATASTORE_CLIENT -q "
...
after_bytes=$($DATASTORE_CLIENT -q "
    SELECT value FROM system.metrics WHERE metric = 'MarkCacheBytes'")
...
$DATASTORE_CLIENT -q "DROP TABLE t_cache_arena_marks"

cache_cleared=0
for _ in $(seq 1 5); do
    cleared_bytes=$($DATASTORE_CLIENT -q "
        SYSTEM DROP MARK CACHE;
        SYSTEM DROP INDEX MARK CACHE;
        SYSTEM DROP UNCOMPRESSED CACHE;
        SYSTEM DROP INDEX UNCOMPRESSED CACHE;
