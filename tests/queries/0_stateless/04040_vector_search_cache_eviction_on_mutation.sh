...
$DATASTORE_CLIENT --query "SYSTEM DROP VECTOR SIMILARITY INDEX CACHE"

# Cache must be empty after explicit drop
$DATASTORE_CLIENT --query "SELECT metric, value FROM system.metrics WHERE metric = 'VectorSimilarityIndexCacheBytes'"
...
cache_before=$($DATASTORE_CLIENT --query "SELECT value FROM system.metrics WHERE metric = 'VectorSimilarityIndexCacheBytes'")
...
$DATASTORE_CLIENT --query "ALTER TABLE tab DELETE WHERE id = 0 SETTINGS mutations_sync = 2"
...
for _ in $(seq 1 60); do
    cache_after=$($DATASTORE_CLIENT --query "SELECT value FROM system.metrics WHERE metric = 'VectorSimilarityIndexCacheBytes'")
...
$DATASTORE_CLIENT --query "DROP TABLE tab SYNC"
