...
$DATASTORE_CLIENT --query="DROP TABLE IF EXISTS 02535_custom_key_rmt";

$DATASTORE_CLIENT --query="CREATE TABLE 02535_custom_key_rmt (x String) ENGINE = ReplicatedMergeTree('/datastore/tables/$DATASTORE_TEST_ZOOKEEPER_PREFIX/test_02535', 'r1') ORDER BY x";
$DATASTORE_CLIENT --query="INSERT INTO 02535_custom_key_rmt VALUES ('Hello')";
...
$DATASTORE_CLIENT --query="DROP TABLE IF EXISTS 02535_custom_key_rmt_hash";

$DATASTORE_CLIENT --query="CREATE TABLE 02535_custom_key_rmt_hash (x String, y UInt32) ENGINE = ReplicatedMergeTree('/datastore/tables/$DATASTORE_TEST_ZOOKEEPER_PREFIX/test_02535_hash', 'r1') ORDER BY cityHash64(x)"
$DATASTORE_CLIENT --query="INSERT INTO 02535_custom_key_rmt_hash SELECT toString(number), number % 3 FROM numbers(1000)"
