...
run_integer_test() {
    query_result=$($DATASTORE_CLIENT_BINARY -q "SELECT arrayRandomSample([1,2,3], 2)")
...
run_string_test() {
    query_result=$($DATASTORE_CLIENT_BINARY -q "SELECT arrayRandomSample(['a','b','c'], 2)")
    mapfile -t sorted_result < <(echo "$query_result" | tr -d "[]'" | tr ',' '\n' | sort)
    declare -A expected_outcomes
...
run_nested_array_test() {
    query_result=$($DATASTORE_CLIENT_BINARY -q "SELECT arrayRandomSample([[7,2],[3,4],[7,6]], 2)")
...
run_higher_k_test() {
    query_result=$($DATASTORE_CLIENT_BINARY -q "SELECT arrayRandomSample([1,2,3], 5)")
    mapfile -t sorted_result < <(echo "$query_result" | tr -d '[]' | tr ',' '\n' | sort -n)
...
run_integer_with_samples_0_test() {
    query_result=$($DATASTORE_CLIENT_BINARY -q "SELECT arrayRandomSample([1,2,3], 0)")
    mapfile -t sorted_result < <(echo "$query_result" | tr -d '[]' | tr ',' '\n' | sort -n)
...
run_empty_array_with_k_test() {
    query_result=$($DATASTORE_CLIENT_BINARY -q "SELECT arrayRandomSample([], 5)")
...
run_non_unsigned_integer_k_test() {
    ...
    query_result=$($DATASTORE_CLIENT_BINARY -q "SELECT arrayRandomSample([1, 2, 3], -5)" 2>&1)
    ...
    # Test with string
    query_result=$($DATASTORE_CLIENT_BINARY -q "SELECT arrayRandomSample([1, 2, 3], 'a')" 2>&1)
    if [[ "$query_result" == *"ILLEGAL_TYPE_OF_ARGUMENT"* ]]; then
    ...
    query_result=$($DATASTORE_CLIENT_BINARY -q "SELECT arrayRandomSample([1, 2, 3], 1.5)" 2>&1)
    if [[ "$query_result" == *"ILLEGAL_TYPE_OF_ARGUMENT"* ]]; then
...
run_multi_row_scalar_k_test() {
    ...
    $DATASTORE_CLIENT_BINARY -q "DROP DATABASE IF EXISTS ${db}"
    $DATASTORE_CLIENT_BINARY -q "CREATE DATABASE ${db}"
    $DATASTORE_CLIENT_BINARY -q "CREATE TABLE ${db}.array_test (arr Array(Int32)) ENGINE = Memory"

    # Insert multi-row data into the table
    $DATASTORE_CLIENT_BINARY -q "INSERT INTO ${db}.array_test VALUES ([1, 2, 3]), ([4, 5, 6]), ([7, 8, 9])"

    # Query using arrayRandomSample function and store the result, k is scalar here (for example, 2)
    query_result=$($DATASTORE_CLIENT_BINARY -q "SELECT arrayRandomSample(arr, 2) FROM ${db}.array_test")

    # Drop the table
    $DATASTORE_CLIENT_BINARY -q "DROP DATABASE ${db}"
