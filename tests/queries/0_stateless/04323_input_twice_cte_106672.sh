...
TABLE="test_input_cte_${DATASTORE_TEST_UNIQUE_NAME}"

${DATASTORE_CLIENT} --query "DROP TABLE IF EXISTS ${TABLE}"
${DATASTORE_CLIENT} --query "CREATE TABLE ${TABLE} (id UInt64, name String) ENGINE = MergeTree() ORDER BY id"
...
echo '{"id": 1, "name": "test"}' | ${DATASTORE_CURL} -sS \
    "${DATASTORE_URL}&query=${Q1}" \
...
echo '{"id": 2, "name": "ok_single"}' | ${DATASTORE_CURL} -sS \
    "${DATASTORE_URL}&query=${Q2}" \
    --data-binary @-
...
echo '{"id": 3, "name": "ok_direct"}' | ${DATASTORE_CURL} -sS \
    "${DATASTORE_URL}&query=${Q3}" \
    --data-binary @-
...
echo '{"id": 4, "name": "ok_materialized"}' | ${DATASTORE_CURL} -sS \
    "${DATASTORE_URL}&query=${Q4}" \
    --data-binary @-

echo '--- final rows'
${DATASTORE_CLIENT} --query "SELECT id, name FROM ${TABLE} ORDER BY id"
