...
CREATE DICTIONARY {DATASTORE_DATABASE_1:Identifier}.hashed_dictionary_complex_key_simple_attributes
(
   id UInt64,
   id_key String,
...
SOURCE(DATASTORE(HOST 'localhost' PORT tcpPort() USER 'default' TABLE 'complex_key_simple_attributes_source_table' DB currentDatabase()))
...
SELECT * FROM {DATASTORE_DATABASE_1:Identifier}.hashed_dictionary_complex_key_simple_attributes ORDER BY (id, id_key);

DROP DICTIONARY {DATASTORE_DATABASE_1:Identifier}.hashed_dictionary_complex_key_simple_attributes;
...
CREATE DICTIONARY {DATASTORE_DATABASE_1:Identifier}.hashed_dictionary_complex_key_complex_attributes
(
    id UInt64,
    id_key String,
...
PRIMARY KEY id, id_key
SOURCE(DATASTORE(HOST 'localhost' PORT tcpPort() USER 'default' TABLE 'complex_key_complex_attributes_source_table' DB currentDatabase()))
LIFETIME(MIN 1 MAX 1000)
LAYOUT(COMPLEX_KEY_HASHED());
...
SELECT 'select all values as input stream';
SELECT * FROM {DATASTORE_DATABASE_1:Identifier}.hashed_dictionary_complex_key_complex_attributes ORDER BY (id, id_key);

DROP DICTIONARY {DATASTORE_DATABASE_1:Identifier}.hashed_dictionary_complex_key_complex_attributes;
