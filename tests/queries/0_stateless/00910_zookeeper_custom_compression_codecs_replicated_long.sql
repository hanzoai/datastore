...
) ENGINE = ReplicatedMergeTree('/datastore/tables/{database}/test_00910/compression_codec_replicated', '1') ORDER BY tuple();
...
CREATE TABLE compression_codec_replicated2(
  ...
  somestr FixedString(3) CODEC(LZ4HC(7)),
  othernum Int64 CODEC(Delta)
) ENGINE = ReplicatedMergeTree('/datastore/tables/{database}/test_00910/compression_codec_replicated', '2') ORDER BY tuple();
...
) ENGINE = ReplicatedMergeTree('/datastore/tables/{database}/test_00910/compression_codec_multiple', '1') ORDER BY tuple();
...
CREATE TABLE compression_codec_multiple_replicated2 (
    ...
    ddd Date CODEC(NONE, NONE, NONE, Delta(1), LZ4, ZSTD, LZ4HC, LZ4HC),
    somenum Float64 CODEC(Delta(4), LZ4, LZ4, ZSTD(2), LZ4HC(5), ZSTD(3), ZSTD)
) ENGINE = ReplicatedMergeTree('/datastore/tables/{database}/test_00910/compression_codec_multiple', '2') ORDER BY tuple();
...
) ENGINE = ReplicatedMergeTree('/datastore/tables/{database}/test_00910/compression_codec_multiple_more_types_replicated', '1') ORDER BY tuple();
...
) ENGINE = ReplicatedMergeTree('/datastore/tables/{database}/test_00910/compression_codec_multiple_with_key_replicated', '1') PARTITION BY somedate ORDER BY id SETTINGS index_granularity = 2, index_granularity_bytes = '10Mi';
