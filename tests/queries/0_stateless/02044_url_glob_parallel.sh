...
while [[ $i -lt $retries ]]; do
    timeout 5s ${DATASTORE_CLIENT} --max_threads 10 --query "SELECT * FROM url('http://127.0.0.{1..10}:${DATASTORE_PORT_HTTP}/?query=SELECT+sleep(1)', TSV, 'x UInt8')" --format Null && break
    ((++i))
done
