...
) ENGINE=ReplicatedMergeTree('/datastore/tables/{database}/test_01581/replicated_deduplicate', 'r1') ORDER BY id;

CREATE TABLE IF NOT EXISTS replicated_deduplicate_by_columns_r2 (
    id Int32, val UInt32, unique_value UInt64 MATERIALIZED rowNumberInBlock()
) ENGINE=ReplicatedMergeTree('/datastore/tables/{database}/test_01581/replicated_deduplicate', 'r2') ORDER BY id;
