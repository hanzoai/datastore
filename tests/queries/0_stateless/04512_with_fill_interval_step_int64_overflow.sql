...
-- doLongJump), where step_len is doubled repeatedly. It is reached only for the plain-modular DateTime
-- (UInt32) kinds SECOND/MINUTE/HOUR: their Add*Impl truncates to UInt32, so a large per-jump delta
-- wraps back into range and lets doLongJump keep doubling step_len until step * jumps_count overflows
...
-- doubling), and calendar kinds converge via DateLUT. The DateTime64 case below is therefore a
-- correctness smoke test of the same code path (the Decimal64 branch uses the identical
-- mulStepWrapping via the same FOR_EACH_INTERVAL_KIND macro, so it is protected by construction), not
-- an overflow trigger.
