...
$DATASTORE_CLIENT -q 'drop table if exists data_02263'
$DATASTORE_CLIENT -q 'create table data_02263 (key Int) engine=Memory()'
$DATASTORE_CLIENT -q 'insert into data_02263 format TSV settings max_threads=1 1' |& grep --max-count 1 -F --only-matching "Cannot parse input: expected '\n' before: 'settings max_threads=1 1'"
$DATASTORE_CLIENT --allow_settings_after_format_in_insert=1 -q 'insert into data_02263 format TSV settings max_threads=1 1'
$DATASTORE_CLIENT -q 'select * from data_02263'
$DATASTORE_CLIENT --allow_settings_after_format_in_insert=1 -q 'insert into data_02263 settings max_threads=1 format tsv settings max_threads=1' |& grep --max-count 1 -F --only-matching "You have SETTINGS before and after FORMAT"
$DATASTORE_CLIENT -q 'drop table data_02263'
