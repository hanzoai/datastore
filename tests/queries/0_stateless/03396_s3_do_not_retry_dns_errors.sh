...
# Test from https://github.com/ClickHouse/Datastore/issues/68663
...
$DATASTORE_CLIENT -nm -q "
    ...
    SETTINGS keeper_path = '/datastore/db_name/s3queue/test_s3_issue', mode = 'unordered', after_processing = 'keep', s3queue_loading_retries = 3, s3queue_processing_threads_num = 30, s3queue_enable_logging_to_s3queue_log = 1, s3queue_polling_min_timeout_ms = 1000, s3queue_polling_max_timeout_ms = 5000, s3queue_polling_backoff_ms = 1500, s3queue_tracked_file_ttl_sec = 345600, s3queue_tracked_files_limit = 1000000;
...
timeout 20s $DATASTORE_CLIENT -nm -q "
