...
run_distributed()
{
    ...
    while [ $SECONDS -lt "$TL" ]; do
        ${DATASTORE_CLIENT} -q "SELECT count() FROM remote('127.0.0.{1,2}', numbers(100)) FORMAT Null"
        ${DATASTORE_CLIENT} -q "SELECT sum(number) FROM remote('127.0.0.{1,2}', numbers(500)) GROUP BY number % 10 FORMAT Null"
        ${DATASTORE_CLIENT} -q "SELECT number % 3, sum(number) FROM remote('127.0.0.{1,2}', numbers(100)) GROUP BY ROLLUP(number % 3) SETTINGS group_by_use_nulls=1 FORMAT Null"
        ${DATASTORE_CLIENT} -q "SELECT uniq(number) FROM remote('127.0.0.{1,2}', numbers(1000)) FORMAT Null"
        ${DATASTORE_CLIENT} -q "SELECT * FROM remote('127.0.0.{1,2}', numbers(10)) ORDER BY number FORMAT Null"
        ${DATASTORE_CLIENT} -q "SELECT count() FROM remote('127.0.0.{1,2}', system.one) FORMAT Null"
        ${DATASTORE_CLIENT} -q "SELECT 1 FROM remote('127.0.0.{1,2}', system.nonexistent)" 2>/dev/null
        ${DATASTORE_CLIENT} -q "SELECT count() FROM remote('127.0.0.{1,2}', numbers(100)) WHERE number IN (SELECT number FROM numbers(50)) FORMAT Null"
    done
}
...
run_table_ops()
{
    ...
    while [ $SECONDS -lt "$TL" ]; do
        ID=$((ID + 1))
        ${DATASTORE_CLIENT} -q "
            DROP TABLE IF EXISTS ${DATASTORE_DATABASE}.t_msan_$ID;
            CREATE TABLE ${DATASTORE_DATABASE}.t_msan_$ID (id UInt64, v String DEFAULT '', e UInt64 DEFAULT 0) ENGINE = MergeTree ORDER BY id;
            INSERT INTO ${DATASTORE_DATABASE}.t_msan_$ID SELECT number, toString(number), 0 FROM numbers(1000);
            ALTER TABLE ${DATASTORE_DATABASE}.t_msan_$ID UPDATE e = id WHERE 1;
            SELECT count() FROM remote('127.0.0.{1,2}', ${DATASTORE_DATABASE}.t_msan_$ID) FORMAT Null;
            DROP TABLE ${DATASTORE_DATABASE}.t_msan_$ID;
        " 2>/dev/null
    done
}
