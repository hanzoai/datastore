...
# https://github.com/ClickHouse/Datastore/issues/99528
...
${DATASTORE_CLIENT} --query "SELECT 1 FROM system.asynchronous_inserts FORMAT Pretty" > /dev/null
${DATASTORE_CLIENT} --query "SELECT 1 FROM system.asynchronous_inserts FORMAT PrettyCompact" > /dev/null
${DATASTORE_CLIENT} --query "SELECT 1 FROM system.asynchronous_inserts FORMAT PrettySpace" > /dev/null
