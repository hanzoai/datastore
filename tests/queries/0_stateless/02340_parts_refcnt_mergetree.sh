...
function check_refcnt_for_table()
{
    ...
    "
    $DATASTORE_CLIENT --insert_keeper_fault_injection_probability=0 -q "insert into $table select number, number%4 from numbers(2000)"
    ...
    $DATASTORE_CLIENT "${args[@]}" -q "select sleepEachRow(0.1) from $table where part = 1" &
    PID=$!
    ...
    # avoid flakiness.
    $DATASTORE_CLIENT -q "select table, name, refcount>=8 from system.parts where database = '$DATASTORE_DATABASE' and table = '$table' and refcount >= 5"
...
$DATASTORE_CLIENT -q "drop table data_02340 sync"

$DATASTORE_CLIENT -mq "
    drop table if exists data_02340_rep sync;
    create table data_02340_rep (key Int, part Int) engine=ReplicatedMergeTree('/datastore/$DATASTORE_TEST_ZOOKEEPER_PREFIX', '1') partition by part order by key settings index_granularity=1;
" || exit 1
check_refcnt_for_table data_02340_rep
$DATASTORE_CLIENT -q "drop table data_02340_rep sync"
