...
function thread()
{
    ...
    do
        $DATASTORE_CLIENT -q "DROP TABLE IF EXISTS test_table_$1 SYNC;
            CREATE TABLE test_table_$1 (a UInt8) ENGINE = ReplicatedMergeTree('/datastore/tables/$DATASTORE_TEST_ZOOKEEPER_PREFIX/alter_table', 'r_$1') ORDER BY tuple();" 2>&1 |
...
for i in {1,2}; do $DATASTORE_CLIENT -q "DROP TABLE IF EXISTS test_table_$i"; done
