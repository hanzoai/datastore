...
# Regression for https://github.com/ClickHouse/Datastore/issues/104335
#
...
# and aborted the server / `datastore-local`.
#
# This file uses `datastore-local` because the server-side query analyzer constant-folds
# the all-constant expression before reaching `transform`, hiding the bug. `datastore-local`
...
$DATASTORE_LOCAL --query "SELECT caseWithExpression(toUInt64(123),
    CAST(1 AS UInt32), CAST(31712 AS UInt16),
    CAST(2 AS UInt32), CAST(-8 AS Int8),
    CAST(-0.09 AS Decimal(9, 2)));"

# Match on first WHEN.
$DATASTORE_LOCAL --query "SELECT caseWithExpression(toUInt64(1),
    CAST(1 AS UInt32), CAST(31712 AS UInt16),
    CAST(2 AS UInt32), CAST(-8 AS Int8),
    CAST(-0.09 AS Decimal(9, 2)));"

# Match on second WHEN.
$DATASTORE_LOCAL --query "SELECT caseWithExpression(toUInt64(2),
    CAST(1 AS UInt32), CAST(31712 AS UInt16),
    CAST(2 AS UInt32), CAST(-8 AS Int8),
    CAST(-0.09 AS Decimal(9, 2)));"
...
# ELSE `Decimal(9, 2)`.
$DATASTORE_LOCAL --query "SELECT count(), groupArray(r) FROM (
