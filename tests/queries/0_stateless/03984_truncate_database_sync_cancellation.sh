...
DB_NAME="${DATASTORE_DATABASE}_trunc_cancel"
TRUNCATE_QUERY_ID="truncate_db_cancel_${DATASTORE_DATABASE}_$$"

function cleanup()
{
    $DATASTORE_CLIENT --query "SYSTEM DISABLE FAILPOINT truncate_database_tables_pause" 2>/dev/null ||:
    $DATASTORE_CLIENT --query "KILL QUERY WHERE query_id = '${TRUNCATE_QUERY_ID}' FORMAT Null" 2>/dev/null ||:
    wait 2>/dev/null ||:
...
$DATASTORE_CLIENT --query "SYSTEM ENABLE FAILPOINT truncate_database_tables_pause"
...
$DATASTORE_CLIENT --query "SYSTEM WAIT FAILPOINT truncate_database_tables_pause PAUSE"

# Set is_killed WITHOUT waiting for the query to exit (no SYNC keyword).
$DATASTORE_CLIENT --query "KILL QUERY WHERE query_id = '${TRUNCATE_QUERY_ID}' FORMAT Null"
...
$DATASTORE_CLIENT --query "SYSTEM DISABLE FAILPOINT truncate_database_tables_pause"
