...
TEST_START=$(${DATASTORE_CLIENT} --query "SELECT now()")
...
for iteration in {1..5}; do
    ...
    ${DATASTORE_CLIENT} --query "DROP TABLE IF EXISTS ${DATASTORE_DATABASE}.filelog_src SYNC"
    ${DATASTORE_CLIENT} --query "DROP TABLE IF EXISTS ${DATASTORE_DATABASE}.filelog_mv SYNC"
    ${DATASTORE_CLIENT} --query "DROP TABLE IF EXISTS ${DATASTORE_DATABASE}.filelog_dst SYNC"

    ${DATASTORE_CLIENT} --query "
        CREATE TABLE ${DATASTORE_DATABASE}.filelog_dst (k UInt64, v UInt64) ENGINE = MergeTree ORDER BY k
    "
    ...
    ${DATASTORE_CLIENT} --query "
        CREATE TABLE ${DATASTORE_DATABASE}.filelog_src (k UInt64, v UInt64)
        ENGINE = FileLog('${DATA_DIR}/', 'CSV')
        SETTINGS poll_directory_watch_events_backoff_max = 1000
    "
    ${DATASTORE_CLIENT} --query "
        CREATE MATERIALIZED VIEW ${DATASTORE_DATABASE}.filelog_mv TO ${DATASTORE_DATABASE}.filelog_dst
        AS SELECT * FROM ${DATASTORE_DATABASE}.filelog_src
    "
    ...
    for attempt in {1..300}; do
        count=$(${DATASTORE_CLIENT} --query "SELECT count() FROM ${DATASTORE_DATABASE}.filelog_dst")
        [[ "${count}" -ge 10 ]] && break
        sleep 0.5
    done
    ...
    ${DATASTORE_CLIENT} --query "DROP TABLE ${DATASTORE_DATABASE}.filelog_mv SYNC"

    # Give the background thread time to attempt streaming with no MV
    sleep 0.2

    ${DATASTORE_CLIENT} --query "DROP TABLE ${DATASTORE_DATABASE}.filelog_src SYNC"
    ${DATASTORE_CLIENT} --query "DROP TABLE ${DATASTORE_DATABASE}.filelog_dst SYNC"
