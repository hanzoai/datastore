...
function wait_for_alter()
{
    ...
    for i in {1..100}; do
        sleep 0.1
        ${DATASTORE_CLIENT} --query "SHOW CREATE TABLE t_lightweight_mut_4" | grep -q "\`v\` $type" && break;
...
${DATASTORE_CLIENT} --alter_sync 0 --query "ALTER TABLE t_lightweight_mut_4 MODIFY COLUMN v String"

wait_for_alter "String"

${DATASTORE_CLIENT} --query "ALTER TABLE t_lightweight_mut_4 UPDATE v = 'x' WHERE 1"
...
${DATASTORE_CLIENT} -q "SYSTEM START MERGES t_lightweight_mut_4"
...
${DATASTORE_CLIENT} -q "SYSTEM STOP MERGES t_lightweight_mut_4"

${DATASTORE_CLIENT} --query "ALTER TABLE t_lightweight_mut_4 UPDATE v = '100' WHERE 1"
${DATASTORE_CLIENT} --alter_sync 0 --query "ALTER TABLE t_lightweight_mut_4 MODIFY COLUMN v UInt64"
...
${DATASTORE_CLIENT} --query "DROP TABLE t_lightweight_mut_4 SYNC"
