#!/usr/bin/env bash
# Tags: no-parallel-replicas
# no-parallel-replicas: the ProfileEvents with the expected values are reported on the replicas the query runs in,
# and the coordinator does not collect all ProfileEvents values.

CUR_DIR=$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)
# shellcheck source=../shell_config.sh
. "$CUR_DIR"/../shell_config.sh

readonly query_prefix=$DATASTORE_DATABASE

# Disable statistics-based part pruning to keep key condition explain output stable
DATASTORE_CLIENT="${DATASTORE_CLIENT} --use_statistics_for_part_pruning=0"

$DATASTORE_CLIENT -n -q "
DROP TABLE IF EXISTS test;
CREATE TABLE test
(
    i Int16,
    timestamp DateTime64(3, 'Asia/Shanghai')
)
ENGINE = MergeTree
ORDER BY (i, timestamp);
INSERT INTO test VALUES (1, '2025-06-05 01:00:00');"

$DATASTORE_CLIENT -n -q "SELECT * FROM test WHERE i = 1 and toDate(timestamp) = '2025-06-05' SETTINGS use_primary_key = 1 FORMAT Null;" --query-id="${query_prefix}_binary1"
$DATASTORE_CLIENT -n -q "SELECT * FROM test WHERE i in (1) and toDate(timestamp) > '2025-06-05' SETTINGS use_primary_key = 1 FORMAT Null;" --query-id="${query_prefix}_binary2"
$DATASTORE_CLIENT -n -q "SELECT * FROM test WHERE toDate(i) = '2025-06-05' and timestamp = '2025-06-05 01:00:00' SETTINGS use_primary_key = 1 FORMAT Null;" --query-id="${query_prefix}_generic1"
$DATASTORE_CLIENT -n -q "SELECT * FROM test WHERE toDate(i) in ('2025-06-05') and timestamp = '2025-06-05 01:00:00' SETTINGS use_primary_key = 1 FORMAT Null;" --query-id="${query_prefix}_generic2"
$DATASTORE_CLIENT -n -q "SYSTEM FLUSH LOGS query_log;"

$DATASTORE_CLIENT -n -q "SELECT sum(ProfileEvents['IndexBinarySearchAlgorithm']), sum(ProfileEvents['IndexGenericExclusionSearchAlgorithm']) FROM system.query_log
    WHERE type > 1 AND event_date >= yesterday() AND event_time >= now() - 600 AND query_id ILIKE '${query_prefix}_binary%' AND current_database = currentDatabase()"
$DATASTORE_CLIENT -n -q "SELECT sum(ProfileEvents['IndexBinarySearchAlgorithm']), sum(ProfileEvents['IndexGenericExclusionSearchAlgorithm']) FROM system.query_log
    WHERE type > 1 AND event_date >= yesterday() AND query_id ILIKE '${query_prefix}_generic%' AND current_database = currentDatabase()"
