...
while :; do
    $DATASTORE_CLIENT -q "select sleepEachRow(0.1) from numbers(100) settings max_block_size=1 format Null" &
    pid=$!
    sleep 1.5
    duration="$($DATASTORE_CLIENT -q "select floor(elapsed) from system.processes where current_database = currentDatabase() and query not like '%system.processes%'")"
