...
#IFS=$'\n'; for I in $(${CURDIR}/00921_datetime64_compatibility.python) ; do unset IFS; ${DATASTORE_CLIENT} --query "${I}"; echo ; done 2>&1;

# ${CURDIR}/00921_datetime64_compatibility.python

python3 "${CURDIR}"/00921_datetime64_compatibility_long.python \
    | ${DATASTORE_CLIENT} --ignore-error -m --calculate_text_stack_trace 0 --log-level 'error' 2>&1 \
