...
TABLE_PATH="04092_iceberg_mutate/${DATASTORE_TEST_UNIQUE_NAME}"
...
${DATASTORE_CLIENT} --allow_insert_into_iceberg=1 --query "INSERT INTO ${TABLE} VALUES ('a')"
...
${DATASTORE_CLIENT} --query "DETACH TABLE ${TABLE}"
${DATASTORE_CLIENT} --query "ATTACH TABLE ${TABLE}"
...
${DATASTORE_CLIENT} --allow_insert_into_iceberg=1 --validate_mutation_query=0 --query "ALTER TABLE ${TABLE} UPDATE c0 = 'b' WHERE TRUE"

${DATASTORE_CLIENT} --query "SELECT c0 FROM ${TABLE}"
