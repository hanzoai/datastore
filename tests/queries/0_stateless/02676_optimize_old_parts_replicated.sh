...
CREATE TABLE test_without_merge (i Int64) ENGINE = ReplicatedMergeTree('/datastore/tables/{database}/test02676_without_merge', 'node') ORDER BY i SETTINGS merge_selecting_sleep_ms=1000;
...
CREATE TABLE test_replicated (i Int64) ENGINE = ReplicatedMergeTree('/datastore/tables/{database}/test02676', 'node')  ORDER BY i
...
CREATE TABLE test_replicated (i Int64) ENGINE = ReplicatedMergeTree('/datastore/tables/{database}/test02676_partition_only', 'node')  ORDER BY i
PARTITION BY i
...
CREATE TABLE test_replicated_limit (i Int64) ENGINE = ReplicatedMergeTree('/datastore/tables/{database}/test02676_partition_only_limit', 'node')  ORDER BY i
PARTITION BY i
