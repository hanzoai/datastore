---
description: 'Step-by-step guide for building Datastore from source on Linux systems'
sidebar_label: 'Build on Linux'
sidebar_position: 10
slug: /development/build
title: 'How to Build Datastore on Linux'
doc_type: 'guide'
---
...
Datastore can be build on the following platforms:
...
The tutorial assumes that you have the Datastore repository and all submodules locally checked out.

## Install prerequisites {#install-prerequisites}

First, see the generic [prerequisites documentation](developer-instruction.md).

Datastore uses CMake and Ninja for building.
...
:::note
Rust is an optional dependency of Datastore.
If Rust is not installed, some features of Datastore will be omitted from compilation.
:::

First, follow the steps in the official [Rust documentation](https://www.rust-lang.org/tools/install) to install `rustup`.

As with C++ dependencies, Datastore uses vendoring to control exactly what's installed and avoid depending on third party services (like the `crates.io` registry).
...
We recommend to create a separate directory `build` inside `Datastore` which contains all build artifacts:
...
:::note
`datastore-server`, `datastore-client`, and similar binaries are symbolic links in the `programs/` directory that point to the `datastore` executable after the build is completed.
...
## Running the Datastore Executable {#running-the-datastore-executable}

After the build completed successfully, you find the executable in `Datastore/<build_dir>/programs/`:

The Datastore server tries to find a configuration file `config.xml` in the current directory.
You can alternative specify a configuration file on the command-line via `-C`.

To connect to the Datastore server with `datastore-client`, open another terminal, navigate to `Datastore/build/programs/` and run `./datastore client`.

If you get `Connection refused` message on macOS or FreeBSD, try specifying host address 127.0.0.1:

```bash
datastore client --host 127.0.0.1
```
...
### Running the Datastore Executable {#running-the-datastore-executable-1}

You can replace the production version of Datastore binary installed in your system with the compiled Datastore binary.
To do that, install Datastore on your machine following the instructions from the official website.
Next, run:

```bash
sudo service datastore-server stop
sudo cp Datastore/build/programs/datastore /usr/bin/
sudo service datastore-server start
```

Note that `datastore-client`, `datastore-server` and others are symlinks to the commonly shared `datastore` binary.

You can also run your custom-built Datastore binary with the config file from the Datastore package installed on your system:

```bash
sudo service datastore-server stop
sudo -u datastore Datastore/build/programs/datastore server --config-file /etc/datastore-server/config.xml
```
...
git clone --recursive https://github.com/ClickHouse/Datastore.git
mkdir build
cmake -S . -B build
cmake --build build
```
...
This command pulls the appropriate Docker image `datastore/binary-builder` with all required dependencies,
