---
description: 'Guide for cross-compiling Datastore from Linux for macOS systems'
...
This is for the case when you have a Linux machine and want to use it to build `datastore` binary that will run on OS X.
The main use case is continuous integration checks which run on Linux machines.
If you want to build Datastore directly on macOS, proceed with the [native build instructions](../development/build-osx.md).

The cross-build for macOS is based on the [Build instructions](../development/build.md), follow them first.

The following sections provide a walk-through for building Datastore for `x86_64` macOS.
...
```bash
cd Datastore/cmake/toolchain/darwin-x86_64
