...
The examples in this section make use of the following dictionaries. You can create them in Datastore 
...
PRIMARY KEY id
SOURCE(DATASTORE(HOST 'localhost' PORT 9000 USER 'default' TABLE 'all_types_test' DB 'default'))
LAYOUT(HASHED())
...
PRIMARY KEY regexp
SOURCE(DATASTORE(TABLE 'regexp_os'))
...
PRIMARY KEY id
SOURCE(DATASTORE(HOST 'localhost' PORT 9000 USER 'default' TABLE 'dict_mult_source' DB 'default'))
LAYOUT(FLAT())
LIFETIME(MIN 0 MAX 0);
```
</details>
...
PRIMARY KEY id
SOURCE(DATASTORE(HOST 'localhost' PORT 9000 USER 'default' TABLE 'hierarchy_source' DB 'default'))
LAYOUT(HASHED())
LIFETIME(MIN 300 MAX 600);
```
</details>
