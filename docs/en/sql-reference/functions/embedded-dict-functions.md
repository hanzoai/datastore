...
As such, the examples shown in this section will throw an exception in [Datastore Fiddle](https://fiddle.datastore.com/) and in quick release and production deployments by default, unless first configured.
:::
...
Datastore supports working with multiple alternative geobases (regional hierarchies) simultaneously, in order to support various perspectives on which countries certain regions belong to.

The 'datastore-server' config specifies the file with the regional hierarchy:
