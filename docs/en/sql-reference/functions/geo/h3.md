...
Note: In Datastore v25.4 or older, `geoToH3()` takes values in order `(lon, lat)`. As per Datastore v25.5, the input values are in order `(lat, lon)`. The previous behaviour can be restored using setting `geotoh3_argument_order = 'lon_lat'`.
...
Note: In Datastore v24.12 or older, `h3ToGeo()` returns values in order `(lon, lat)`. As per Datastore v25.1, the returned values are in order `(lat, lon)`. The previous behaviour can be restored using setting `h3togeo_lon_lat_result_order = true`.
