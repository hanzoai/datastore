...
| `partition_columns_in_data_file`        | Parameter is optional. Only used with `HIVE` partition strategy. Tells Datastore whether to expect partition columns to be written in the data file. Defaults `false`.                                                                                                                                                                                                          |
...
:::note
Datastore uses filename extensions to determine the format of the data. For example, we could have run the previous command without the `CSVWithNames`:
...
Datastore also can determine the compression method of the file. For example, if the file was zipped up with a `.csv.gz` extension, Datastore would decompress the file automatically.
:::

:::note
Parquet files with names like `*.parquet.snappy` or `*.parquet.zstd` can confuse Datastore and cause `TOO_LARGE_COMPRESSED_BLOCK` or `ZSTD_DECODER_FAILED` errors.
This is because Datastore would attempt to read the entire file as Snappy or ZSTD-encoded data when, in fact, Parquet applies compression at the row-group and column level.
...
- 'https://datastore-public-datasets.s3.amazonaws.com/my-test-bucket-768/some_prefix/some_file_1.csv'
- 'https://datastore-public-datasets.s3.amazonaws.com/my-test-bucket-768/some_prefix/some_file_2.csv'
- 'https://datastore-public-datasets.s3.amazonaws.com/my-test-bucket-768/some_prefix/some_file_3.csv'
- 'https://datastore-public-datasets.s3.amazonaws.com/my-test-bucket-768/some_prefix/some_file_4.csv'
- 'https://datastore-public-datasets.s3.amazonaws.com/my-test-bucket-768/another_prefix/some_file_1.csv'
- 'https://datastore-public-datasets.s3.amazonaws.com/my-test-bucket-768/another_prefix/some_file_2.csv'
- 'https://datastore-public-datasets.s3.amazonaws.com/my-test-bucket-768/another_prefix/some_file_3.csv'
- 'https://datastore-public-datasets.s3.amazonaws.com/my-test-bucket-768/another_prefix/some_file_4.csv'
...
```sql
INSERT INTO FUNCTION s3('https://datastore-public-datasets.s3.amazonaws.com/my-test-bucket-768/test-data.csv.gz', 'CSV', 'name String, value UInt32', 'gzip')
...
```sql
SELECT * FROM s3('https://datastore-public-datasets.s3.amazonaws.com/my-test-bucket-768/**', 'CSV', 'name String, value UInt32', 'gzip');
```

The below get data from all `test-data.csv.gz` files from any folder inside `my-test-bucket` directory recursively:

```sql
SELECT * FROM s3('https://datastore-public-datasets.s3.amazonaws.com/my-test-bucket-768/**/test-data.csv.gz', 'CSV', 'name String, value UInt32', 'gzip');
```

Note. It is possible to specify custom URL mappers in the server configuration file. Example:
```sql
SELECT * FROM s3('s3://datastore-public-datasets/my-test-bucket-768/**/test-data.csv.gz', 'CSV', 'name String, value UInt32', 'gzip');
```
The URL `'s3://datastore-public-datasets/my-test-bucket-768/**/test-data.csv.gz'` would be replaced to `'http://datastore-public-datasets.s3.amazonaws.com/my-test-bucket-768/**/test-data.csv.gz'`
...
## Using S3 credentials (Datastore Cloud) {#using-s3-credentials-datastore-cloud}
...
Role-based access for S3 in Datastore Cloud is documented [here](/cloud/data-sources/secure-s3).
...
When reading Avro files that contain **nested records** which diverge across files (for example, some files have an extra field inside a nested object), Datastore may return an error such as:

> The number of leaves in record doesn't match the number of elements in tuple...

This happens because Datastore expects all nested record structures to match the same schema.  
