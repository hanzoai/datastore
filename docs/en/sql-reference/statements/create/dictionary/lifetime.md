...
Datastore periodically updates dictionaries based on the `LIFETIME` tag (defined in seconds).
...
You can set a time interval for updates, and Datastore will choose a uniformly random time within this range. This is necessary in order to distribute the load on the dictionary source when updating on a large number of servers.
...
If `<min>0</min>` and `<max>0</max>`, Datastore does not reload the dictionary by timeout.
In this case, Datastore can reload the dictionary earlier if the dictionary configuration file was changed or the `SYSTEM RELOAD DICTIONARY` command was executed.

When updating the dictionaries, the Datastore server applies different logic depending on the type of [source](./sources/):

- For a text file, it checks the time of modification. If the time differs from the previously recorded time, the dictionary is updated.
- Dictionaries from other sources are updated every time by default.

For other sources (ODBC, PostgreSQL, Datastore, etc), you can set up a query that will update the dictionaries only if they really changed, rather than each time. To do this, follow these steps:

- The dictionary table must have a field that always changes when the source data is updated.
- The settings of the source must specify a query that retrieves the changing field. The Datastore server interprets the query result as a row, and if this row has changed relative to its previous state, the dictionary is updated. Specify the query in the `<invalidate_query>` field in the settings for the [source](./sources/).
...
- If the source is Datastore, MySQL, PostgreSQL, ODBC there will be an additional part of `WHERE`, where `update_field` is compared as greater or equal with the last update time.
...
```sql
...
SOURCE(DATASTORE(... update_field 'added_time' update_lag 15))
...
```
