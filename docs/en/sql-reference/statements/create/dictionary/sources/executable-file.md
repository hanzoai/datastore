...
description: 'Configure an executable file as a dictionary source in Datastore.'
doc_type: 'reference'
---
...
| `command_termination_timeout` | The executable script should contain a main read-write loop. After the dictionary is destroyed, the pipe is closed, and the executable file will have `command_termination_timeout` seconds to shutdown before Datastore will send a SIGTERM signal to the child process. Specified in seconds. Default value is `10`. Optional. |
...
That dictionary source can be configured only via XML configuration. Creating dictionaries with executable source via DDL is disabled; otherwise, the DB user would be able to execute arbitrary binaries on the Datastore node.
