...
# Query by SQL
descriptor = flight.FlightDescriptor.for_command("SELECT * FROM my_table WHERE id > 100")
info = client.get_flight_info(descriptor, options)
...
```python
descriptor = flight.FlightDescriptor.for_command("SELECT 1 AS x, 'hello' AS y")
...
- A ticket returned by `GetFlightInfo` or `PollFlightInfo` (prefixed with `~TICKET-`).
...
```python
descriptor = flight.FlightDescriptor.for_command("INSERT INTO my_table FORMAT Arrow")
writer, _ = client.do_put(descriptor, schema, options)
writer.write_batch(batch)
writer.close()
```
...
cmd = CommandStatementUpdate(query="CREATE TABLE t (x UInt32) ENGINE = Memory")
wrapper = any_pb2.Any()
wrapper.Pack(cmd)
descriptor = flight.FlightDescriptor.for_command(wrapper.SerializeToString())
...
Cancels a running query associated with a `FlightInfo`. The query ID is extracted from the `FlightInfo`'s `app_metadata` field.
...
Returns all current ClickHouse settings and their values for the session. Returns a map of setting names to string values.
...
## Querying Data: Complete Example {#complete-example}
...
# Create a table
ticket = flight.Ticket("CREATE TABLE test (id UInt32, value String) ENGINE = MergeTree ORDER BY id")
# For DDL, use do_get with a raw SQL ticket
reader = client.do_get(ticket, options)
reader.read_all()

# Insert data using DoPut
...
descriptor = flight.FlightDescriptor.for_command("SELECT * FROM test ORDER BY id")
info = client.get_flight_info(descriptor, options)
for endpoint in info.endpoints:
    reader = client.do_get(endpoint.ticket, options)
    table = reader.read_all()
