...
The non-flat `V3` prefix additionally carries a `shared_data_buckets` count (present when the shared-data serialization version is `MAP_WITH_BUCKETS` or `ADVANCED`). It sizes the per-bucket reader state and column vectors, so a decoder must reject an implausible bucket count up front; legitimate values come from a small MergeTree setting (a few hundred at most), and ClickHouse rejects a count above the structural ceiling with `INCORRECT_DATA` ("JSON/Object column has too many shared data buckets").
:::
