...
The web terminal is an experimental in-browser interface that provides an interactive `datastore-client` session over WebSocket. It is served from any Datastore HTTP port at the `/webterminal` path.
...
After enabling, navigate to `/webterminal` on any Datastore HTTP port (for example, `http://localhost:8123/webterminal`) to open the terminal.
