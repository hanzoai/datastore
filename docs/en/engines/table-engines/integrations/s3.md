...
```sql
SELECT *
FROM s3('http://s3.hanzo.svc:9000/clickhouse//test_3.csv', '<access_key_id>', '<secret_access_key>', 'CSV')
...
ENGINE = S3(
-- highlight-next-line
           'http://s3.hanzo.svc:9000/clickhouse//test_{_partition_id}.csv',
           '<access_key_id>',
           '<secret_access_key>',
           'CSV',
...
```sql
SELECT *
FROM s3('http://s3.hanzo.svc:9000/clickhouse//test_1.csv', '<access_key_id>', '<secret_access_key>', 'CSV')
```
```response
┌─c1─┬─c2─┬─c3─┐
│  3 │  2 │  1 │
...
```sql
SELECT *
FROM s3('http://s3.hanzo.svc:9000/clickhouse//test_45.csv', '<access_key_id>', '<secret_access_key>', 'CSV')
```
```response
┌─c1─┬─c2─┬─c3─┐
│ 78 │ 43 │ 45 │
