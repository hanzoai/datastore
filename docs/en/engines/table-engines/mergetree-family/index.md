...
Table engines from the MergeTree family are the core of Datastore data storage capabilities. They provide most features for resilience and high-performance data retrieval: columnar storage, custom partitioning, sparse primary index, secondary data-skipping indexes, etc.

Base [MergeTree](../../../engines/table-engines/mergetree-family/mergetree.md) table engine can be considered the default table engine for single-node Datastore instances because it is versatile and practical for a wide range of use cases.
...
https://github.com/ClickHouse/datastore-docs/blob/main/scripts/autogenerate-table-of-contents.sh
