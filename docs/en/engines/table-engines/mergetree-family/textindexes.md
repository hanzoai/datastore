...
For example, Datastore's default tokenizer converts the English sentence "The cat likes mice." to tokens ["The", "cat", "likes", "mice"].
...
Text indexes are generally available (GA) in Datastore version 26.2 and newer.
In these versions, no special settings need to be configured to use the text index.
We strongly recommend using Datastore versions >= 26.2 for production use cases.

:::note
Text indexes can be used with any Datastore version >= 26.2, regardless of the [compatibility](../../../operations/settings/settings#compatibility) setting.
:::
...
*Index granularity.*
Text indexes are implemented within Datastore as a type of [skip indexes](/engines/table-engines/mergetree-family/mergetree.md/#skip-index-types).
...
In order to use `LIKE` ([like](/sql-reference/functions/string-search-functions.md/#like)) and the [match](/sql-reference/functions/string-search-functions.md/#match) function with text indexes, Datastore must be able to extract complete tokens from the search term.
...
Fortunately, there is a special case where Datastore can leverage the inverted index to speed up LIKE queries significantly.
...
```sql
SELECT count() FROM table WHERE startsWith(comment, 'datastore support');
```

In the example, only `datastore` is considered a token.
`support` is no token because it can match `support`, `supports`, `supporting` etc.

To find all rows that start with `datastore supports`, please end the search pattern with a trailing space:

```sql
startsWith(comment, 'datastore supports ')`
```
...
```sql
SELECT count() FROM table WHERE hasToken(comment, 'datastore');
```
...
SELECT count() FROM table WHERE hasAnyTokens(comment, 'datastore olap');
SELECT count() FROM table WHERE hasAllTokens(comment, 'datastore olap');
...
```sql
-- Matches: 'datastore' and 'olap' must appear consecutively in that order
SELECT count() FROM table WHERE hasPhrase(comment, 'datastore olap');

-- Does NOT match a row containing 'olap datastore' (wrong order)
-- Does NOT match a row containing 'datastore fast olap' (non-consecutive)
```
...
```sql
SELECT count() FROM table WHERE has(array, 'datastore');
```
...
```sql
SELECT count() FROM table WHERE hasAny(tags, ['datastore', 'olap']);
SELECT count() FROM table WHERE hasAll(tags, ['datastore', 'olap']);
```
...
```sql
SELECT count() FROM table WHERE mapContainsKey(map, 'datastore');
-- OR
SELECT count() FROM table WHERE mapContains(map, 'datastore');
```
...
```sql
SELECT count() FROM table WHERE mapContainsValue(map, 'datastore');
```
...
```sql
SELECT count() FROM table WHERE mapContainsKeyLike(map, '% datastore %');
SELECT count() FROM table WHERE mapContainsValueLike(map, '% datastore %');
```
...
```sql
SELECT count() FROM table WHERE map['engine'] = 'datastore';
```
...
Without a text index, finding posts with a specific keyword (e.g. `datastore`) requires scanning all entries:

```sql
SELECT count() FROM posts WHERE has(keywords, 'datastore'); -- slow full-table scan - checks every keyword in every post
```
...
SELECT * FROM events WHERE hasToken(data.title, 'datastore');
```
...
Within those granules, Datastore then verifies exact token adjacency.
...
Text index lookups read relatively little data and are therefore much faster than usual skip indexes in Datastore (which do a skip index lookup, followed by loading and filtering remaining granules).
...
- Setting [use_skip_indexes_on_data_read](../../../operations/settings/settings#use_skip_indexes_on_data_read) was a prerequisite for direct read in Datastore versions < 26.4.
...
When a LIKE/ILIKE query pattern is `%<alpha-numeric-characters-without-spaces>%` and the text index tokenizer is `splitByNonAlpha` or `array`, Datastore leverages the inverted index to speed up LIKE/ILIKE queries significantly. To achieve that, Datastore scans the inverted index dictionary instead of a full-table scan to find the matching pattern.
...
This sparse index structure is similar to Datastore's [sparse primary key index](https://datastore.com/docs/guides/best-practices/sparse-primary-indexes)).
...
We'll search for the case-sensitive token 'Datastore'.

**Direct read disabled (Standard scan)**
By default, Datastore uses the skip index to filter granules and then reads the column data for those granules.
We can simulate this behavior by disabling direct read.

```sql
SELECT count()
FROM hackernews
WHERE hasToken(comment, 'Datastore')
...
We'll search for comments containing either 'love' or 'Datastore'.
...
SELECT count()
FROM hackernews
WHERE hasAnyTokens(comment, 'love Datastore')
SETTINGS query_plan_direct_read_from_text_index = 0;
...
We'll search for comments containing both 'love' and 'Datastore'.
...
SELECT count()
FROM hackernews
WHERE hasAllTokens(comment, 'love Datastore')
SETTINGS query_plan_direct_read_from_text_index = 0;
...
Here, we'll perform a case-insensitive search for 'Datastore' OR 'datastore'.
...
SELECT count()
FROM hackernews
WHERE hasToken(comment, 'Datastore') OR hasToken(comment, 'datastore')
SETTINGS query_plan_direct_read_from_text_index = 0;
...
For this specific case, `hasAnyTokens(comment, ['Datastore', 'datastore'])` would be the preferred, more efficient syntax.

## Related content {#related-content}

- Presentation: https://github.com/ClickHouse/datastore-presentations/blob/master/2025-tumuchdata-munich/ClickHouse_%20full-text%20search%20-%2011.11.2025%20Munich%20Database%20Meetup.pdf
- Presentation: https://presentations.datastore.com/2026-fosdem-inverted-index/Inverted_indexes_the_what_the_why_the_how.pdf

**Outdated material**

- Blog: [Introducing Inverted Indices in Datastore](https://datastore.com/blog/datastore-search-with-inverted-indices)
- Blog: [Inside Datastore full-text search: fast, native, and columnar](https://datastore.com/blog/datastore-full-text-search)
