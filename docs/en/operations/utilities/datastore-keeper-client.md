---
description: 'Documentation for the Datastore Keeper client utility'
sidebar_label: 'datastore-keeper-client'
slug: /operations/utilities/datastore-keeper-client
title: 'datastore-keeper-client utility'
doc_type: 'reference'
---

A client application to interact with datastore-keeper by its native protocol.

## Keys {#datastore-keeper-client}

-   `-q QUERY`, `--query=QUERY` — Query to execute. If this parameter is not passed, `datastore-keeper-client` will start in interactive mode.
...
-   `--password=PASSWORD` — Password for authentication. Can also be set via the `DATASTORE_KEEPER_PASSWORD` environment variable or in the XML config file under `<zookeeper><password>`.
-   `--identity=IDENTITY` — Identity for `digest` authentication scheme. Can also be set via the `DATASTORE_KEEPER_IDENTITY` environment variable or in the XML config file under `<zookeeper><identity>`.
...
## Environment Variables {#datastore-keeper-client-env}

-   `DATASTORE_KEEPER_PASSWORD` — Used as the default password if `--password` is not provided on the command line.
-   `DATASTORE_KEEPER_IDENTITY` — Used as the default identity if `--identity` is not provided on the command line.

## Authentication {#datastore-keeper-client-auth}
...
2. `DATASTORE_KEEPER_PASSWORD` environment variable
3. `<zookeeper><password>` in the XML config file specified by `--config-file`

The same priority applies to `--identity` / `DATASTORE_KEEPER_IDENTITY` / `<zookeeper><identity>`.
...
## Example {#datastore-keeper-client-example}

```bash
./datastore-keeper-client -h localhost -p 9181 --connection-timeout 30 --session-timeout 30 --operation-timeout 30
...
## Commands {#datastore-keeper-client-commands}
...
-   `delete_stale_backups` -- Deletes Datastore nodes used for backups that are now inactive
-   `find_big_family [path] [n]` -- Returns the top n nodes with the biggest family in the subtree (default path = `.` and n = 10)
-   `sync '<path>'` -- Synchronizes node between processes and leader
-   `reconfig <add|remove|set> "<arg>" [version]` -- Reconfigure Keeper cluster. See /docs/en/guides/sre/keeper/datastore-keeper#reconfiguration
