...
sidebar_label: 'datastore-disks'
sidebar_position: 59
slug: /operations/utilities/datastore-disks
title: 'Clickhouse-disks'
doc_type: 'reference'
---

A utility providing filesystem-like operations for Datastore disks. It can work in both interactive and not interactive modes.

## Program-wide options {#program-wide-options}

* `--config-file, -C` -- path to Datastore config, defaults to `/etc/datastore-server/config.xml`.
* `--save-logs` -- Log progress of invoked commands to `/var/log/datastore-server/datastore-disks.log`.
...
All disks which are available in config are initialized lazily. This means that the corresponding object for a disk is initialized only when corresponding disk is used in some command. This is done to make the utility more robust and to avoid touching of disks which are described in config but not used by a user and can fail during initialization. However, there should be a disk which is initialized at the datastore-disks launch. This disk is specified with parameter `--disk` through command-line (default value is `default`).
...
1. **`local` Disk**: This disk is designed to mimic the local file system from which the `datastore-disks` utility was launched. Its initial path is the directory from which `datastore-disks` was started, and it is mounted at the root directory of the file system.

2. **`default` Disk**: This disk is mounted to the local file system in the directory specified by the `datastore/path` parameter in the configuration (the default value is `/var/lib/datastore`). Its initial path is set to `/`.

## Clickhouse-disks state {#datastore-disks-state}
