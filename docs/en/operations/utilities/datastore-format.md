---
description: 'Guide to using the format utility for working with Datastore data formats'
slug: /operations/utilities/datastore-format
title: 'datastore-format'
doc_type: 'reference'
---
...
```bash title="Query"
$ datastore-format --query "select number from numbers(10) where number%2 order by number desc;"
```
...
```bash title="Query"
$ datastore-format --oneline --hilite <<< "SELECT sum(number) FROM numbers(5);"
```
...
```bash title="Query"
$ datastore-format -n <<< "SELECT min(number) FROM numbers(5); SELECT max(number) FROM numbers(5);"
```
...
```bash title="Query"
$ datastore-format --seed Hello --obfuscate <<< "SELECT cost_first_screen BETWEEN a AND b, CASE WHEN x >= 123 THEN y ELSE NULL END;"
```
...
```bash title="Query"
$ datastore-format --seed World --obfuscate <<< "SELECT cost_first_screen BETWEEN a AND b, CASE WHEN x >= 123 THEN y ELSE NULL END;"
```
...
```bash title="Query"
$ datastore-format --backslash <<< "SELECT * FROM (SELECT 1 AS x UNION ALL SELECT 1 UNION DISTINCT SELECT 3);"
```
