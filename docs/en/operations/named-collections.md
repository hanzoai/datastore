...
when Datastore starts. They simplify the creation of objects and the hiding of credentials
...
To manage named collections with DDL a user must have the `named_collection_control` privilege.  This can be assigned by adding a file to `/etc/datastore-server/users.d/`.  The example gives the user `default` both the `access_management` and `named_collection_control` privileges:

```xml title='/etc/datastore-server/users.d/user_default.xml'
<datastore>
  <users>
    <default>
...
```xml title='/etc/datastore-server/config.d/named_collections.xml'
<datastore>
     <named_collections>
        <name>
...
## Named collections for accessing a remote Datastore database {#named-collections-for-accessing-a-remote-datastore-database}
...
### Example of using named collections with a dictionary with source Datastore {#example-of-using-named-collections-with-a-dictionary-with-source-datastore}

```sql
CREATE DICTIONARY dict(a Int64, b String)
PRIMARY KEY a
SOURCE(DATASTORE(NAME remote1 TABLE test DB default))
LIFETIME(MIN 1 MAX 2)
LAYOUT(HASHED());
