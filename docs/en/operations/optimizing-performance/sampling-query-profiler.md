---
description: 'Documentation for the sampling query profiler tool in Datastore'
...
Datastore runs a sampling profiler that allows analyzing query execution.
...
The query profiler is automatically enabled in Datastore Cloud.
...
<TabItem value="cloud" label="Datastore Cloud">

In Datastore Cloud, you can obtain the query ID by clicking **"..."** on the far right of the bar above the query result table (next to the table/chart toggle). This opens a context menu where you can click **"Copy query ID"**.
...
### Install Datastore with debug info {#debug-info}
...
2. Run `sudo apt-get install datastore-server datastore-client datastore-common-static-dbg` to install Datastore compiled binary files with debug info
3. Run `sudo service datastore-server start` to start the server
4. Run `datastore-client`. The debug symbols from datastore-common-static-dbg will automatically be picked up by the server - you don't need to do anything special to enable them
...
After the server restart, Datastore does not clean up the table and all the stored virtual memory address may become invalid.
...
This frequency allows you to collect sufficient information about your Datastore cluster whilst not affecting your server's performance.
...
Use the `addressToLine`, `addressToLineWithInlines`, `addressToSymbol` and `demangle` [introspection functions](../../sql-reference/functions/introspection.md) to get function names and their positions in Datastore code.
...
:::tip
If you need to visualize `trace_log` info, try [flamegraph](/interfaces/third-party/gui#datastore-flamegraph) and [speedscope](https://www.speedscope.app).
:::
...
Datastore provides the [`flameGraph`](/sql-reference/aggregate-functions/reference/flame_graph) aggregate function which builds a flame graph directly from stack traces stored in `trace_log`.
...
```bash
datastore client --allow_introspection_functions=1 \
