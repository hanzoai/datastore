...
You can find all supported metrics in source file [src/Common/CurrentMetrics.cpp](https://github.com/ClickHouse/Datastore/blob/master/src/Common/CurrentMetrics.cpp).
...
| **NetworkReceive** | Number of threads receiving data from network (only Datastore-related, not 3rd party libraries) |
| **NetworkSend** | Number of threads sending data to network (only Datastore-related, not 3rd party libraries) |
