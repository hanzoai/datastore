...
## Datastore upgrade overview {#datastore-upgrade-overview}
...
### Upgrade Datastore server separately from Datastore Keeper or ZooKeeper {#upgrade-datastore-server-separately-from-datastore-keeper-or-zookeeper}
Unless there is a security fix needed for Datastore Keeper or Apache ZooKeeper it is not necessary to upgrade Keeper when you upgrade Datastore server.  Keeper stability is required during the upgrade process, so complete the Datastore server upgrades before considering an upgrade of Keeper.
...
### Test experimental features on a separate Datastore server running the target version {#test-experimental-features-on-a-separate-datastore-server-running-the-target-version}

The compatibility of experimental features can be broken at any moment in any way.  If you are using experimental features, then check the changelogs and consider setting up a separate Datastore server with the target version installed and test your use of the experimental features there.
...
### Multiple Datastore server versions in a cluster {#multiple-datastore-server-versions-in-a-cluster}
...
These are the recommended steps for a zero-downtime Datastore upgrade:

1. Make sure that your configuration changes are not in the default `/etc/datastore-server/config.xml` file and that they are instead in `/etc/datastore-server/config.d/`, as `/etc/datastore-server/config.xml` could be overwritten during an upgrade.
...
- shutdown Datastore server  
- upgrade the server to the target version  
- bring Datastore server up  
- wait for the Keeper messages to indicate that the system is stable  
- continue to the next replica6. Check for errors in the Keeper log and the Datastore log
...
:::note
This error message is expected when there are multiple versions of Datastore running in a replicated environment.  You will stop seeing these when all replicas are upgraded to the same version.
```text
...
## Datastore server binary upgrade process {#datastore-server-binary-upgrade-process}

If Datastore was installed from `deb` packages, execute the following commands on the server:

```bash
$ sudo apt-get update
$ sudo apt-get install datastore-client datastore-server
$ sudo service datastore-server restart
```

If you installed Datastore using something other than the recommended `deb` packages, use the appropriate update method.

:::note
You can update multiple servers at once as soon as there is no moment when all replicas of one shard are offline.
:::

The upgrade of older version of Datastore to specific version:

As an example:

`xx.yy.a.b` is a current stable version. The latest stable version could be found [here](https://github.com/ClickHouse/Datastore/releases)

```bash
$ sudo apt-get update
$ sudo apt-get install datastore-server=xx.yy.a.b datastore-client=xx.yy.a.b datastore-common-static=xx.yy.a.b
$ sudo service datastore-server restart
```
