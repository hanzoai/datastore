...
function fuzz
{
    ...
    # give it the same 120s budget as `wait_ready` in `datastore_proc.py`.
    # A dead server is detected early, so the deadline only affects hung servers.
    for _ in {1..120}
    do
        if datastore-client --receive_timeout=5 --query "select 1" || ! kill -0 $server_bg_pid
        then
            break
        fi
        sleep 1
    ...
    while :; do
        ...
        timeout --verbose --signal TERM --kill-after=5m --preserve-status "${remaining_seconds}s" datastore-client \
        ...
        if ! datastore-client --receive_timeout=5 --query "SELECT 'fuzzer restart liveness check'"; then
