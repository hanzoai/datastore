...
def check():
    print("Remove all images with names starting with datastore/datastore- (forced)")
    Shell.check(
        "docker images --format '{{.Repository}}:{{.Tag}}' | grep '^datastore/datastore-' | xargs -r docker rmi -f",
        verbose=True,
    )
