...
<vector_similarity_index_cache_size_ratio>0.99</vector_similarity_index_cache_size_ratio>
    ...
    def create_minio_log_tables(self):
        ...
        #
        # storage_policy = 'default' pins the diagnostic table to local disk.
        # On s3 storage runs the default merge_tree policy is S3
        # (s3_storage_policy_for_merge_tree_by_default.xml), and these are
        # diagnostics ABOUT S3 activity; there is no reason to store them ON S3.
        # 'default' is a local policy on every stateless config (no config
        # remaps it), so this is a no-op on non-s3 runs.
        setup_steps = [
            (
