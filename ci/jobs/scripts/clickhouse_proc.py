...
"""
    ...
    def _reset_minio_log_disk_dirs():
        """Remove the minio log table's pinned disk directory before re-creating it.

        This lives outside run_path* (see create_minio_log_tables), so it can
        survive across restarts; a stale or planted symlink at the path must
    ...
    def create_minio_log_tables(self):
        ...
        #
        # Only the server logs are collected. The audit webhook emitted one
        # event per S3 API request, so on s3 runs system.minio_audit_logs grew
        # to millions of rows and its dump hung past the 600s cap; the stream is
        # no longer needed and is dropped at the source.
        #
        # minio_server_logs must live on LOCAL disk: it is a diagnostic ABOUT S3
        # activity, and on s3 runs the default merge_tree policy is S3. An
        # explicit `disk = disk(type = 'local', ...)` pins it under
        # /var/lib/clickhouse/disks/ in both the public and private/cloud repos,
        # unlike `storage_policy = 'default'` (local in public, cloud in private).
        setup_steps = [
            (
                # start() wipes only run_path*, so a restart destroys this
                # table's metadata while its data - pinned to a disk outside
                # run_path* - survives. Both callers run right after a fresh
                # start(), so no live table references the directory; wipe it so
                # a dead incarnation's parts are not carried over.
...
"""
    ...
    def _timeout_wrapper_expired(cls, res, stderr):
        ...
        if res == 124:
            return True
        if res == 137:
            return cls._TIMEOUT_KILL_DIAG in (stderr or "")
        return False
