...
# DATASTORE_CI_LOGS_HOST - remote host
# DATASTORE_CI_LOGS_USER - password for user
# DATASTORE_CI_LOGS_PASSWORD - password for user

# Pre-configured destination cluster, where to export the data
DATASTORE_CI_LOGS_CLUSTER=${DATASTORE_CI_LOGS_CLUSTER:-system_logs_export}
...
function __set_connection_args()
{
    ...
    CONNECTION_ARGS=(
        ...
        --user "${DATASTORE_CI_LOGS_USER:?}" --host "${DATASTORE_CI_LOGS_HOST:?}"
...
function __shadow_credentials()
{
    ...
    exec &> >(perl -pe '
        s(\Q$ENV{DATASTORE_CI_LOGS_HOST}\E)[DATASTORE_CI_LOGS_HOST]g;
        s(\Q$ENV{DATASTORE_CI_LOGS_USER}\E)[DATASTORE_CI_LOGS_USER]g;
        s(\Q$ENV{DATASTORE_CI_LOGS_PASSWORD}\E)[DATASTORE_CI_LOGS_PASSWORD]g;
    ')
}
...
function check_logs_credentials()
{
    ...
    datastore-client "${CONNECTION_ARGS[@]:?}" -q 'SELECT 1 FORMAT Null' && return 0 || code=$?
    if [ "$code" != 0 ]; then
...
function setup_logs_replication()
{
    ...
    if [[ -n "$DATASTORE_CI_LOGS_HOST" ]]; then
    ...
    debug_or_sanitizer_build=$(datastore-client -q "WITH ((SELECT value FROM system.build_options WHERE name='BUILD_TYPE') AS build, (SELECT value FROM system.build_options WHERE name='CXX_FLAGS') as flags) SELECT build='Debug' OR flags LIKE '%fsanitize%'")
    echo "Build is debug or sanitizer: $debug_or_sanitizer_build"

    # For each system log table:
    echo 'Create %_log tables'
    datastore-client --query "SHOW TABLES FROM system LIKE '%\\_log'" | while read -r table
    do
        ...
        statement=$(datastore-client --format TSVRaw --query "SHOW CREATE TABLE system.${table}" | sed -r -e '
        ...
        echo "$statement" | datastore-client --database_replicated_initial_query_timeout_sec=10 \
        ...
        datastore-client --query "
            ...
            ENGINE = Distributed(${DATASTORE_CI_LOGS_CLUSTER}, default, ${table}_${hash})
...
function stop_logs_replication()
{
    ...
    datastore-client --query "select database||'.'||table from system.tables where database = 'system' and (table like '%_sender' or table like '%_watcher')" | {
        tee /dev/stderr
    } | {
        timeout --verbose --preserve-status --signal TERM --kill-after 5m 15m xargs -n1 -P10 -r -i datastore-client --query "drop table {}"
    }
}
