...
class Targeting:
    ...
    def _tests_owning_data_file(cls, fpath: str):
        ...
        pattern that matches no test would make `datastore-test` exit 1 (the
    ...
    def functional_test_hash_batch_file(cls, fpath: str):
        ...
        `datastore-test --run-by-hash-*` uses to bucket the given changed path,
        or `None` if it cannot be resolved to a concrete test source file.

        `datastore-test`'s `is_test_from_dir`/`get_selected_tests` only look
    ...
    def is_sequential_functional_test(test_source_file: str) -> bool:
        ...
        Mirrors `datastore-test`'s own `is_sequential_test`/tag-parsing logic,
    ...
    def get_changed_tests(self):
        ...
        for fpath in changed_files:
            ...
            # pattern (which would make datastore-test exit 1).
