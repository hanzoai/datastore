...
class ClickHouseService:
    def __init__(
        self,
        ch_config_dir: str = f"{temp_dir}/etc/datastore-server",
        ch_var_lib_dir: str = f"{temp_dir}/var/lib/datastore",
        ...
        self.pid_file = f"{ch_config_dir}/datastore-server.pid"
    ...
    def __enter__(self):
        ...
        clickhouse_bin = Path(temp_dir) / "datastore"
        if not clickhouse_bin.exists():
            self._download_binary()

        # Create symlinks if absent
        for link_name in ("datastore-server", "datastore-client", "datastore-local"):
        ...
        argv = [
            str(Path(temp_dir) / "datastore-server"),
        ...
        with open(f"{self.log_dir}/datastore-server.log", "w") as log_fd:
    ...
    def _download_binary() -> None:
        dest = Path(temp_dir) / "datastore"
        if dest.exists():
            print(f"Datastore binary already present at [{dest}], skipping download")
            dest.chmod(0o755)
            return
        arch = "aarch64" if Utils.is_arm() else "amd64"
        url = f"https://datastore-builds.s3.us-east-1.amazonaws.com/master/{arch}/datastore"
    ...
    def _print_server_log(self) -> None:
        log_path = Path(self.log_dir) / "datastore-server.log"
        if log_path.exists():
            print(f"--- {log_path} ---")
