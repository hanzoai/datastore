...
remote_servers:
    {DATASTORE_CI_LOGS_CLUSTER}:
        shard:
            replica:
                secure: 1
                user: '{DATASTORE_CI_LOGS_USER}'
                host: '{DATASTORE_CI_LOGS_HOST}'
                port: 9440
                password: '{DATASTORE_CI_LOGS_PASSWORD}'
"""
DATASTORE_CI_LOGS_CLUSTER = "system_logs_export"
...
class ClickHouseProc:
    ...
    CH_LOCAL_LOG = f"{temp_dir}/datastore-local.log"
    CH_LOCAL_ERR_LOG = f"{temp_dir}/datastore-local.err.log"
    ...
    def __init__(
        ...
        ch_config_dir="/etc/datastore-server",
        ch_var_lib_dir="/var/lib/datastore",
    ):
        ...
        self.log_dir = f"{temp_dir}/var/log/datastore-server"
        self.pid_file = f"{self.ch_config_dir}/datastore-server.pid"
        ...
        self.command = f"datastore-server --config-file {self.config_file} --pid-file {self.pid_file} -- --path {self.run_path0} --user_files_path {self.user_files_path} --top_level_domains_path {self.ch_config_dir}/top_level_domains --logger.stderr {self.log_dir}/stderr.log"
        self.ch_config_dir_replica_1 = f"/etc/datastore-server1"
        self.config_file_replica_1 = f"{self.ch_config_dir_replica_1}/config.xml"
        self.ch_config_dir_replica_2 = f"/etc/datastore-server2"
        self.config_file_replica_2 = f"{self.ch_config_dir_replica_2}/config.xml"
        self.pid_file = f"{self.ch_config_dir}/datastore-server.pid"
        self.pid_file_replica_1 = (
            f"{self.ch_config_dir_replica_1}/datastore-server.pid"
        )
        self.pid_file_replica_2 = (
            f"{self.ch_config_dir_replica_2}/datastore-server.pid"
        )
        self.pid_0 = 0
        self.pid_1 = 0
        self.pid_2 = 0
        ...
        self.port_1 = 19000
        self.port_2 = 29000
        self.replica_command_1 = f"datastore-server --config-file {self.config_file_replica_1} --pid-file {self.pid_file_replica_1} -- --path {self.run_path1} --user_files_path {self.user_files_path} --logger.stderr {self.log_dir}/stderr1.log --logger.log {self.log_dir}/datastore-server1.log --logger.errorlog {self.log_dir}/datastore-server1.err.log --tcp_port {self.port_1} --tcp_port_secure 19440 --http_port 18123 --https_port 18443 --interserver_http_port 19009 --tcp_with_proxy_port 19010 --mysql_port 19004 --postgresql_port 19005 --keeper_server.tcp_port 19181 --keeper_server.server_id 2 --prometheus.port 19988 --macros.replica r2"
        self.replica_command_2 = f"datastore-server --config-file {self.config_file_replica_2} --pid-file {self.pid_file_replica_2} -- --path {self.run_path2} --user_files_path {self.user_files_path} --logger.stderr {self.log_dir}/stderr2.log --logger.log {self.log_dir}/datastore-server2.log --logger.errorlog {self.log_dir}/datastore-server2.err.log --tcp_port {self.port_2} --tcp_port_secure 29440 --http_port 28123 --https_port 28443 --interserver_http_port 29009 --tcp_with_proxy_port 29010 --mysql_port 29004 --postgresql_port 29005 --keeper_server.tcp_port 29181 --keeper_server.server_id 3 --prometheus.port 29988 --macros.shard s2"
        self.proc = None
        self.proc_1 = None
        self.proc_2 = None
        self.pid = 0
        ...
        Utils.set_env("DATASTORE_CONFIG_DIR", self.ch_config_dir)
        Utils.set_env("DATASTORE_CONFIG", self.config_file)
        Utils.set_env(
            "DATASTORE_SCHEMA_FILES", f"{self.ch_var_lib_dir}/format_schemas"
        )
        Utils.set_env("DATASTORE_USER_FILES", f"{self.user_files_path}")
...
""")
    ...
    def log_cluster_config():
        return Shell.check(
            f"./ci/jobs/scripts/functional_tests/setup_log_cluster.sh --config-logs-export-cluster ./tmp_ci/etc/datastore-server/config.d/system_logs_export.yaml",
            verbose=True,
        )
...
"""
        file_path = "/etc/datastore-server/config.d/max_server_memory_usage_to_ram_ratio.xml"
    ...
    def _install_light(self):
        """
        Installs Datastore config into ci temporary directory, this way of installation does not require mounting /etc|var/datastore-server into docker container.
        ...
        commands = [
            ...
            f"ln -sf {temp_dir}/datastore {temp_dir}/datastore-server",
            f"ln -sf {temp_dir}/datastore {temp_dir}/datastore-client",
        ]
        res = True
        for command in commands:
            res = res and Shell.check(command, verbose=True)
        if not res:
            print("Failed to install Datastore config")

        return res

    def start_light(self):
        """
        Start Datastore server with config installed with _install_config()
        """
        print(f"Starting Datastore server")
        # check binary available and do decompression in the meantime
        assert Shell.check("datastore --version", verbose=True)
        self.pid_file = f"{temp_dir}/datastore-server.pid"
        self.start_cmd = f"{temp_dir}/datastore-server --config-file={temp_dir}/config.xml --pid-file {self.pid_file}"
        print("Command: ", self.start_cmd)
        self.log_fd = open(f"{self.log_dir}/datastore-server.log", "w")
...
<vector_similarity_index_cache_size_ratio>0.99</vector_similarity_index_cache_size_ratio>
    ...
    def create_log_export_config(self):
        ...
        config_content = LOG_EXPORT_CONFIG_TEMPLATE.format(
            DATASTORE_CI_LOGS_CLUSTER=DATASTORE_CI_LOGS_CLUSTER,
            DATASTORE_CI_LOGS_HOST=self.log_export_host,
            DATASTORE_CI_LOGS_USER=DATASTORE_CI_LOGS_USER,
            DATASTORE_CI_LOGS_PASSWORD=self.log_export_password,
        )
    ...
    def start_log_exports(self, check_start_time):
        ...
        if self.log_export_host:
            os.environ["DATASTORE_CI_LOGS_CLUSTER"] = DATASTORE_CI_LOGS_CLUSTER
            os.environ["DATASTORE_CI_LOGS_HOST"] = self.log_export_host
            os.environ["DATASTORE_CI_LOGS_USER"] = DATASTORE_CI_LOGS_USER
            os.environ["DATASTORE_CI_LOGS_PASSWORD"] = self.log_export_password
        info = Info()
    ...
    def create_minio_log_tables(self):
        ...
        res = Shell.check(
            'datastore-client --enable_json_type=1 --query "CREATE TABLE system.minio_audit_logs (log JSON(time DateTime64(9))) ENGINE = MergeTree ORDER BY tuple()"',
            verbose=True,
        )
        res = res and Shell.check(
            'datastore-client --enable_json_type=1 --query "CREATE TABLE system.minio_server_logs (log JSON(time DateTime64(9))) ENGINE = MergeTree ORDER BY tuple()"',
            verbose=True,
        )
    ...
    def wait_ready(self, replica_num=0):
        ...
        if replica_num == 1:
            ...
            port = self.port_1
            proc = self.proc_1
            err_log = f"{self.log_dir}/datastore-server.err.1.log"
        elif replica_num == 2:
            pid_file = self.pid_file_replica_2
            port = self.port_2
            proc = self.proc_2
            err_log = f"{self.log_dir}/datastore-server.err.2.log"
        elif replica_num == 0:
            pid_file = self.pid_file
            port = self.port
            proc = self.proc
            err_log = f"{self.log_dir}/datastore-server.err.log"
        else:
            assert False
        i = 0
        self.pid = None
        ...
        for attempt in range(attempts):
            res, out, err = Shell.get_res_stdout_stderr(
                f'datastore-client --port {port} --query "select 1"', verbose=True
            )
            if out.strip() == "1":
    ...
    def _flush_system_logs(self):
        for proc, port in zip(
            ...
            if proc:
                res = Shell.check(
                    f'datastore-client --port {port} --query "system flush logs"',
                    verbose=True,
                )
                Shell.check(
                    f'datastore-client --port {port} --query "SYSTEM FLUSH ASYNC INSERT QUEUE"',
                    verbose=True,
                )
                if not res:
                    return False
...
datastore-client --query "SHOW DATABASES"
datastore-client --query "CREATE DATABASE datasets"
datastore-client < ./tests/docker_scripts/create.sql
...
if [[ -n "$USE_S3_STORAGE_FOR_MERGE_TREE" ]] && [[ "$USE_S3_STORAGE_FOR_MERGE_TREE" -eq 1 ]]; then
    ...
    datastore-client --query "RENAME TABLE datasets.hits_v1 TO test.hits"
    datastore-client --query "RENAME TABLE datasets.visits_v1 TO test.visits"
fi
datastore-client --query "CREATE TABLE test.hits_s3  (WatchID UInt64, JavaEnable UInt8, Title String, GoodEvent Int16, EventTime DateTime, EventDate Date, CounterID UInt32, ClientIP UInt32, ClientIP6 FixedString(16), RegionID UInt32, UserID UInt64, CounterClass Int8, OS UInt8, UserAgent UInt8, URL String, Referer String, URLDomain String, RefererDomain String, Refresh UInt8, IsRobot UInt8, RefererCategories Array(UInt16), URLCategories Array(UInt16), URLRegions Array(UInt32), RefererRegions Array(UInt32), ResolutionWidth UInt16, ResolutionHeight UInt16, ResolutionDepth UInt8, FlashMajor UInt8, FlashMinor UInt8, FlashMinor2 String, NetMajor UInt8, NetMinor UInt8, UserAgentMajor UInt16, UserAgentMinor FixedString(2), CookieEnable UInt8, JavascriptEnable UInt8, IsMobile UInt8, MobilePhone UInt8, MobilePhoneModel String, Params String, IPNetworkID UInt32, TraficSourceID Int8, SearchEngineID UInt16, SearchPhrase String, AdvEngineID UInt8, IsArtifical UInt8, WindowClientWidth UInt16, WindowClientHeight UInt16, ClientTimeZone Int16, ClientEventTime DateTime, SilverlightVersion1 UInt8, SilverlightVersion2 UInt8, SilverlightVersion3 UInt32, SilverlightVersion4 UInt16, PageCharset String, CodeVersion UInt32, IsLink UInt8, IsDownload UInt8, IsNotBounce UInt8, FUniqID UInt64, HID UInt32, IsOldCounter UInt8, IsEvent UInt8, IsParameter UInt8, DontCountHits UInt8, WithHash UInt8, HitColor FixedString(1), UTCEventTime DateTime, Age UInt8, Sex UInt8, Income UInt8, Interests UInt16, Robotness UInt8, GeneralInterests Array(UInt16), RemoteIP UInt32, RemoteIP6 FixedString(16), WindowName Int32, OpenerName Int32, HistoryLength Int16, BrowserLanguage FixedString(2), BrowserCountry FixedString(2), SocialNetwork String, SocialAction String, HTTPError UInt16, SendTiming Int32, DNSTiming Int32, ConnectTiming Int32, ResponseStartTiming Int32, ResponseEndTiming Int32, FetchTiming Int32, RedirectTiming Int32, DOMInteractiveTiming Int32, DOMContentLoadedTiming Int32, DOMCompleteTiming Int32, LoadEventStartTiming Int32, LoadEventEndTiming Int32, NSToDOMContentLoadedTiming Int32, FirstPaintTiming Int32, RedirectCount Int8, SocialSourceNetworkID UInt8, SocialSourcePage String, ParamPrice Int64, ParamOrderID String, ParamCurrency FixedString(3), ParamCurrencyID UInt16, GoalsReached Array(UInt32), OpenstatServiceName String, OpenstatCampaignID String, OpenstatAdID String, OpenstatSourceID String, UTMSource String, UTMMedium String, UTMCampaign String, UTMContent String, UTMTerm String, FromTag String, HasGCLID UInt8, RefererHash UInt64, URLHash UInt64, CLID UInt32, YCLID UInt64, ShareService String, ShareURL String, ShareTitle String, ParsedParams Nested(Key1 String, Key2 String, Key3 String, Key4 String, Key5 String, ValueDouble Float64), IslandID FixedString(16), RequestNum UInt32, RequestTry UInt8) ENGINE = MergeTree() PARTITION BY toYYYYMM(EventDate) ORDER BY (CounterID, EventDate, intHash32(UserID)) SAMPLE BY intHash32(UserID) SETTINGS index_granularity = 8192, storage_policy='s3_cache'"
# AWS S3 is very inefficient, so increase memory even further:
datastore-client --max_estimated_execution_time 0 --max_execution_time "$MAX_EXECUTION_TIME" --max_memory_usage 30G --max_memory_usage_for_user 30G --query "INSERT INTO test.hits_s3 SELECT * FROM test.hits SETTINGS enable_filesystem_cache_on_write_operations=0, write_through_distributed_cache=0, max_insert_threads=16"

datastore-client --query "CREATE TABLE test.hits_parquet (Title String, URL String, Referer String, SearchPhrase String, WatchID UInt64, UserID UInt64, CounterID UInt32, EventTime DateTime, EventDate Date, RegionID UInt32, ClientIP UInt32) ENGINE = S3('https://datastore-public-datasets.s3.eu-central-1.amazonaws.com/hits_compatible/hits.parquet', NOSIGN)"

datastore-client --query "SHOW TABLES FROM test"
datastore-client --query "SELECT count() FROM test.hits"
datastore-client --query "SELECT count() FROM test.visits"
"""
        if with_s3_storage:
    ...
    def insert_system_zookeeper_config(self):
        for _ in range(10):
            res = Shell.check(
                f"datastore-client --query \"insert into system.zookeeper (name, path, value) values ('auxiliary_zookeeper2', '{temp_dir}/chroot/', '')\"",
                verbose=True,
            )
            time.sleep(1)
            if res:
    ...
    def terminate(self, force=False):
        if self.minio_proc:
            # remove the webhook so it doesn't spam with errors once we stop Datastore
            Shell.check(
        ...
        print("Terminate Datastore processes")

        Shell.check(f"ps -ef | grep  datastore")
        for proc, pid_file, pid, run_path in (
            ...
            if proc and pid:
                if force:
                    # Use datastore stop --force when this issue is fixed
                    # https://github.com/ClickHouse/Datastore/issues/99142
                    proc.terminate()
                    try:
                    ...
                    f"cd {run_path} && datastore stop --pid-path {Path(pid_file).parent} --max-tries 300 --do-not-kill >/dev/null",
                    verbose=True,
                ):
                    continue
                print(
    ...
    def _get_jemalloc_profiles(cls):
        ...
        # format of jemalloc profile: datastore.jemalloc.$PID.$COUNT.m$COUNT.heap
        # test runs can generate jemalloc profiles for multiple PIDs because datastore local and multiple servers
        # can be started
        ...
        chbinary = Shell.get_output("readlink -f $(which datastore)")
    ...
    def check_fatal_messages_in_logs(self):
        ...
        fatal_hits = Shell.get_output(
            f"cd {self.log_dir} && grep -a '<Fatal>' datastore-server*.log 2>/dev/null | head -n 1 || true"
        )
        if sanitizer_hits or fatal_hits:
            server_log = pick_latest_file(
                "datastore-server*.err.log"
            ) or pick_latest_file("datastore-server*.log")
        ...
        results.append(
            Result.from_commands_run(
                name="Lost s3 keys",
                command=f"cd {self.log_dir} && ! grep -a 'Code: 499.*The specified key does not exist' datastore-server*.log | grep -v -e 'a.myext' -e 'ReadBuffer is canceled by the exception' -e 'DistributedCacheTCPHandler' -e 'ReadBufferFromDistributedCache' -e 'ReadBufferFromS3' -e 'ReadBufferFromAzureBlobStorage' -e 'AsynchronousBoundedReadBuffer' -e 'caller id: None:DistribCache' | head -n100 | tee /dev/stderr | grep -q .",
            )
        )
        results.append(
            Result.from_commands_run(
                name="Lost forever for SharedMergeTree",
                command=f"cd {self.log_dir} && ! grep -a 'it is lost forever' datastore-server*.log | head -n100 | tee /dev/stderr | grep -q .",
            )
        )
        results.append(
            Result.from_commands_run(
        ...
        results.append(
            Result.from_commands_run(
                name="S3_ERROR No such key thrown (in datastore-server.log or datastore-server.err.log)",
                command=f"cd {self.log_dir} && ! grep -a 'Code: 499.*The specified key does not exist' datastore-server*.log | grep -v -e 'a.myext' -e 'ReadBuffer is canceled by the exception'  -e 'DistributedCacheTCPHandler' -e 'ReadBufferFromDistributedCache' -e 'ReadBufferFromS3' -e 'ReadBufferFromAzureBlobStorage' -e 'AsynchronousBoundedReadBuffer' -e 'caller id: None:DistribCache' | head -n100 | tee /dev/stderr | grep -q .",
            )
        )
    ...
    def dump_system_tables(self):
        # Stop server so we can safely read data with datastore-local.
        # Why do we read data with datastore-local?
        ...
        # Shell.check("rm /etc/datastore-server/users.d/limits.yaml", verbose=True)
        # Shell.check("datastore-client -q \"system reload config\" ||:", verbose=True)
        TABLES = [
            "query_log",
            "zookeeper_log",
        ...
        Shell.check(
            f"sed -i 's|<log>.*</log>|<log>{self.CH_LOCAL_LOG}</log>|' /etc/datastore-server/config.xml"
        )
        Shell.check(
            f"sed -i 's|<errorlog>.*</errorlog>|<errorlog>{self.CH_LOCAL_ERR_LOG}</errorlog>|' /etc/datastore-server/config.xml"
        )
        ...
        for table in TABLES:
            ...
            res, stdout, stderr = Shell.get_res_stdout_stderr(
                f"cd {self.run_path0} && datastore local {command_args} {path_arg} --query \"select * from system.{table} into outfile '{temp_dir}/system_tables/{table}.tsv' format TSVWithNamesAndTypes\" {command_args_post}",
                verbose=True,
            )
            if res != 0:
            ...
            if self.is_shared_catalog or self.is_db_replicated:
                ...
                res, stdout, stderr = Shell.get_res_stdout_stderr(
                    f"cd {self.run_path1} && datastore local {command_args} {path_arg} --query \"select * from system.{table} into outfile '{temp_dir}/system_tables/{table}.1.tsv' format TSVWithNamesAndTypes\" {command_args_post}",
                    verbose=True,
                )
                if res != 0:
                    print(
            ...
            if self.is_db_replicated:
                ...
                res, stdout, stderr = Shell.get_res_stdout_stderr(
                    f"cd {self.run_path2} && datastore local {command_args} {path_arg} --query \"select * from system.{table} into outfile '{temp_dir}/system_tables/{table}.2.tsv' format TSVWithNamesAndTypes\" {command_args_post}",
                    verbose=True,
                )
                if res != 0:
                    print(
    ...
    def save_system_metadata_files_from_remote_database_disk(self):
        if not os.path.exists(
            "/etc/datastore-server/config.d/remote_database_disk.xml"
        ):
            return

        # Store system database and table metadata files
        system_db_uuid = Shell.get_output(
            "datastore disks -C /etc/datastore-server/config.xml --disk disk_db_remote -q 'read metadata/system.sql' | grep -F UUID | awk -F\"'\" '{print $2}'",
            verbose=True,
        )
        ...
        self.system_db_sql = Shell.get_output(
            "datastore disks -C /etc/datastore-server/config.xml --disk disk_db_remote -q 'read metadata/system.sql'",
            verbose=True,
        )
        ...
        system_table_sql_files = (
            Shell.get_output(
                f"datastore disks -C /etc/datastore-server/config.xml --disk disk_db_remote -q 'ls store/{self.system_db_uuid[:3]}/{self.system_db_uuid}/'",
                verbose=True,
            )
            .strip()
            .split("\n")
        ...
        for system_table_sql_file in system_table_sql_files:
            ...
            sql_content = Shell.get_output(
                f"datastore disks -C /etc/datastore-server/config.xml --disk disk_db_remote -q 'read store/{self.system_db_uuid[:3]}/{self.system_db_uuid}/{system_table_sql_file}'",
                verbose=True,
            )
    ...
    def restore_system_metadata_files_from_remote_database_disk(self):
        ...
        if os.path.exists("/etc/datastore-server/config.d/remote_database_disk.xml"):
            os.remove("/etc/datastore-server/config.d/remote_database_disk.xml")

        # Restore system database and table metadata files for `datastore local`
