...
class ClickHouseSetup:
    def __init__(self, work_dir, binary_path=""):
        ...
        self.server_log = self.work_dir / "datastore-server.log"
        self.server_err_log = self.work_dir / "datastore-server.err.log"
        self.client_log = self.work_dir / "client.log"
        self.status_file = self.work_dir / "status"
        if not binary_path:
            self.binary_path = self.work_dir / "datastore"
        else:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--path", help="Path to the datastore binary", default="")
    parser.add_argument(
        "--test",
    ...
    Shell.check(f"rm -rf /home/max/work/Datastore/ci/tmp/wd/")
    Shell.check(f"chmod +x {args.path}")
    Shell.check(f"{args.path} --version", strict=True)

    def cleanup():
        Shell.check(f"pkill -f 'datastore server'", verbose=True, strict=False)
