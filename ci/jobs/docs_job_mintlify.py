...
def _protected_docs_guard():
    ...
    info = Info()
    if "pr-autogenerated-docs" in info.pr_labels:
        print("OK: skipping protected docs guards for autogenerated docs.")
        return True
    changed_files = info.get_changed_files()
    if changed_files is None:
    ...
    if not merge_base:
        print("Error: the PR merge base is unavailable, cannot run the check.")
        return False
