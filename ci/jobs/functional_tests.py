...
from ci.jobs.scripts.datastore_proc import DatastoreProc
...
def stateless_memory_limit(source):
    """Per-test cgroup memory limit (`datastore-test --memory-limit`) for a run
...
def run_tests(
    ...
    # Remove --report-logs-stats, it hides sanitizer errors in def reportLogStats(args): datastore_execute(args, "SYSTEM FLUSH LOGS")
    ...
    # Hand the time budget to `datastore-test` itself via `--global_time_limit`
    ...
    # own per-test alarm window when the deadline is reached: `datastore-test`
...
def main():
    ...
    if args.count:
        ...
        # setting randomization added to `tests/datastore-test` after the PR's
    ...
    if is_bugfix_validation:
        ...
        bt_paths = {bt: f"{temp_dir}/datastore_{bt}" for bt in build_types}
        ...
        Shell.run(
            f"cp {temp_dir}/datastore_{build_types[0]} {temp_dir}/datastore",
            verbose=True,
            strict=True,
        )
    elif args.path:
    ...
    stage = args.param or JobStages.INSTALL_DATASTORE
    if stage:
    ...
    if res and JobStages.INSTALL_DATASTORE in stages:
        ...
        commands = [
            "rm -rf /etc/datastore-client/* /etc/datastore-server/* /etc/datastore-server1/* /etc/datastore-server2/*",
            # google *.proto files
            "mkdir -p /usr/share/datastore/ && ln -sf /usr/local/include /usr/share/datastore/protos",
            ...
            "cp programs/server/config.xml programs/server/users.xml /etc/datastore-server/",
        ...
        # `datastore-client` processes alive at once (~0.4 GiB each, ~17 GiB in
    ...
    if res and JobStages.START in stages:
        ...
        def start():
            ...
            # at each failure point so the opaque "Start Datastore Server"
            ...
            if not (CH.start_minio(test_type="stateless") and CH.start_azurite()):
                ...
                return False
            if not CH.start():
                print("SETUP FAILURE: datastore-server process did not start")
                return False
            if not CH.wait_ready():
                ...
                print("SETUP FAILURE: datastore-server not ready (wait_ready)")
                return False
        ...
        if is_bugfix_validation:
            ...
            if test_result.is_ok():
                for bugfix_bt in build_types[1:]:
                    ...
                    # process (transient `datastore-client` invocations, stray
                    ...
                    CH.stop_server()
                    Shell.run(
                        f"rm -f {ch_path}/datastore && "
                        f"cp {temp_dir}/datastore_{bugfix_bt} {ch_path}/datastore",
                        verbose=True,
                        strict=True,
                    )
                    Shell.run(
                    ...
                    # stage does via `datastore-server --version` - instead of
                    ...
                    # time out; worse, a later `datastore local` (log scraping)
                    ...
                    if has_stateful_tests:
                        ...
                        if not CH.prepare_stateful_data(
                            ...
                            # Prefer the concrete sub-command + Datastore error
