...
This directory contains the CI (Continuous Integration) system for Datastore based on the `praktika` framework. The CI system is responsible for automated building, testing, and deployment of Datastore.

## Directory Structure

- `./ci/praktika/` - Core praktika module that provides generic CI functionality. This module is designed to be reusable and must not include any Datastore-specific logic.
...
- `./ci/docker/` - Contains Dockerfiles and related resources for building container images used in Datastore tests.
